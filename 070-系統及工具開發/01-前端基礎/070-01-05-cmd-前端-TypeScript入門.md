...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[070-01-03-cmd-前端-JavaScript基礎]]", "[[070-01-04-cmd-前端-JavaScript非同步與API]]"]
updated: 2026-08-28
---
...
> [!abstract] 這篇你會學到
> - TypeScript 到底解決什麼問題（把 [[070-01-03-cmd-前端-JavaScript基礎]] 的四個陷阱擋在上線前）
...
- [[070-01-03-cmd-前端-JavaScript基礎]] —— TS 是 JS 的超集合，先會 JS 才有意義
- [[070-01-04-cmd-前端-JavaScript非同步與API]] —— `Promise<T>` 的型別會用到
- [[070-01-06-svc-前端-前端建置工具與套件管理]] —— TS 幾乎都跟 Vite / npm 一起出現
...
回顧 [[070-01-03-cmd-前端-JavaScript基礎]] 提到的陷阱，在 JS 裡它們全都要等**執行到那一行**才爆炸：
...
把 [[070-01-04-cmd-前端-JavaScript非同步與API]] 的 API 客戶端改寫成 TypeScript。
...
| XSS | ★★★★★ 見 [[070-01-03-cmd-前端-JavaScript基礎]] 安全性章節 |
...
- ★★★ **`@types/*` 也是第三方套件**，同樣受供應鏈攻擊影響。
  用 `npm audit`、`npm ci`（鎖定 lockfile）控管。見 [[070-01-06-svc-前端-前端建置工具與套件管理]]。
...
> 把 [[070-01-03-cmd-前端-JavaScript基礎]] 的 `取得顯示資料()` 加上完整型別標註（含參數與回傳值），
> 並用 `tsc --noEmit` 驗證。
>
...
- [[070-01-03-cmd-前端-JavaScript基礎]] —— TS 要解決的那些 JS 陷阱
- [[070-01-04-cmd-前端-JavaScript非同步與API]] —— 這篇改寫的 API 客戶端原型
- [[070-01-06-svc-前端-前端建置工具與套件管理]] —— Vite、npm 與供應鏈安全
- [[070-01-07-cmd-前端-瀏覽器開發者工具]] —— source map 與線上除錯
