...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[01-HTML結構與語意]]", "[[02-CSS版面與排版]]"]
updated: 2026-08-28
---
...
- [[01-HTML結構與語意]] —— DOM 操作的對象就是 HTML 的元素樹
- [[02-CSS版面與排版]] —— JS 改樣式時實際上是在改 CSS 的優先權
- 會開瀏覽器的 F12 開發者工具（詳見 [[07-瀏覽器開發者工具]]）
...
| ★★★★ `Blocked by CORS policy` | 跨網域請求，伺服器沒回 `Access-Control-Allow-Origin` | 這是**伺服器端**問題，見 [[04-JavaScript非同步與API]] |
...
處理方式見 [[02-CSS版面與排版]] 的 cache busting 段落。
...
- ★★★ **`target="_blank"` 要配 `rel="noopener noreferrer"`**（見 [[01-HTML結構與語意]]）
...
> SyntaxError: Unexpected end of input
> ```
> 注意它抓不到邏輯錯誤與型別錯誤 —— 那要靠 ESLint 與 [[05-TypeScript入門]]。

---

## 延伸閱讀

- [[01-HTML結構與語意]] —— DOM 操作的對象
- [[02-CSS版面與排版]] —— 為什麼要改 class 而不是 style
- [[04-JavaScript非同步與API]] —— fetch、Promise 與 CORS
- [[05-TypeScript入門]] —— 用型別把這篇提到的陷阱擋在編譯期
- [[07-瀏覽器開發者工具]] —— Console 與 Network 的完整用法
