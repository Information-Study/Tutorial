...
| `Subject` 的 `CN` | 主體名稱 | **已被 SAN 取代**（見 [[04-CN與SAN設定與瀏覽器相容性]]） |
...
| **`self signed certificate in certificate chain`** | 自簽憑證未被信任 | 派送根憑證（見 [[09-根憑證派送與信任]]） |
| `certificate has expired` | 過期 | 續期；**檢查自動續期是否正常** |
| **`Hostname mismatch`** | **SAN 不含該網域** | 重新申請時加上（見 [[04-CN與SAN設定與瀏覽器相容性]]） |
...
> 產生 CSR 時**一定要在 SAN 中列出所有網域**（見 [[04-CN與SAN設定與瀏覽器相容性]]）。
>
...
- [[02-CSR產生與req設定檔]] — 下一步：產生 CSR
- [[04-CN與SAN設定與瀏覽器相容性]] — SAN 的完整說明
- [[06-自建根CA]] — 建立內部 CA
- [[09-根憑證派送與信任]] — 各平台的信任清單
- [[12-憑證生命週期管理]] — 自動化與監控
- [[06-Nginx-HTTPS與Certbot]] — 在 Nginx 上部署
