...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[020-01-16-cmd-Linux-網路基礎指令]]", "[[010-02-09-guide-網概-TCP與UDP]]"]
updated: 2026-08-28
---
...
- [[020-01-16-cmd-Linux-網路基礎指令]] —— `ip`、`ss`、`ping`、`nc`，本篇直接引用不重講
- [[010-02-09-guide-網概-TCP與UDP]] —— 三向交握，階段 ① 的失敗都是 TCP 層的事
- [[010-02-10-guide-網概-連接埠與應用層協定]] —— 為什麼 SSH 是 22/tcp
- [[060-01-04-03-guide-ss-netstat-與lsof]] —— 確認 sshd 有沒有在聽
- [[020-01-17-cmd-Linux-systemd服務管理]] —— `systemctl status ssh` 與 socket activation
...
| `Permission denied (publickey)` | ★★★★ ⑤ | 網路、加密、主機身分**全部正常** | 伺服器 `journalctl -u ssh`（見 [[020-02-01-02-cmd-SSH-金鑰認證與ssh-agent]]） |
...
★★★★ 兩種金鑰，兩個方向，不要搞混
  ...
  【使用者金鑰】        客戶端 → 證明給伺服器看「我是誰」
    ...
    → 完整寫在 [[020-02-01-02-cmd-SSH-金鑰認證與ssh-agent]]，本篇不重複
```
...
對照 HTTPS 的信任模型（見 [[090-01-01-guide-PKI-PKI與憑證基礎]]）：
...
> 這是判斷「我的 `~/.ssh/config` 到底生效沒」最快的方法（見 [[020-02-01-03-svc-SSH-客戶端設定檔]]）。
...
>
> 改埠的正確位置（改埠本身的討論在 [[020-02-01-04-svc-sshd-伺服器端設定]]，這裡只講「為什麼改了沒用」）：
> ```bash
...
> 遮掉版本字串是低價值的隱匿式安全，真正該做的是及時更新與限制來源 —— 見 [[020-02-01-07-svc-SSH-安全強化]]。
...
> ```
> 細節見 [[090-02-07-guide-防護-SELinux與AppArmor]]。★★ RHEL 系 `HashKnownHosts` 預設 `no`，可直接 grep。
...
★★★★★ 看到 HOST KEY CHANGED，先停手，跑這棵決策樹
  ...
  └────────▶ 【成因④：可能是真的 MITM】★★★★★
             ...
             → 立刻改用【帶外管道】取指紋；不符 → 資安事件通報，
               ...
               （見 [[090-05-03-guide-資安設備-入侵偵測與防禦IDS-IPS]]）
```
...
所以兩個 unit 都要加。日誌的完整運用見 [[020-01-19-guide-Linux-日誌系統]]。
...
>
> 機關的正確處理：
> ① ★★★★ 把設定**限定到那一台**（`Host` 區塊，見 [[020-02-01-03-svc-SSH-客戶端設定檔]]），不要寫在 `Host *`
...
這是貫穿整個 SSH 章節（尤其 [[020-02-01-04-svc-sshd-伺服器端設定]] 與 [[020-02-01-07-svc-SSH-安全強化]]）的基調。
**在你還連得進去的時候**，先把後路鋪好：
...
★★★ `at` 與 systemd timer 的替代寫法見 [[020-01-18-guide-Linux-排程工作]]。
...
| 11 | 主機時間正確 | `ssh sysadm@srv-web01 timedatectl` | `System clock synchronized: yes`（見 [[020-01-28-cmd-Linux-時間同步NTP與chrony]]） | ★★★ |
...
| ★★★★ `Permission denied (publickey).` | ①～④ 全通，**問題純在使用者認證** | 看 `ssh -v` 的 `Authentications that can continue`；伺服器端看 `journalctl -u ssh`。詳見 [[020-02-01-02-cmd-SSH-金鑰認證與ssh-agent]] |
...
| ★★★ `Connection timed out` | 封包被靜默丟棄：防火牆 DROP、路由不通、機器沒開 | `nc -zv`、`ping`、查 ufw/firewalld/資安設備；必要時 [[060-01-04-01-guide-tcpdump-基礎抓包]] |
...
| ★★★ 連線卡住五到十秒才出現密碼提示 | 伺服器對客戶端 IP 做**反向 DNS 查詢逾時**，或 GSSAPI 認證嘗試逾時 | 伺服器 `UseDNS no`；客戶端 `-o GSSAPIAuthentication=no`（設定值見 [[020-02-01-04-svc-sshd-伺服器端設定]]） |
...
| ★★★ `client_loop: send disconnect: Broken pipe` 閒置後斷線 | 中間 NAT／防火牆的連線表逾時清掉了閒置連線 | 客戶端 `ServerAliveInterval 60`／伺服器 `ClientAliveInterval`（見 [[020-02-01-03-svc-SSH-客戶端設定檔]]） |
...
- `active (listening)` 卻連不上 → **是防火牆**：`sudo ufw status verbose`，
  確認 `22/tcp ALLOW IN 192.168.20.0/24` 有涵蓋你的來源網段（見 [[090-02-02-guide-防火牆-ufw基礎與實務]]）
...
這樣**仍失敗** → 問題不在設定檔，最後手段是兩邊同時抓包（見 [[060-01-04-01-guide-tcpdump-基礎抓包]]）：
...
| 項目 | 做法 | 重要度 |
| --- | --- | --- |
| ★★★★ 稽核軌跡 | 登入成功／失敗都要進集中日誌（`journalctl -u ssh` → syslog → SIEM），保存期限依機關規定。見 [[090-05-09-guide-資安設備-日誌集中與SIEM]] | ★★★★ |
| ★★★★ 最小權限 | 不用 `root` 直連（`PermitRootLogin no`），用個人帳號登入後 `sudo`，**才有「誰做了什麼」的軌跡** | ★★★★ |
| ★★★★ 個資風險 | `ssh -vvv` 輸出、`tcpdump` 的 pcap 含內部主機名與帳號，**送廠商前要遮蔽**，不要貼公開論壇 | ★★★★ |
| ★★★ TWGCB 對應 | 政府組態基準對 SSH 有明確要求（停用 root 登入、限制認證方式、閒置逾時），見 [[090-06-03-guide-TWGCB-Linux項目分類詳解]] | ★★★ |
...
> [!tip] ★★★ 第一次連線之後立刻要做的三件事
> ① 建立金鑰認證並停用密碼登入 → [[020-02-01-02-cmd-SSH-金鑰認證與ssh-agent]]、[[020-02-01-04-svc-sshd-伺服器端設定]]
> ② 建立第二個具 sudo 權限的維運帳號（★★★★ 唯一帳號被鎖就沒救了）
> ③ 套用初始安全基線 → [[090-02-01-guide-防護-伺服器初始安全設定]]
...
> 見「`ssh -v` 的三段式判讀」與 [[020-02-01-02-cmd-SSH-金鑰認證與ssh-agent]]。
>
> **Q2.** **不能證明。** ★★★★★
...
- [[020-02-01-02-cmd-SSH-金鑰認證與ssh-agent]] —— 本篇之後的**必做下一步**：把密碼登入換成金鑰，
  以及 `Permission denied (publickey)` 的完整排查
- [[020-02-01-03-svc-SSH-客戶端設定檔]] —— 把本篇用到的 `-o` 旗標固化成 `~/.ssh/config`，
  包含老設備的相容設定要怎麼**限定在單一 Host** 而不污染全域
- [[020-02-01-04-svc-sshd-伺服器端設定]] —— 伺服器端的 `sshd_config`：改埠、限制來源、
  停用 root 登入，以及每一次改動的鎖門預防
- [[020-02-01-07-svc-SSH-安全強化]] —— 演算法清單的建議值、`fail2ban`、雙因素，
  本篇提到的「暫時降級旗標」在這裡有長期解法
- [[020-01-23-guide-Linux-Linux常見疑難排解]] —— 連不上時「網路 vs. 服務 vs. 權限」的通用分層排查法
- [[090-01-01-guide-PKI-PKI與憑證基礎]] —— 對照 HTTPS 的 CA 信任模型，理解 SSH 為什麼要 TOFU、
  以及 SSH CA 能解決什麼
- [[060-01-04-03-guide-ss-netstat-與lsof]] —— 確認 sshd 監聽狀態的完整用法，本篇只引用不重講
- [[060-01-06-01-guide-傳輸-scp與sftp傳輸]] —— 第一次連線建立信任之後的檔案傳輸，
  它們共用同一份 `known_hosts`
