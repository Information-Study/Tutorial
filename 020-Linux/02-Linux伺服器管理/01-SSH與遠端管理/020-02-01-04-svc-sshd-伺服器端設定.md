...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[020-02-01-02-cmd-SSH-金鑰認證與ssh-agent]]", "[[020-01-17-cmd-Linux-systemd服務管理]]"]
updated: 2026-08-28
---
...
- [[020-02-01-02-cmd-SSH-金鑰認證與ssh-agent]] —— 本篇所有建議都預設「你已經有一把能登入的金鑰」
- [[020-02-01-01-cmd-SSH-原理與第一次連線]] —— 認證流程與 host key 的角色
- [[020-01-17-cmd-Linux-systemd服務管理]] —— `reload` 與 `restart` 的差別、`systemctl edit` 建 drop-in
- [[020-02-01-03-svc-SSH-客戶端設定檔]] —— 客戶端怎麼配合伺服器端的埠與 `ControlMaster` 的陷阱
...
> 還有機會改回來。完整差異見 [[020-01-17-cmd-Linux-systemd服務管理]]。
...
「這台跟基準差在哪」（[[020-02-03-02-ref-標準化-基準設定與範本化]] 在 SSH 上的具體應用）：
...
但 `systemd-run` 的執行結果會寫進 journal，事後查得到（選型見 [[020-02-02-02-cmd-systemd-timer與cron選型]]）。
...
（見 [[020-02-01-03-svc-SSH-客戶端設定檔]]）。驗證成功立刻 `sudo systemctl stop sshd-rollback.timer`。
...
> 同樣的列表語意也適用於 unit 的 `ExecStart=`，見 [[020-02-02-01-svc-systemd-unit撰寫實戰]]。
...
最保險是用 syslog identifier `journalctl -t sshd -f`；監控與告警規則（[[020-01-19-guide-Linux-日誌系統]]、
[[100-01-02-guide-日誌-日誌集中與輪替]]）記得一起改，否則會出現「登入失敗告警突然歸零」的假象。
...
> 正解是用 `ProxyJump`（`ssh -J`）取代，見 [[020-02-01-03-svc-SSH-客戶端設定檔]]。
> 客戶端 `-L` / `-R` / `-D` 怎麼用見 [[020-02-01-05-cmd-SSH-隧道與埠轉發]]，本篇只講伺服器端「准不准」。
...
目錄不能可寫等細節）在 [[020-02-01-06-svc-SFTP-與受限使用者]]，本篇只說明這兩個指令可以放在 Match 裡。
...
> ```
> ★★★★ 稽核通常要求保存 6 個月以上，本機留不住就送集中日誌（[[020-01-19-guide-Linux-日誌系統]]、[[100-01-02-guide-日誌-日誌集中與輪替]]）。
> ★★★ **不要**用 `LogLevel DEBUG` 當常態設定 —— 官方手冊明確警告它違反使用者隱私。
> 自動封鎖（fail2ban）見 [[090-02-05-guide-防護-Fail2ban入侵防護]]，本篇不重複。

★★ 演算法設定（`KexAlgorithms` / `Ciphers` / `MACs` / `PubkeyAcceptedAlgorithms`）的建議清單、
`ssh-audit` 掃描、2FA、SSH CA **全部在 [[020-02-01-07-svc-SSH-安全強化]]**；本篇只教你怎麼查目前用的是什麼：
...
> `--reload`，見 [[090-02-04-guide-防火牆-firewalld]]（Ubuntu 側是 [[090-02-02-guide-防火牆-ufw基礎與實務]]）。
>
...
[[020-02-03-02-ref-標準化-基準設定與範本化]] 的檢核流程與 [[090-06-04-guide-TWGCB-Linux本機導入]] 的檢測項目。
...
> 用 `ProxyJump` 取代（[[020-02-01-03-svc-SSH-客戶端設定檔]]）。後者會連帶失效 `pam_faillock` 帳號鎖定、
...
>    本機留不住就送集中日誌（[[020-01-19-guide-Linux-日誌系統]]、[[100-01-02-guide-日誌-日誌集中與輪替]]）。
> 3. **最小權限**：`AllowGroups ssh-users` + `DisableForwarding yes`，需要的人用 `Match` 個別放行，
>    並在變更紀錄寫明理由（[[090-02-08-guide-防護-系統強化與稽核]]、[[090-07-09-guide-資安實踐-資安稽核與符合性檢核]]）。
> 4. **組態基準**：TWGCB 對 SSH 有明確項目（禁止 root 登入、閒置逾時、`PermitEmptyPasswords no`、
>    `LogLevel` 等），導入與檢測見 [[090-06-04-guide-TWGCB-Linux本機導入]] 與 [[090-06-07-guide-TWGCB-Linux檢測與符合性報告]]。
...
> 更好的是把 `/etc/ssh/sshd_config.d/` 納入版控或組態管理 —— 見 [[020-02-03-02-ref-標準化-基準設定與範本化]]
> 與 [[060-01-06-03-guide-傳輸-備份策略與還原演練]]。
...
> 見「觀念說明」與 [[020-01-17-cmd-Linux-systemd服務管理]]。
>
...
> 同樣的列表語意也適用 unit 的 `ExecStart=`，見 [[020-02-02-01-svc-systemd-unit撰寫實戰]]。
>
...
> 官方手冊明確警告它違反使用者隱私。見「日誌」與 [[020-01-19-guide-Linux-日誌系統]]。
>
...
> 演算法清單怎麼選見 [[020-02-01-07-svc-SSH-安全強化]]。此題見「RHEL 系對照」摺疊區塊。

## 延伸閱讀

- [[020-02-01-07-svc-SSH-安全強化]] —— 演算法清單、`ssh-audit`、2FA、SSH CA、fail2ban 整合，本篇刻意留給它的部分
- [[020-02-01-02-cmd-SSH-金鑰認證與ssh-agent]] —— 關掉密碼登入的前提，authorized_keys 選項與 agent 的正確用法
- [[020-02-01-06-svc-SFTP-與受限使用者]] —— `ChrootDirectory` + `ForceCommand internal-sftp` 的完整做法與權限地雷
- [[020-02-01-05-cmd-SSH-隧道與埠轉發]] —— 本篇只講伺服器端「准不准」，客戶端 `-L` / `-R` / `-D` 看這篇
- [[020-02-01-03-svc-SSH-客戶端設定檔]] —— `ProxyJump`、`ControlMaster`；驗證新設定時 `ControlPath=none` 為何重要
- [[020-01-17-cmd-Linux-systemd服務管理]] —— `reload` / `restart` / `systemctl edit` drop-in 的完整說明
- [[020-01-19-guide-Linux-日誌系統]] —— journald 與 rsyslog 的分工、保存期限、`journalctl` 進階過濾
- [[090-06-04-guide-TWGCB-Linux本機導入]] —— 政府組態基準對 SSH 的具體項目與導入方式
- [[020-02-03-02-ref-標準化-基準設定與範本化]] —— 把 `sshd_config.d/` 納入組態管理與版控的做法
