...
- [[16-網路基礎指令]] —— `ip`、`ss`、`ping`、`nc`，本篇直接引用不重講
- [[09-網概-TCP與UDP]] —— 三向交握，階段 ① 的失敗都是 TCP 層的事
- [[10-網概-連接埠與應用層協定]] —— 為什麼 SSH 是 22/tcp
- [[03-ss-netstat-與lsof]] —— 確認 sshd 有沒有在聽
- [[17-systemd服務管理]] —— `systemctl status ssh` 與 socket activation

> [!tip] 這篇的定位
> 本篇**不是**教「怎麼打 `ssh` 指令」，那三分鐘就會了。
...
★★★★ SSH 連線六階段（每一階段有專屬的失敗訊息）
  ...
  ⑤ 使用者認證（依序 publickey → keyboard-interactive → password）
...
> 但**排錯時把它們分開看非常有用**：`no matching host key type` 是「連要用哪種
> host key 都談不攏」（③）；`Host key verification failed` 是「談攏了、簽章也對，
> 但你的 known_hosts 不同意」（④）。兩者的處置方式完全不同，混在一起就會亂改設定。
...
★★★★ 兩種金鑰，兩個方向，不要搞混
  ...
  【host key 主機金鑰】  伺服器 → 證明給客戶端看「我是誰」
    ...
    公鑰 /etc/ssh/ssh_host_ed25519_key.pub  （★ 可公開，指紋從這裡算）
    客戶端記錄在 ~/.ssh/known_hosts
    ★★ 由 openssh-server 安裝時自動產生
    ★★★★ 重灌 / 重建 VM / 從範本 clone → 這組會換掉

  【使用者金鑰】        客戶端 → 證明給伺服器看「我是誰」
    私鑰 ~/.ssh/id_ed25519  →  伺服器記錄在 ~/.ssh/authorized_keys
    → 完整寫在 [[02-SSH-金鑰認證與ssh-agent]]，本篇不重複
```
...
| 檔案 | 演算法 | 現況 | 重要度 |
| --- | --- | --- | --- |
| `ssh_host_ed25519_key` | Ed25519 | ★★★★ **現代預設，客戶端優先選它**，指紋最該登記的就是這個 | ★★★★ |
| `ssh_host_rsa_key` | RSA 3072 | ★★★ 相容老客戶端（用 rsa-sha2-256/512 簽章） | ★★★ |
| `ssh_host_ecdsa_key` | ECDSA nistp256 | ★★ 相容用，有些資安基準要求停用 NIST 曲線 | ★★ |
| `ssh_host_dsa_key` | DSA | ★ 歷史遺留，OpenSSH 9.8 編譯預設關閉、10.0 完全移除 | ★ |
...
```text
-rw------- 1 root root 411 Aug 12 09:41 /etc/ssh/ssh_host_ed25519_key       # ★★★★★ 600 root
-rw-r--r-- 1 root root  96 Aug 12 09:41 /etc/ssh/ssh_host_ed25519_key.pub   # ★ 644 公鑰
```
...
>   十台共用同一把私鑰，攻破任何一台就能冒充其餘九台
> - **★★★★ 備份整個 `/etc` 到沒加密的網芳或 git repo**
> - **★★★★ 交機前用同一份磁碟映像檔大量佈署**
>
> clone 之後一定要重新產生：
> ```bash
...
> 但必須事先通知並記錄新指紋，否則就是自己製造狼來了。

### ★★★★ TOFU：SSH 信任模型的先天弱點

SSH 沒有 CA、沒有憑證鏈（除非另外建 SSH CA），它用的是
...
★★★★ TOFU 的邏輯與它的破口
  ...
  第一次連線
    ssh 問你：「這把 key 我沒看過，你要信嗎？」→ 你打了 yes → 寫進 known_hosts
              ▲ ★★★★★ 整個模型的安全性【全部押在這一刻】
                你如果隨手打 yes，等於沒有任何驗證

  第二次以後
    ssh 自己比對 known_hosts，一致就靜靜連進去；不一致 → 大聲警告並【拒絕連線】

  ★★★★★ 所以 SSH 能防的是「第一次之後的中間人」，防不了「第一次就被中間人接手」。
         第一次連線若在被劫持的網路上，你信的是攻擊者的 key，
         之後每次連線都不會有任何警告。
```
...
| 這一行 | 意思 | 重要度 |
| --- | --- | --- |
| `can't be established` | `known_hosts` 裡沒有這台 —— **不是錯誤，是 TOFU 的提問** | ★★★ |
| `ED25519 key fingerprint is` | 對方**選中**的 host key 型別 | ★★ |
| `SHA256:kV3m...` | ★★★★★ **這 43 個字元就是你要比對的東西** | ★★★★★ |
| `not known by any other names` | 這把 key 沒有以別的主機名／IP 出現在 known_hosts | ★★★ |
| `(yes/no/[fingerprint])` | ★★★★ 第三個選項是 OpenSSH 8.4 加的 —— **可直接貼指紋讓它自己比** | ★★★★ |

> [!danger] ★★★★★ 這一刻打 `yes` 而沒有比對，等於關掉 SSH 的全部主機驗證
> `yes` 的意思是「我已經用其他方式確認過這把 key 是對的」。
> 沒確認過就打 `yes`，只是把**當下線路上的任何一台機器**寫進了信任清單。
> 在機關內網通常沒事；**在跨機關專線、VPN 出口、廠商遠端維護、公用 Wi-Fi 上，這是真的風險**。
...
> SHA256 指紋是 43 個大小寫混雜的 base64 字元。人眼比對時**只會看頭尾幾個字**，
> 中間有一段不同根本看不出來 —— 這正是攻擊者會利用的。
> 讓 `ssh` 自己做字串比對，是**唯一不會偷懶的比對方式**。
> ★★ 注意 OpenSSH 8.4 起這個比對**區分大小寫**。
...
# Hash 過（★★★ Debian / Ubuntu 預設 HashKnownHosts yes）
|1|Xr8QpM3vK2sT9dW=|hN7bLyF4uJ6aE0oG5PnR= ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIB7q...KcE
```

| 欄位 | 說明 | 重要度 |
| --- | --- | --- |
| `\|1\|salt\|hash` | ★★★ 主機名／IP 的 HMAC-SHA1，**看不出是哪台** | ★★★ |
| `ssh-ed25519` + `AAAAC3...` | host key 型別與 base64 公鑰本體 | ★★ |
| 行首 `@revoked` | ★★★★ 標記撤銷，之後連到這把 key 會**直接拒絕** | ★★★★ |
| 行首 `@cert-authority` | ★★★ 這是 SSH CA 公鑰，可簽發主機憑證 | ★★★ |

> [!note] 為什麼 Debian 系要 hash？★★★
> 防止「攻破一台跳板機 → 讀 `known_hosts` → 得到整個機房的主機清單」，
> 這是真實的橫向移動手法。代價是不能再用 `grep` 找主機，必須改用 `ssh-keygen -F`。

```bash
ssh-keygen -lF 192.168.20.31        # 查詢並直接印出已登記的指紋
```

```text
# Host 192.168.20.31 found: line 14                # ★★★ 行號，處置時要用
192.168.20.31 SHA256:kV3mQ0Zc8pQ2rT1sXhN9dW7bLyF4uJ6aE0oG5PnRcIs   # ★★★★ 拿去跟交機表比
```

```bash
ssh-keygen -F 10.0.0.99; echo "exit=$?"    # 去掉 -l 只印原始行；查不到 exit=1
```

```text
exit=1        # ★★★ 沒有這台的記錄，腳本可以用這個判斷
```
...
Original contents retained as /home/sysadm/.ssh/known_hosts.old   # ★★★ 自動備份，刪錯救得回
```

★★★ 非 22 埠的記錄格式是 `[host]:port`，查詢與刪除都要寫成 `ssh-keygen -R '[192.168.20.31]:2222'`。
...
> SSH 演算法不是只看 `sshd_config`，還被全系統密碼原則覆蓋：
> ```bash
...
> DEFAULT
> ```
> 連老設備談不攏演算法時，RHEL 上有人會直接 `update-crypto-policies --set LEGACY`，
> ★★★★ 但那會把 HTTPS、資料庫連線的加密強度**一起降低**，
> **機關環境請優先用單一連線的 `-o` 旗標，不要動全系統策略**。
>
> **防火牆**是 firewalld：`sudo firewall-cmd --permanent --add-service=ssh && sudo firewall-cmd --reload`
>
> **SELinux ★★★★**：改了 SSH 埠一定要同時放行，否則 sshd 起不來：
> ```bash
> sudo semanage port -a -t ssh_port_t -p tcp 2222
> ```
> 細節見 [[07-SELinux與AppArmor]]。★★ RHEL 系 `HashKnownHosts` 預設 `no`，可直接 grep。
...
★★★★★ 看到 HOST KEY CHANGED，先停手，跑這棵決策樹
  ...
  ┌── 這台最近有沒有【變更紀錄】？（重灌／重建 VM／從快照還原／換硬體）
  │    ├─ 有 ─▶ 【成因①：主機重建】★★★
  │    │        到 IPMI/PVE console 跑 ssh-keygen -lf 取新指紋 → 與畫面上那串逐字比對
  │    │        → 相符才 ssh-keygen -R，然後重新登記
  │    └─ 沒有 ┐
  ├── 這個 IP 是不是【DHCP 或 NAT 後面】？
  │    ├─ 是 ─▶ 【成因②：IP 換人了】★★★
  │    │        同一個 IP 現在是另一台機器（租約重派／NAT 對應改了）
  │    │        用 ip neigh / arp -n 看 MAC 有沒有變；改用主機名別拿 IP 當識別
  │    └─ 否 ┐
  ├── 這個位址後面是不是【負載平衡／VIP／多台後端】？
  │    ├─ 是 ─▶ 【成因③：連到不同後端】★★★
  │    │        每台後端有各自的 host key，輪詢時就會跳警告（見下表）
  │    └─ 否 ┐
  └────────▶ 【成因④：可能是真的 MITM】★★★★★
             ✗ 不要重試、不要打密碼、不要 -R
             → 立刻改用【帶外管道】取指紋；不符 → 資安事件通報，
               保留 known_hosts 與 ssh -vvv 輸出，檢查同網段有無 ARP 欺騙
               （見 [[03-入侵偵測與防禦IDS-IPS]]）
```

**成因①（主機重建）的完整處置** —— 注意【1】【2】在前，【4】才動手：
...
```bash
# ★★★★ 【2】跟警告畫面上那串逐字比對，一致才往下做
ssh-keygen -lF 192.168.20.31    # ★★★ 【3】先看清楚舊的是哪一行，不要盲刪
ssh-keygen -R 192.168.20.31     # ★★★ 【4】刪除（會自動留 known_hosts.old）
ssh sysadm@192.168.20.31        # ★★★★ 【5】重新連線，用貼指紋的方式重新登記
```

**成因③（多台後端共用位址）的處置**，由好到差：

| 做法 | 說明 | 重要度 |
| --- | --- | --- |
| ★★★★ 所有後端共用同一組 host key | 把同一組 `/etc/ssh/ssh_host_*` 佈到每台 —— 前提是這組私鑰保護等級要提高 | ★★★★ |
| ★★★ 每台後端的 key 都登記 | 同一個主機名可以有多行，任一符合即通過 | ★★★ |
| ★★★ 用 SSH CA 簽發主機憑證 | 最正規，`@cert-authority` 一行搞定整個機房 | ★★★ |
...
debug1: Authentications that can continue: publickey,password    # 【關鍵字 4】伺服器接受什麼
```

| 關鍵字 | 沒看到代表 | 重要度 |
| --- | --- | --- |
| `Connecting to` | ★★★ **名稱解析失敗**（DNS／`/etc/hosts`），連 IP 都沒解出來 | ★★★ |
| `Connection established` | ★★★★ TCP 沒通 → 查防火牆、路由、sshd 是否在聽 | ★★★★ |
| `Server host key:` | ★★★★ 卡在演算法協商 → 階段 ③（`no matching ...`） | ★★★★ |
| `Offering public key:` | ★★★ **根本沒有可用的金鑰**，或金鑰權限錯被略過 | ★★★ |
| `Authentications that can continue:` | ★★★★ 沒到這行代表問題不在階段 ⑤，往前面找 | ★★★★ |

> [!tip] `Authentications that can continue:` 這行最有價值 ★★★★
> - 只列 `publickey` → 伺服器**已停用密碼登入**，沒把公鑰放上去就是進不去
> - 列了 `password` 但一直被拒 → 密碼真的錯，或帳號被鎖（看 `faillock`）
> - 完全沒出現 → 問題**不在階段 ⑤**
>
> ★★★ 排錯順序是 `-v` → `-vv`，不要一開始就 `-vvv`（訊息太多反而看不到重點）。

伺服器端對照看（需要另一條既有連線或 console）：

```bash
sudo journalctl -u ssh -u 'ssh@*' -f -n 50
```
...
所以兩個 unit 都要加。日誌的完整運用見 [[19-日誌系統]]。
...
SHA-1 已可做出選定前綴碰撞。★★★★ **注意停用的是「簽章演算法」不是「RSA 金鑰」** ——
同一把 RSA 金鑰改用 `rsa-sha2-256` / `rsa-sha2-512` 簽章完全沒問題（OpenSSH 7.2 起就支援），
...
# 更老的設備連 KEX 都談不攏時
ssh -o KexAlgorithms=+diffie-hellman-group14-sha1 -o HostKeyAlgorithms=+ssh-rsa admin@10.0.30.5
```

| 選項 | 管什麼 | 什麼時候需要 |
| --- | --- | --- |
| `HostKeyAlgorithms=+ssh-rsa` | ★★★ **對方的 host key**（階段 ③／④） | `no matching host key type` |
| `PubkeyAcceptedAlgorithms=+ssh-rsa` | ★★★ **你的使用者金鑰簽章**（階段 ⑤） | 金鑰確實在 `authorized_keys` 卻 `Permission denied (publickey)` |
| `KexAlgorithms=+...` | ★★★ 金鑰交換演算法（階段 ③） | `no matching key exchange method found` |

★★★★ `+` 是「加回清單」，`=`（不加號）是「整個換掉」。
**寫錯成 `=ssh-rsa` 會把其他演算法全砍掉**，變成連現代主機都連不上。

> [!danger] ★★★★ 這些旗標是「暫時的」，而且要有紀錄
> 加回 `ssh-rsa` / `dh-group1-sha1` 的實際後果：
> - **★★★★ 主機認證的簽章可被偽造** —— SHA-1 碰撞讓「證明我是這台機器」的保證變弱
> - **★★★★★ `diffie-hellman-group1-sha1` 是 1024-bit DH**，國家級對手已可破解（Logjam）
> - ★★★ 一旦寫進 `~/.ssh/config` 就會被忘記，三年後還在用
>
> 機關的正確處理：
> ① ★★★★ 把設定**限定到那一台**（`Host` 區塊，見 [[03-SSH-客戶端設定檔]]），不要寫在 `Host *`
> ② ★★★★ 開一張**設備汰換／韌體升級**追蹤單，寫上型號與到期日
> ③ ★★★ 這類老設備的管理介面**只允許從管理網段存取**
> ④ ★★ 設定檔加註解寫明「為什麼降級、誰核准、預計何時移除」，稽核時交代得出來
...
成因：客戶端透過 `SendEnv LANG LC_*`（Debian/Ubuntu 的 `/etc/ssh/ssh_config` 預設就有）
把自己的 locale 帶過去，但**伺服器沒裝那個 locale**。

```bash
locale -a | grep -i zh        # 在伺服器上跑；沒有輸出 = 沒有中文 locale
```

```bash
sudo locale-gen zh_TW.UTF-8 en_US.UTF-8 && sudo update-locale   # ★★★ 解法一（建議）
ssh -o SendEnv= sysadm@srv-web01                                # ★★ 解法二：這次不要帶過去
```

> [!warning] ★★★ 亂碼不一定跟 locale 有關
> `LC_*` 影響的是排序、日期、訊息語言。中文變成 `????` 或 `\x??` 通常是
> **終端機模擬器的編碼**設錯（PuTTY 的 Window→Translation 要選 UTF-8），跟伺服器 locale 無關。
> 先判斷是哪一邊的問題再動手。

**（b）★★ `TERM` 不對造成畫面錯亂**

老系統（RHEL 6、嵌入式設備）沒有 `xterm-256color` 的 terminfo，
症狀是 `vim`／`top` 畫面破碎、方向鍵變亂碼：

```bash
ssh sysadm@srv-web01 'echo $TERM'      # 看客戶端帶過去的是什麼
TERM=xterm ssh admin@10.0.30.5         # ★★ 臨時降級
```
...
| 客戶端 | known_hosts 位置 | 要注意 |
| --- | --- | --- |
| OpenSSH（Linux／macOS／Win10+） | `~/.ssh/known_hosts` | ★ 本篇所有指令的基準，指紋是 SHA256 base64 |
| PuTTY | ★★★ **Windows 登錄檔** `HKCU\Software\SimonTatham\PuTTY\SshHostKeys` | ★★★★ 指紋格式自成一套，比對要用 PuTTY 顯示的那串或改用 `plink` |
| MobaXterm | ★★ 自己的 session 資料夾 | ★★ 會自動開額外連線（sftp 分頁），連線數限制要留意 |
| WSL | ★★★ WSL 內的 `~/.ssh/`，**與 Windows 的是兩份** | ★★★ 同一個人有兩份 known_hosts，警告只跳其中一邊 |

> [!tip] ★★★ 機關統一用 OpenSSH 客戶端，維運成本會低很多
> Windows 10 1809 起內建 OpenSSH 客戶端。統一之後 `~/.ssh/config` 可以共用、
> `known_hosts` 格式一致、教學文件只要寫一份、指紋比對流程不用因人而異。
> PuTTY 留給「必須用序列埠 console」的場合就好。
...
> 批次維運腳本若不分這兩者，就會把「連不上的機器」誤判成「檢查沒通過的機器」，
> 對一台根本沒連上的機器發出錯誤告警或執行錯誤的補救動作。★★★★

**什麼時候需要 `-t`（強制配置 pty）**：

```bash
ssh sysadm@srv-web01 'sudo systemctl restart nginx'          # ★★★ 失敗
ssh -t sysadm@srv-web01 'sudo systemctl restart nginx'       # ★★★ 成功
```

```text
sudo: a terminal is required to read the password; either use the -S option ...
[sudo] password for sysadm:
Connection to srv-web01 closed.
```

| 情境 | 要不要 `-t` | 說明 |
| --- | --- | --- |
| `sudo` 需要輸入密碼 | ★★★★ 要 | 沒 tty 就問不了密碼 |
| 跑 `top` / `vim` / `less` | ★★★ 要 | 否則畫面全亂 |
| 要 `Ctrl-C` 能中斷遠端程式 | ★★★ 要 | 訊號要靠 tty 傳過去 |
| 輸出接管線 `\| grep` | ★★★★ **不要** | ★★★★ 會混入控制字元與 `Connection to ... closed.`，把解析弄壞 |
| 在 cron／CI 裡跑 | ★★★★ **不要** | 沒有 tty，會噴 `Pseudo-terminal will not be allocated`（不是錯誤，只是提醒） |

### ★★★★ 全機共用的 `/etc/ssh/ssh_known_hosts`

機關有二十台伺服器、十個維運人員時，讓每個人各自 TOFU 二十次是**流程上的漏洞** ——
只要有一個人隨手按 yes，那台的信任就破了。
正解是由管理者**集中維護一份**經過帶外驗證的清單，發到每台管理工作站：
...
> 它常跟 `-o UserKnownHostsFile=/dev/null` 一起出現在 CI/CD 腳本裡，
> 實際意義是：**永久關閉 SSH 的主機驗證，且不留任何記錄**。
> 對 CI 來說，任何能做 ARP 欺騙或 DNS 劫持的人都能攔下部署流程、拿到部署金鑰、
> 把惡意程式送進正式站。
...
★★★★ 動 SSH 設定之前的四道保險
  ...
  ① ★★★★★ 保留一條【既有的 SSH 連線不要關】
     → 重啟 sshd 不會斷掉已建立的連線，改壞了還能用它改回來
     → 關掉它 = 自斷退路

  ② ★★★★ 開【第二個終端機】測試新連線
     → 舊連線不動，用新視窗連連看；連不上就回舊視窗還原
  ...
  ④ ★★★★ 確認【帶外管道】真的能用（不是「應該能用」）
     → IPMI / iDRAC / iLO 的密碼【現在】就試登入一次
     → PVE / VMware console、雲端主機的 serial console 現在確認有啟用
     ★★★★★ 很多人是被鎖在外面之後，才發現 IPMI 密碼沒人知道
```
...
/etc/ssh/sshd_config: terminating, 1 bad configuration options   # ★★★★ 千萬別在這時重啟
```

保險起見的自動回滾（改設定前先掛一個定時還原，確認沒問題再取消）：
...
sudo atq                      # 確認新設定沒問題之後
sudo atrm <job號碼>            # ★★★ 把它刪掉，否則十分鐘後會被還原
```

★★★ `at` 與 systemd timer 的替代寫法見 [[18-排程工作]]。
...
```text
【步驟一】從 iDRAC console 取得三種 host key 指紋，寫進交機表（★★★★ 帶外）
...
| 項目 | 內容 |
| --- | --- |
| 資產編號 / 主機 | `MIS-SRV-2026-031`、`srv-web01` / `192.168.20.31` |
| OS / OpenSSH | Ubuntu 24.04.3 LTS / 9.6p1 |
...
| 取得方式 / 人員 | ★★★★ iDRAC 虛擬主控台（帶外）／王小明 2026-08-28 |
...
# 設計原則：★★★★★ 指紋不符立刻中止絕不寫入／★★★★ 寫入前先備份、失敗自動回滾／
...
usage() {
  printf '用法：%s <主機> <埠> <SHA256:預期指紋> [known_hosts路徑]\n%s\n' "$PROG" \
    "★★★★ 指紋必須從【帶外管道】取得（IPMI/iDRAC/PVE console、交機文件）：
       ssh-keygen -lf /etc/ssh/ssh_host_ed25519_key.pub" >&2
  exit 2
}
...
printf '%s' "$EXPECT" | grep -Eq '^SHA256:[A-Za-z0-9+/]{43}$' \
  || die "指紋格式不正確：$EXPECT（★★★ 不要包含結尾句點，也不要用 MD5 格式）"
...
if ssh-keygen -f "$KNOWN_HOSTS" -F "$HOSTSPEC" >"$WORKDIR/existing.txt" 2>/dev/null \
  ...
  die "known_hosts 已有 $HOSTSPEC，但指紋不符（已登記 $OLD_FP／預期 $EXPECT）
        ★★★★★ 這是 HOST KEY CHANGED 的情境，禁止本腳本自動覆蓋。
        先查變更單並用帶外管道重取指紋，確認後手動：ssh-keygen -R '$HOSTSPEC'"
fi
...
if [ -z "$MATCH_LINE" ]; then
  ...
  die "★★★★★ 沒有任何一把 host key 的指紋與預期相符！
        ...
        ★★★★★ 確認原因之前【不要】重試、【不要】輸入密碼。三個查法：
          【1】查變更單：這台最近有沒有重灌／重建／從快照還原
          【2】用帶外管道重新取一次指紋，跟交機表核對
          【3】確認 IP 沒被 DHCP 指派給別台：ip neigh show $HOST
        都排除仍不符 → 依資安事件流程通報，保留本檔案作為證據"
fi
...
ssh -p "$PORT" -o BatchMode=yes -o StrictHostKeyChecking=yes \
    ...
    -o PreferredAuthentications=publickey -- "$HOST" true 2>"$WORKDIR/verify.err"
set -e
...
cat <<EOF
★★★ 後續：把「已完成第一次連線驗證」與日期填回交機表；下一步做金鑰認證並停用密碼登入
    回滾：cp -p '$BACKUP' '$KNOWN_HOSTS'   或   ssh-keygen -f '$KNOWN_HOSTS' -R '$HOSTSPEC'
EOF
SCRIPT
```
...
★★★★ 此時 **`known_hosts` 完全沒有被動過** —— 沒有備份、沒有寫入、沒有殘留，
因為備份與寫入都在【5】，而【4】的 `die` 讓流程根本走不到那裡。
...
> 然後把密碼或金鑰的認證過程送給他。所有後續連線都不會再有警告。
>
...
>
> **③ ★★★★★ 從 VM 範本 clone 多台機器而不重新產生 host key**
> 後果：多台共用同一把主機私鑰，攻破一台就能**無警告地冒充其他所有台**。
...
> 後果：語法錯或規則寫錯 → sshd 起不來或把自己擋在外面 → 只能派人到機房。
> ★★★★ 動手前先確認 IPMI／console **現在**能用，不是「應該能用」。
>
> **⑥ ★★★★★ 用 `ssh-keyscan` 的結果當作「已驗證」寫進交機表**
> 後果：整套驗證流程變成形式主義。有中間人時兩邊拿到的都是中間人的 key，比對必然相符。

**機關情境的四個要點**：

| 項目 | 做法 | 重要度 |
| --- | --- | --- |
| ★★★★ 稽核軌跡 | 登入成功／失敗都要進集中日誌（`journalctl -u ssh` → syslog → SIEM），保存期限依機關規定。見 [[09-日誌集中與SIEM]] | ★★★★ |
| ★★★★ 最小權限 | 不用 `root` 直連（`PermitRootLogin no`），用個人帳號登入後 `sudo`，**才有「誰做了什麼」的軌跡** | ★★★★ |
| ★★★★ 個資風險 | `ssh -vvv` 輸出、`tcpdump` 的 pcap 含內部主機名與帳號，**送廠商前要遮蔽**，不要貼公開論壇 | ★★★★ |
| ★★★ TWGCB 對應 | 政府組態基準對 SSH 有明確要求（停用 root 登入、限制認證方式、閒置逾時），見 [[03-TWGCB-Linux項目分類詳解]] | ★★★ |

> [!warning] ★★★★ 不用 root 直連，不只是資安要求，也是排錯需求
> 三個人都用 `root` 登入，日誌上全是 `Accepted ... for root`，出事時**無法知道是誰改的**。
> 個人帳號 + `sudo` 才拼得出完整軌跡，這在機關的事故調查與稽核裡是硬需求。
...
| 階段 | 典型訊息 | 第一動作 | 重要度 |
| --- | --- | --- | --- |
| ① TCP | `Connection refused` / `timed out` | `ss -ltnp \| grep :22` ／ `nc -zv host 22` 與防火牆 | ★★★ |
...
| `ssh-keygen -F <host>` | 查詢原始行（hash 過也查得到），沒找到 exit=1 | ★★★★ |
...
| `ssh -o VisualHostKey=yes <host>` | 連線時顯示 randomart 圖形（★★ 只是輔助） | ★★ |
...
| `~/.ssh/known_hosts` | 個人已信任的主機 | `600`（目錄 `700`） | ★★★★ |
...
| `Permission denied (publickey)`（老 server） | `-o PubkeyAcceptedAlgorithms=+ssh-rsa` | ★★★ |
| `no matching key exchange method` | `-o KexAlgorithms=+diffie-hellman-group14-sha1`（★★★★★ 弱 DH） | ★★★★ |
| ★★★★ 通用注意 | `+` 是**加回**，`=` 是**整個換掉**，寫成 `=` 連現代主機都連不上 | ★★★★ |
...
| `systemctl status ssh` 是 inactive 但連得上 | ★★★ Ubuntu socket activation，正常 | ★★★ |
...
> 在測試機上刻意製造三種失敗，用 `ssh -v` 確認自己判斷的階段正確：
> ① 停掉 sshd（`sudo systemctl stop ssh.socket ssh.service`）從別台連；
> ② 恢復服務，用 `sudo ufw deny from <你的IP> to any port 22` 擋掉；
> ③ 恢復防火牆，指定不存在的演算法 `ssh -o KexAlgorithms=diffie-hellman-group1-sha1 host`。
>
> **參考解答**
> ① **`Connection refused`，階段 ①** —— ★★★ TCP RST 是核心直接回的（有機器、沒程序在聽）。
>    `ssh -v` 只停在 `debug1: Connecting to ...`，連 `Connection established.` 都不會出現。
> ② **`Connection timed out`（約兩分鐘），階段 ①** —— ★★★ ufw 預設是 DROP 不是 REJECT，
>    客戶端得不到任何回應只能等重傳耗盡。
>    ★★★★ 「refused 立刻回、timeout 要等很久」是現場最快的區分方式。
> ③ **`Unable to negotiate ... no matching key exchange method found.`，階段 ③** ——
>    ★★★ 會先看到 `Connection established.`，證明階段 ① 通 ——
>    這正是「錯誤訊息告訴你哪些階段已經成功」的實例。
>    收尾：`sudo ufw delete deny ...`、`sudo systemctl start ssh.socket`。

> [!question]- 練習 2：完整走一次帶外驗證流程
> 用一台 VM 模擬交機：① 從 **VM console**（不是 SSH）取得三種指紋抄成表；
> ② 從另一台用 `ssh-first-connect` 完成第一次連線；
> ③ 重新產生 host key（`sudo rm -f /etc/ssh/ssh_host_* && sudo ssh-keygen -A && sudo systemctl restart ssh`）；
> ④ 再連一次，用決策樹完成處置。
>
> **參考解答**
> 第 ② 步應看到 `[ MATCH]` 與 `exit=0`。★★★★
> 第 ④ 步會看到 `REMOTE HOST IDENTIFICATION HAS CHANGED!` 與 `Offending ED25519 key in ...:NN`。
> **正確處置順序**：這是**成因①（主機重建）**，因為你自己剛做的、有變更紀錄 →
> 從 console 取新指紋 → 跟警告畫面上 `The fingerprint for the ED25519 key sent by
> the remote host is` 那串逐字比對 → 相符才 `ssh-keygen -R`（★★★ 會留 `known_hosts.old`）
> → 用新指紋重跑 `ssh-first-connect`。
> ★★★★ 重點是**你在刪除之前做了確認**。順序反過來就是本篇最反對的做法。

> [!question]- 練習 3：驗證腳本真的會擋下錯誤的指紋
> 把交機表上的指紋改掉一個字元，重跑 `ssh-first-connect`，檢查三件事：
> ① 有沒有中止；② `known_hosts` 有沒有被動到；③ exit code 是多少。
>
> **參考解答**
> ① ★★★★★ 應在 `【4】比對指紋` 就中止，並把實收的 key 存到
>    `/tmp/ssh-first-connect-mismatch-<pid>.txt`。
> ② ★★★★ `known_hosts` **完全沒被動過** —— 備份與寫入都在【5】，
>    【4】的 `die` 讓流程走不到那裡。用 `md5sum ~/.ssh/known_hosts` 前後比對即可確認。
> ③ exit code 是 **1**。★★★
> **延伸思考**：若把邏輯改成「不符就警告後繼續」，這支腳本的安全價值就歸零 ——
> ★★★★★ **驗證失敗必須是硬中止，不能是可略過的警告**。
> 腳本在【1】處理「已有記錄但指紋不同」時同樣是硬中止，因為那正是 HOST KEY CHANGED 的情境。
