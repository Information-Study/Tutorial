...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[020-02-01-04-svc-sshd-伺服器端設定]]", "[[060-01-06-01-guide-傳輸-scp與sftp傳輸]]", "[[020-01-08-cmd-Linux-檔案權限與擁有者]]"]
updated: 2026-08-28
---
...
- [[020-02-01-04-svc-sshd-伺服器端設定]] —— `sshd_config` 的通用指令、drop-in 載入順序、`Match` 的語意
- [[060-01-06-01-guide-傳輸-scp與sftp傳輸]] —— **客戶端**的 `sftp` 操作、批次傳輸、基本的 chroot 權限規則
- [[020-01-08-cmd-Linux-檔案權限與擁有者]] —— rwx、setgid、POSIX ACL
- [[020-01-09-cmd-Linux-使用者與群組管理]] —— `useradd` / `usermod` / `chage` 的一般用法
- [[020-02-01-02-cmd-SSH-金鑰認證與ssh-agent]] —— 公鑰認證與 `authorized_keys` 選項語法

> [!note] 這篇跟 [[060-01-06-01-guide-傳輸-scp與sftp傳輸]] 的分工 ★★★
> [[060-01-06-01-guide-傳輸-scp與sftp傳輸]] 站在**客戶端**：怎麼傳、怎麼保留權限、跳板機怎麼過，
...
細節見 [[060-01-06-01-guide-傳輸-scp與sftp傳輸]] 與 [[020-01-08-cmd-Linux-檔案權限與擁有者]]，這裡只留驗證方式：
...
| **★★★★ `AllowTcpForwarding no` + `AllowStreamLocalForwarding no` + `PermitTunnel no`** | 禁止 `-L`／`-R`／UNIX socket／tun 轉發 | **受限帳號變成內網跳板**（見 [[020-02-01-05-cmd-SSH-隧道與埠轉發]]），或轉發到 Docker socket＝root |
...
> 忘了建這個空檔就完全沒有操作紀錄 —— 稽核會問。同步策略見 [[060-01-06-02-guide-rsync-同步與備份]]。
...
> （見 [[060-01-06-03-guide-傳輸-備份策略與還原演練]]、[[090-07-07-guide-資安實踐-台灣資安法規與個資法]]）。
...
集中與長期保存見 [[100-01-02-guide-日誌-日誌集中與輪替]]，稽核軌跡的要求見 [[090-07-09-guide-資安實踐-資安稽核與符合性檢核]]。
...
> sudo repquota -s /srv
> ```
> ★★★ **配額是整個檔案系統的**，所以 `/srv` 最好是獨立分割區（見 [[020-01-04-cmd-Linux-檔案系統與目錄結構]]）；
...
> Nginx 加固細節見 [[060-02-02-09-guide-Nginx-安全設定]]。
...
```ini
# /etc/systemd/system/sftp-intake.service   （unit 寫法見 [[020-02-02-01-svc-systemd-unit撰寫實戰]]）
[Unit]
...
# /etc/systemd/system/sftp-intake.timer     （選型見 [[020-02-02-02-cmd-systemd-timer與cron選型]]）
[Unit]
...
| **★★★★★ 上傳的 `.php` 被瀏覽器執行了** | 上傳目錄落在 web root 且被 `location ~ \.php$` 比對到 | 目錄移出 web root；`location ^~` + `return 403`；**當資安事件處理**（[[060-02-02-09-guide-Nginx-安全設定]]） |
...
| 傳輸加密 | SFTP 走 SSH 已加密；演算法加固見 [[020-02-01-07-svc-SSH-安全強化]] | ★★★ |

密碼與帳號政策通則見 [[090-07-02-guide-資安實踐-密碼與帳號管理實務]]，主機層加固見 [[090-02-08-guide-防護-系統強化與稽核]]。
...
> 驗收：上傳 `t.php` 後 `curl` 必須拿到 **403/404，絕不能是 200**（見 [[060-02-02-09-guide-Nginx-安全設定]]）。

## 延伸閱讀

- [[020-02-01-04-svc-sshd-伺服器端設定]] —— `Match`、drop-in 載入順序與 `sshd -T` 的完整說明，本篇的地基
- [[060-01-06-01-guide-傳輸-scp與sftp傳輸]] —— 客戶端怎麼傳、`sftp` 批次模式、chroot 權限規則的完整版
- [[020-02-01-07-svc-SSH-安全強化]] —— 演算法加固、2FA、Fail2ban，受限帳號之外的整體防線
- [[020-02-01-05-cmd-SSH-隧道與埠轉發]] —— 先懂隧道能做什麼，才知道 `AllowTcpForwarding no` 擋掉了什麼
- [[020-01-08-cmd-Linux-檔案權限與擁有者]] —— setgid、POSIX ACL、`getfacl` 的細節
- [[060-02-02-09-guide-Nginx-安全設定]] —— 上傳目錄與 Web 共存的紅線、`location ^~` 與關閉執行權
- [[060-01-06-02-guide-rsync-同步與備份]] —— rrsync 帳號背後的同步策略與輪替設計
- [[100-01-02-guide-日誌-日誌集中與輪替]] —— 稽核日誌的長期保存、集中與完整性保護
- [[090-07-09-guide-資安實踐-資安稽核與符合性檢核]] —— 稽核會問什麼、要交出哪些證據
