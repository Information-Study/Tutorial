...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[020-02-01-01-cmd-SSH-原理與第一次連線]]", "[[020-01-08-cmd-Linux-檔案權限與擁有者]]"]
updated: 2026-08-28
---
...
> ```
>
> 金鑰部署完成、**在終端 B 實測登入成功之前**，不可以去 [[020-02-01-04-svc-sshd-伺服器端設定]]
> 或 [[020-02-01-07-svc-SSH-安全強化]] 關閉密碼登入。順序錯了，就得靠 IPMI／iDRAC／主控台或跑機房才救得回來。

## 前置知識

- [[020-02-01-01-cmd-SSH-原理與第一次連線]] — 主機金鑰、known_hosts 與第一次連線的信任建立
- [[020-01-08-cmd-Linux-檔案權限與擁有者]] — rwx 模型（本篇只講「SSH 要求哪些權限」）
- [[020-01-09-cmd-Linux-使用者與群組管理]] — 建立維運帳號與服務帳號
- [[020-01-17-cmd-Linux-systemd服務管理]] — 後面用 systemd user unit 常駐 ssh-agent
...
| SSH 憑證 | 只有 CA 公鑰 | ★ 同上，且憑證有效期短 | 佳，見 [[020-02-01-07-svc-SSH-安全強化]] |
...
這樣「刪掉某人的金鑰」才會真的生效在每一台上；離職清除只要改一個檔案再推一次。做法見 [[020-02-03-05-svc-標準化-自動化佈建入門]]。
...
| `cert-authority` / `principals="…"` | 把該行當成 CA 公鑰使用，見 [[020-02-01-07-svc-SSH-安全強化]]；`environment=` 需開 `PermitUserEnvironment`，不建議 | ★★★ |
...
整個作廢 —— 對方送 `deploy v1; rm -rf /` 就照跑。rsync 場景直接用 `rsync` 套件附的 `rrsync`（`rrsync -ro /srv/data`），不要自己寫，用法見 [[060-01-06-02-guide-rsync-同步與備份]]。
...
> 正解是**專屬的、無 passphrase 但有 `restrict,from=,command=` 限制的機器金鑰**，私鑰檔 `600` 且擁有者是那個服務帳號。排程設定見 [[020-01-18-guide-Linux-排程工作]]。

★★★ `~/.ssh/config` 寫 `AddKeysToAgent yes` 之後，第一次用到某把金鑰時輸入 passphrase 就會
自動加進 agent，不用先手動 `ssh-add`（可寫 `yes`／`ask`／`confirm`／時限如 `1h`）。完整語法見 [[020-02-01-03-svc-SSH-客戶端設定檔]]。
...
> - 伺服器端：`sshd_config` 設 `AllowAgentForwarding no`（見 [[020-02-01-04-svc-sshd-伺服器端設定]]）
...
★★★ `-J` 需要 OpenSSH 7.3 以上（2016 年），現役系統都有；寫進 `~/.ssh/config` 的`ProxyJump` 更方便，語法見 [[020-02-01-03-svc-SSH-客戶端設定檔]]。
...
長久解是在 `~/.ssh/config` 逐站寫 `IdentityFile` + `IdentitiesOnly yes`（見 [[020-02-01-03-svc-SSH-客戶端設定檔]]），伺服器端的 `MaxAuthTries` 調整見 [[020-02-01-04-svc-sshd-伺服器端設定]]。
★★★ 順帶一提：這種試誤會累積伺服器端失敗紀錄，**可能觸發 fail2ban 把你自己的 IP 鎖掉**（見 [[020-02-01-07-svc-SSH-安全強化]]）。
...
逐步操作與備援金鑰的準備見 [[020-02-01-07-svc-SSH-安全強化]]、[[090-05-07-guide-資安設備-身分存取管理IAM與MFA]]。★★★ 該不該升級到憑證，判斷準則不是伺服器數量，是這個問題：
...
排程每週執行（systemd timer 語法見 [[020-01-18-guide-Linux-排程工作]]）：
...
★★★ 腳本用 exit code 反映結果，systemd 會把 exit 2 記成 `failed`，可直接被監控系統抓到（見 [[070-06-07-guide-測試-自動化健康檢查實戰]]）。
...
| `User ops not allowed because account is locked` | ★★★ 帳號被鎖 | `sudo passwd -S ops`，見 [[090-07-02-guide-資安實踐-密碼與帳號管理實務]] |
| **完全沒有任何訊息** | 連線根本沒到伺服器 | 網路／防火牆，見 [[090-02-02-guide-防火牆-ufw基礎與實務]] |

RHEL 系用 `journalctl -u sshd`。看不到細節時把 `LogLevel` 調成 `VERBOSE`（會記錄每次認證嘗試的
公鑰指紋，★★★★ 對稽核也很有用），見 [[020-02-01-04-svc-sshd-伺服器端設定]] 與 [[020-01-19-guide-Linux-日誌系統]]。
...
>    → `.gitignore` 加 `id_*`、`*.pem`、`*.key`；伺服器端用 `pre-receive` 攔截，見 [[060-01-01-08-guide-Git-伺服器端與自動部署]]。
...
> | 要求 | 做法 | 對應 |
> | --- | --- | --- |
> | **可歸責性** | 一人一把、註解含姓名、`LogLevel VERBOSE` 記錄每次登入的指紋 | [[020-02-01-04-svc-sshd-伺服器端設定]] |
> | **最小權限** | 機器帳號一律 `restrict` + `command=`；人的帳號用 `from=` 限縮來源 | 本篇〈進階應用〉 |
> | **定期盤點** | 每週自動跑 `ssh-key-audit`，報表保留至少一年 | 本篇〈完整實戰範例〉 |
> | **異動管控** | 新增／註銷要有申請單與核准紀錄，名冊與實機定期對帳 | [[090-07-09-guide-資安實踐-資安稽核與符合性檢核]] |
> | **設定基準** | SSH 組態依 TWGCB Linux 基準檢核（`PermitRootLogin`、`PasswordAuthentication`、`MaxAuthTries` 等） | [[090-06-03-guide-TWGCB-Linux項目分類詳解]] |
> | **完整性監控** | 把 `authorized_keys` 納入 FIM，有人偷加金鑰要能立即告警 | [[090-08-04-guide-Wazuh-FIM檔案完整性監控]] |
>
...
| `~/.ssh/config` | `600`，見 [[020-02-01-03-svc-SSH-客戶端設定檔]] | ★★★ |
...
> 長久解是在 `~/.ssh/config` 逐站寫 `IdentityFile` + `IdentitiesOnly yes`（見 [[020-02-01-03-svc-SSH-客戶端設定檔]]）。★★★ 另外這種試誤會累積伺服器端失敗紀錄，可能觸發 fail2ban 把自己鎖掉。
> 參見〈進階應用 → 四、多把金鑰的試誤問題〉。
>
> **Q8.** 正確順序是**先加新 → 驗證 → 再退舊**：
...
- [[020-02-01-01-cmd-SSH-原理與第一次連線]] — 主機金鑰、`known_hosts` 與中間人攻擊；本篇 `Server accepts key` 之前發生的事都在那篇
- [[020-02-01-03-svc-SSH-客戶端設定檔]] — `~/.ssh/config` 的 `Host`／`Match` 完整語法，以及 `IdentitiesOnly`／`ProxyJump`／`AddKeysToAgent` 的正式寫法
- [[020-02-01-04-svc-sshd-伺服器端設定]] — `PubkeyAuthentication`、`AuthorizedKeysFile`、`StrictModes`、`MaxAuthTries`、`AllowAgentForwarding` 的完整解讀，以及**安全關閉密碼登入**的順序
- [[020-02-01-07-svc-SSH-安全強化]] — FIDO2 硬體金鑰逐步操作、SSH CA 短效憑證、fail2ban 與存取控制
- [[020-02-01-05-cmd-SSH-隧道與埠轉發]] — `permitopen=`／`permitlisten=` 要限制的那些功能實際能做什麼
- [[020-02-01-06-svc-SFTP-與受限使用者]] — 只給檔案傳輸不給 shell 的另一種做法（`ForceCommand internal-sftp` + `ChrootDirectory`）
- [[060-01-01-08-guide-Git-伺服器端與自動部署]] — Deploy Key 的實務，以及用 `pre-receive` 攔截誤推的私鑰
- [[090-05-07-guide-資安設備-身分存取管理IAM與MFA]] — 把 SSH 金鑰納入整體身分治理，MFA 與短效憑證的整合
