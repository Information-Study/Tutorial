...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[020-02-01-04-svc-sshd-伺服器端設定]]", "[[020-02-01-02-cmd-SSH-金鑰認證與ssh-agent]]", "[[090-02-02-guide-防火牆-ufw基礎與實務]]"]
updated: 2026-08-28
---
...
- [[020-02-01-04-svc-sshd-伺服器端設定]] —— `sshd_config` 每個指令的意義、`Include` 順序、`ssh.socket` 陷阱
- [[020-02-01-02-cmd-SSH-金鑰認證與ssh-agent]] —— 金鑰產生、`authorized_keys` 選項
- [[090-02-02-guide-防火牆-ufw基礎與實務]] —— 本篇的「第一層來源限制」靠它
- [[090-02-05-guide-防護-Fail2ban入侵防護]] —— 本篇只給最小 jail，調校與誤封解除看那篇
- [[020-01-17-cmd-Linux-systemd服務管理]] —— 自動回滾 timer 會用到 `systemd-run`
...
> [!note] 本篇與其他篇的分工
> - fail2ban 的 `jail.local`、filter 撰寫、誤封解除 → [[090-02-05-guide-防護-Fail2ban入侵防護]]
> - 防火牆規則語法 → [[090-02-02-guide-防火牆-ufw基礎與實務]]、[[090-02-03-guide-防火牆-nftables與iptables]]
> - TOTP／FIDO2 的逐步操作 → [[090-05-07-guide-資安設備-身分存取管理IAM與MFA]]
> - VPN／跳板機架構選型 → [[090-02-06-guide-防護-遠端存取安全]]
> - `sshd_config` 每個指令的基礎意義、`Include` 與 `ssh.socket` → [[020-02-01-04-svc-sshd-伺服器端設定]]
> - TWGCB 項目逐條解讀 → [[090-06-02-guide-TWGCB-Linux基準文件解讀]]
>
...
>
> 順序細節見 [[020-02-01-04-svc-sshd-伺服器端設定]]。
...
> 見〈2FA 的取捨〉與 [[090-05-07-guide-資安設備-身分存取管理IAM與MFA]]。
...
>     IdentitiesOnly yes      ← ★★★★ 只送指定的那一把（見 [[020-02-01-03-svc-SSH-客戶端設定檔]]）
...
| **SFTP 專用帳號** ★★★★ | `Match Group sftp-only` 內 `DisableForwarding yes` → [[020-02-01-06-svc-SFTP-與受限使用者]] |
...
> ★★★ 代價：日誌量約為 INFO 的 2～3 倍 → 配合 [[100-01-02-guide-日誌-日誌集中與輪替]]、[[090-05-09-guide-資安設備-日誌集中與SIEM]]
> ```
...
規則語法與 nftables 寫法見 [[090-02-02-guide-防火牆-ufw基礎與實務]]、[[090-02-03-guide-防火牆-nftables與iptables]]。
...
**jail 語法、filter 撰寫、誤封解除、遞增封鎖 → [[090-02-05-guide-防護-Fail2ban入侵防護]]，本篇不重複。**
...
架構選型（VPN / 跳板機 / 零信任代理）→ [[090-02-06-guide-防護-遠端存取安全]]
```
...
★★★★ 全程要有「CA 掛掉怎麼辦」的答案：
     ...
     → 必須有離線備援金鑰 + console 存取 + CA 私鑰離線備份（見 [[060-01-06-03-guide-傳輸-備份策略與還原演練]]）
```
...
操作細節（`pam_google_authenticator` 安裝、FIDO2 註冊流程）在 [[090-05-07-guide-資安設備-身分存取管理IAM與MFA]]，
**本節只寫決策與 sshd 怎麼串**。
...
操作細節見 [[090-05-07-guide-資安設備-身分存取管理IAM與MFA]]。
...
>   → 你的所有稽核軌跡【三秒內消失】
> ★★★★★ 必須即時外送到攻擊者拿不到的地方：
>   Wazuh agent → [[090-08-01-svc-Wazuh-Wazuh架構與安裝]]、[[090-08-05-guide-Wazuh-日誌蒐集與解析]]
>   集中日誌／SIEM → [[090-05-09-guide-資安設備-日誌集中與SIEM]]；保存期限（機關常見 6 個月）→ [[100-01-02-guide-日誌-日誌集中與輪替]]
> ```
...
> **項目編號不通用**。逐條解讀見 [[090-06-02-guide-TWGCB-Linux基準文件解讀]]，本機導入見
> [[090-06-04-guide-TWGCB-Linux本機導入]]，自動化檢測見 [[090-06-07-guide-TWGCB-Linux檢測與符合性報告]] 與
...
> ★★★★★ 更好：改用 SSH CA 簽 1 小時短效憑證，CI 每次 build 現簽現用 → [[060-01-01-08-guide-Git-伺服器端與自動部署]]。
...
| **Ubuntu socket 啟動** | ✗ 22.10+ 用 `ssh.socket`，只改 `sshd_config` 可能無效 → [[020-02-01-04-svc-sshd-伺服器端設定]] | ★★★★ |
...
排程每日檢查（見 [[020-02-02-02-cmd-systemd-timer與cron選型]]）：
...
Ubuntu 22.10+ 若看到 `ssh.socket` 在聽而 `ssh.service` inactive，那是 socket 啟動 → [[020-02-01-04-svc-sshd-伺服器端設定]]。
...
- [[020-02-01-04-svc-sshd-伺服器端設定]] —— 本篇每個設定項的基礎意義、`Include` 順序與 `ssh.socket` 陷阱
- [[020-02-01-02-cmd-SSH-金鑰認證與ssh-agent]] —— `authorized_keys` 選項與金鑰產生，SSH CA 的前一站
- [[090-02-05-guide-防護-Fail2ban入侵防護]] —— 本篇只給最小 jail，遞增封鎖、filter 與誤封解除看這篇
- [[090-02-06-guide-防護-遠端存取安全]] —— 「把 SSH 從公網下架」的架構選型（VPN／跳板機／零信任）
- [[090-05-07-guide-資安設備-身分存取管理IAM與MFA]] —— TOTP 與 FIDO2 的逐步操作，本篇只寫決策
- [[090-02-08-guide-防護-系統強化與稽核]] —— SSH 之外的系統層加固與稽核工具
- [[090-06-04-guide-TWGCB-Linux本機導入]]、[[090-06-07-guide-TWGCB-Linux檢測與符合性報告]] —— 把本篇成果對應到政府組態基準
- [[090-05-09-guide-資安設備-日誌集中與SIEM]]、[[090-08-05-guide-Wazuh-日誌蒐集與解析]] —— 讓 SSH 日誌在被清掉之前就送出去
