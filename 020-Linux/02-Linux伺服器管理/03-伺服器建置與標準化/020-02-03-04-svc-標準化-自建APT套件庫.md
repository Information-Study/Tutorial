...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[020-02-03-03-cmd-標準化-第三方APT套件庫實務]]", "[[020-01-14-guide-Linux-套件管理]]", "[[020-02-03-02-ref-標準化-基準設定與範本化]]"]
updated: 2026-08-28
---
...
- [[020-02-03-03-cmd-標準化-第三方APT套件庫實務]] — 本篇是它的反面：那篇教你怎麼**安全地消費**別人的庫，這篇教你怎麼**當供應方**。`signed-by`、pinning 的基本語法與第三方庫的風險評估都在那篇，本篇不重複
- [[020-01-14-guide-Linux-套件管理]] — `apt update` / `apt-cache policy` / dpkg 狀態機的日常操作
- [[020-02-03-02-ref-標準化-基準設定與範本化]] — 本篇要打包的 `org-baseline-config` 就是那篇產出的基準設定
- [[090-01-06-guide-PKI-自建根CA]] 與 [[090-01-09-guide-PKI-根憑證派送與信任]] — 本篇只負責**把已經簽好的根憑證裝進 deb**，CA 的建立與簽發不在這裡
- [[020-01-08-cmd-Linux-檔案權限與擁有者]] — deb 內的檔案權限就是安裝後的權限，打包時錯了裝上去也錯
...
| ★ 只是要讓大家裝某個開源工具的新版 | **不值得** | 上游多半已有官方 APT 庫，走 [[020-02-03-03-cmd-標準化-第三方APT套件庫實務]] 的流程評估後直接用 |
...
> [!tip] 判斷準則：一句話
> **「這東西以後會不會需要『升級』與『移除』？」**
> 會 → 打包成 deb。只是丟一次就不管了 → 用 [[020-02-03-05-svc-標準化-自動化佈建入門]] 的 Ansible 派送就好。
...
# 放入根憑證（來自 [[090-01-06-guide-PKI-自建根CA]] 的產出）
...
完整的 Nginx 設定語法見 [[060-02-02-02-guide-Nginx-設定語法與虛擬主機]] 與 [[060-02-02-06-guide-Nginx-HTTPS與Certbot]]，
這裡只給套件庫**特有**的四個決定：
...
★★★ 這一步**應該寫進 [[020-02-03-01-svc-標準化-新機建置標準流程]] 的基準設定**，不要靠人工加。
`signed-by` 與 pinning 的完整語法與風險說明在 [[020-02-03-03-cmd-標準化-第三方APT套件庫實務]]，這裡只講內部庫的差異。
...
#   （用 [[100-01-03-guide-日誌-系統監控與告警]] 的資產清單比對，或直接掃）
...
> 6. 事後檢討並記入資安事件簿，銜接 [[090-07-03-guide-資安實踐-弱點與修補管理流程]]
...
| **與修補流程銜接** | 上游釋出安全更新 → aptly `mirror update` + `snapshot` + `diff` 附進變更單 → 測試環境 → `publish switch` | ★★★★ 見 [[090-07-03-guide-資安實踐-弱點與修補管理流程]] |

**Release 過期監控腳本**（放進 [[020-01-18-guide-Linux-排程工作]] 的每日排程）：
...
| `org-baseline-config` | [[020-02-03-02-ref-標準化-基準設定與範本化]] 的基準設定（sysctl、journald、SSH 提示） | ★★★ 有 conffiles，使用者可微調且升級時保留 |
...
```bash
# ── 在受管機器上（實際應由 [[020-02-03-02-ref-標準化-基準設定與範本化]] 的範本派送）──
...
> 機密走 [[020-02-03-05-svc-標準化-自動化佈建入門]] 的 Ansible Vault 或專門的秘密管理工具。
...
>   （見 [[090-02-08-guide-防護-系統強化與稽核]]）。
> - 對外那邊：這也正是 [[020-02-03-03-cmd-標準化-第三方APT套件庫實務]] 強調第三方庫風險評估的原因。

> [!warning] ★★★ 機關情境的三個額外要求
> - **稽核軌跡**：`repo-publish` 的日誌、Nginx `access_log`、以及「誰批准發這個版本」的變更單，
>   三者要能對得起來。日誌保留期依機關規定（多數要求 6 個月以上），做法見 [[100-01-02-guide-日誌-日誌集中與輪替]]
> - **最小權限**：`includeb` 的權限與「能登入建置機」的權限要分開；
>   發布是核可後的動作，不是任何一個維運人員隨時可做的事
> - **TWGCB 符合性**：內部庫派送的 `org-baseline-config` 就是基準組態的載體，
>   內容變更等同於組態變更，要走同一套核可與檢測流程（見 [[090-06-04-guide-TWGCB-Linux本機導入]]）

> [!warning] ★★★ 庫本身也要納入備份與弱點管理
> - 備份三件事：`pool/`（deb 檔）、**簽章私鑰**、`conf/`。`db/` 可重建，不用備。做法見 [[060-01-06-03-guide-傳輸-備份策略與還原演練]]
> - 發布機是一台對外服務主機，**它自己也要打修補**。別讓「派送修補的機器」變成沒修補的那台
> - 內部庫延後了上游安全更新的到達時間（因為要先鏡像、測試、切換）——
>   ★★★★ 這個延遲要納入 [[090-07-03-guide-資安實踐-弱點與修補管理流程]] 的 SLA，緊急修補要有繞過流程
...
> 補救只能靠備份還原（見 [[060-01-06-03-guide-傳輸-備份策略與還原演練]]），或事先公告這一次升級會重設設定檔。
...
- [[020-02-03-03-cmd-標準化-第三方APT套件庫實務]] — 本篇的反面。`signed-by`、pinning 的完整語法、第三方庫的風險評估與撤除流程都在那裡，接第三方庫前先讀
- [[020-01-14-guide-Linux-套件管理]] — `apt` / `dpkg` 的日常操作與狀態機；本篇的排錯大量用到 `apt-cache policy` 與 `dpkg.log`
- [[020-02-03-02-ref-標準化-基準設定與範本化]] — 本篇打包的 `org-baseline-config` 就是那篇的產出，兩篇要一起看才完整
- [[020-02-03-01-svc-標準化-新機建置標準流程]] — 內部庫的接入設定應該寫進新機 SOP，不是靠人工事後補
- [[020-02-03-05-svc-標準化-自動化佈建入門]] — 判斷「這件事該打包成 deb 還是用 Ansible 派送」，以及帶外派送金鑰的做法
- [[090-01-09-guide-PKI-根憑證派送與信任]] — 本篇只負責把根憑證裝進 deb；憑證怎麼被各種應用（Java、Node.js、瀏覽器）信任在那篇
- [[090-01-12-guide-PKI-憑證生命週期管理]] — GPG 簽章金鑰的輪替節奏與 TLS 憑證的生命週期管理是同一套思維
- [[090-07-03-guide-資安實踐-弱點與修補管理流程]] — 內部庫延後了安全更新的到達時間，這個延遲要納入修補 SLA 並設計緊急繞過流程
- [[060-02-05-01-guide-MyGuard-MyGuard套件庫介紹]] — 一個真實第三方庫的樣貌，可以對照本篇看「別人是怎麼做的」
- [[060-01-06-03-guide-傳輸-備份策略與還原演練]] — 庫的備份三件事（pool、私鑰、conf）要納入既有備份制度
