...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[18-排程工作]]", "[[22-Shell腳本進階]]"]
updated: 2026-08-28
---
...
| 你應該已經會 | 在哪一篇 |
| --- | --- |
| cron 五欄語法、`*/15`、`crontab -e/-l` 基本操作 | [[18-排程工作]] |
| 四大失敗原因的初階說明（PATH、登入環境、工作目錄、`%`） | [[18-排程工作]] |
| `flock -n` 基本用法、`MAILTO`、`CRON_TZ`、anacron 與 `cron.daily` 概念 | [[18-排程工作]] |
| `set -euo pipefail`、`trap`、退出碼、`$?` 與 `PIPESTATUS` | [[22-Shell腳本進階]] |
| 基本的 shell 變數、重導向、管線 | [[21-Shell腳本入門]] |
| `journalctl -u`、日誌位置與 facility 觀念 | [[19-日誌系統]] |
| systemd timer 的能力比較與選型決策 | [[02-systemd-timer與cron選型]] |

**這篇的定位是「事故現場」。** [[18-排程工作]] 教你怎麼把排程寫對；這篇教你**排程寫錯了要怎麼發現**，
...
> 而且開了 `-L 15` 之後 cron 的日誌量會明顯變大，記得配合 [[02-日誌集中與輪替]] 調整輪替。
...
用 ② 的話，logrotate 是**必要的**，不是可選的（見 [[02-日誌集中與輪替]]）：
...
[[18-排程工作]] 已經教過 `flock -n`。這裡談的是**取捨**與**踩得到的坑**。
...
> （用 [[01-systemd-unit撰寫實戰]] 的 unit 部署到單一台，或用資料庫層的 advisory lock）。
...
>   `KillMode=control-group` 會把整棵程序樹一起清掉，這是 cron 做不到的。見 [[02-systemd-timer與cron選型]]。
...
> ```
> ★★★ **不要用 `setenforce 0` 當解法**，那是把整台機器的防護關掉。詳見 [[07-SELinux與AppArmor]]。
...
System clock synchronized: yes
              NTP service: active                        # ★★★ 見 [[28-時間同步NTP與chrony]]
```
...
而且 cron 不會補。要能補跑就得改用 systemd timer 的 `Persistent=true`（見 [[02-systemd-timer與cron選型]]）
或 anacron。
...
完整的 unit 撰寫方式（包含 `Requires` 與 `After` 的差別、`Type=` 的選擇）見 [[01-systemd-unit撰寫實戰]]；
「這種情況該不該整支改用 timer」的決策表見 [[02-systemd-timer與cron選型]]。
...
- 看到 `System clock was stepped by 7412.339 seconds`（> 10800 秒）→ ★★★★ 那天的排程沒補跑，
  這是「只有某一天沒跑」的典型解釋，見 [[28-時間同步NTP與chrony]]。
...
- 有 `denied` 且 `scontext` 含 `system_cronjob_t` → ★★★★ 確診是 SELinux，用 `audit2why` 看原因，
  用 `semanage fcontext` + `restorecon` 修標籤，**不要 `setenforce 0`**。見 [[07-SELinux與AppArmor]]。
...
> 這同時也是 TWGCB 與 CIS 的檢查項目，見 [[09-使用者與群組管理]]。
...
> 見 [[02-systemd-timer與cron選型]]），或 anacron（見 [[18-排程工作]]）。
...
- [[18-排程工作]] — cron 五欄語法、`*/15` 寫法、四大失敗原因、anacron 的基礎，本篇所有陷阱的前提
- [[02-systemd-timer與cron選型]] — 什麼時候該放棄 cron 改用 timer：`Persistent=`、`RandomizedDelaySec=`、cgroup 逾時的完整比較
- [[01-systemd-unit撰寫實戰]] — `@reboot` 的正確替代方案：用 `Requires=`／`After=` 宣告掛載與資料庫相依
- [[04-服務自動復原與看門狗]] — 排程失敗之後的告警與自動處置（`OnFailure=`、`Restart=`）
- [[22-Shell腳本進階]] — `set -euo pipefail`、`trap`、`PIPESTATUS` 與退出碼設計，本篇的 wrapper 全部建立在這些之上
- [[09-使用者與群組管理]] — 服務帳號建立、`chage` 到期政策、`nologin` 與最小權限
- [[28-時間同步NTP與chrony]] — 時間跳躍的來源與如何避免大幅 step
- [[02-日誌集中與輪替]] — 排程日誌的 logrotate 與集中蒐集，稽核軌跡保存
- [[03-備份策略與還原演練]] — 「備份排程有跑」與「備份真的能還原」是兩件事
