...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[01-systemd-unit撰寫實戰]]", "[[17-systemd服務管理]]", "[[10-程序管理與訊號]]"]
updated: 2026-08-28
---
...
| 篇章 | 你需要從那篇帶過來的東西 |
| --- | --- |
| [[01-systemd-unit撰寫實戰]] | unit 三區段、drop-in 與 template unit 的**寫法**、`Type=notify` 的 **READY=1 就緒通知**、`TimeoutStopSec=` / `KillMode=` 停機語意、`systemd-analyze verify` |
| [[17-systemd服務管理]] | `systemctl` 基本操作、`status` 判讀、`enable` 與 `start` 的差別 |
| [[10-程序管理與訊號]] | SIGTERM／SIGKILL／SIGABRT／SIGSTOP 的差別、core dump、cgroup |
| [[02-systemd-timer與cron選型]] | timer unit 的 `OnCalendar=` / `OnUnitActiveSec=` 語法、`systemctl list-timers` 判讀 |
| [[03-cron排程實務與陷阱]] | `flock` 互斥與 `timeout` 保護的完整用法（本篇的告警腳本會直接用） |
| [[19-日誌系統]] | `journalctl -u` 過濾、journal 磁碟上限與輪替 |
...
> | 主題 | 在哪一篇 | 本篇的分工 |
> | --- | --- | --- |
> | unit 三區段、相依、drop-in／template 的**寫法** | [[01-systemd-unit撰寫實戰]] | 本篇只寫「怎麼用」，不重講機制 |
> | `Type=notify` 的 **`READY=1`（就緒）** | [[01-systemd-unit撰寫實戰]] | 本篇負責**同一條 socket 的另一半：`WATCHDOG=1`（心跳）** |
> | timer 語法與 `list-timers` 判讀 | [[02-systemd-timer與cron選型]] | 本篇寫兩支 timer，但只寫**設計理由與檔案內容** |
> | `flock` 去重、`timeout` 保護 | [[03-cron排程實務與陷阱]] | 本篇**只用不教** |
> | PM2 與 systemd 的雙層退避怎麼對齊 | [[05-PM2與systemd整合]] | 本篇講通則，Node/PM2 情境請看那篇 |
> | 外部監控平台、健康檢查端點設計 | [[03-系統監控與告警]]、[[04-健康檢查與可用性監控]] | 本篇只做**主機本地、用來驅動重啟**的那一層 |
>
...
drop-in 寫法（drop-in 的機制與檔案位置見 [[01-systemd-unit撰寫實戰]]，這裡只寫內容）：
...
> 3. 只把**已經確認「重啟無效」的**那幾個（通常是 78、1 之外的特定值）加進去，一次加一個，並記入變更紀錄（見 [[08-變更管理流程]]）。
>
...
timer 的語法與 `list-timers` 判讀見 [[02-systemd-timer與cron選型]]，這裡只給檔案與設計理由：
...
>
> 端點本身該回什麼、外部平台怎麼探測，屬於 [[04-健康檢查與可用性監控]]；
> 容器情境的等價設計見 [[04-Compose-網路與健康檢查]]。
...
# ④★★★★ 管道一：logger → rsyslog → SIEM（最穩、有稽核軌跡，見 [[09-日誌集中與SIEM]]）
...
> 一定要有從機器外面看的探測（見 [[03-系統監控與告警]] 與 [[04-健康檢查與可用性監控]]）。
...
> 1. 在變更紀錄裡寫明「此為暫時性緩解措施」與追蹤到期日（見 [[08-變更管理流程]]）。
> 2. 保留一次 OOM 當下的 heap dump／`systemd-cgtop` 觀測資料給廠商。
> 3. 排程重啟排在離峰、並先摘出負載平衡（做法見 [[06-部署自動化]]）。
>
...
> 開機流程層級的救援手段見 [[25-開機流程與GRUB救援]]，核心參數見 [[26-核心模組與sysctl調校]]。
...
| ★★★★★ 設了 `StartLimitAction=reboot-force`，一支 worker 掛掉害整台機器重開，且進入重開機迴圈 | 服務層失敗被升級成整機動作，而根因（設定檔錯）重開機修不好 | 立即改回 `StartLimitAction=none`（若已進迴圈，需從 GRUB 進 rescue 或帶外管理，見 [[25-開機流程與GRUB救援]]） |
...
| ★★★★ `/var` 被寫滿，`df` 顯示 journal 佔 12 GB | 重啟迴圈把每次啟動失敗都寫進 journal | 止血：`sudo journalctl --vacuum-size=500M`；根因：先收斂重啟策略；輪替設定見 [[19-日誌系統]] |
...
| ★★★ 資料庫掛掉的當下，五分鐘內收到三十封告警信 | 沒有去重限流，三十個相依服務同時 failed | 告警腳本加時間戳去重（本篇 `svc-alert.sh` 的 `DEDUP_SEC`）與 `flock`（用法見 [[03-cron排程實務與陷阱]]） |
...
`degraded` → 至少還有一個 unit 在 failed，逐一回到【1】處理。事故的分級與通報見 [[09-事件處理與升級流程]]，
排查方法論見 [[10-故障排除方法論]]。
...
>   自動復原參數是稽核時會被問「誰改的、為什麼」的項目（見 [[08-變更管理流程]]）。
...
> - 重啟事件必須**留下可稽核的紀錄**（`logger` 打進 syslog 並集中到 SIEM，見 [[09-日誌集中與SIEM]]）。
...
>   pool 層級的調校見 [[02-PHP-FPM設定與Pool調校]]。
...
> 讓 SIEM 規則好寫（見 [[09-日誌集中與SIEM]]）。完整腳本見本篇〈主動巡檢〉。
...
- [[01-systemd-unit撰寫實戰]] —— unit 三區段、drop-in 與 template 的寫法、`Type=notify` 的 `READY=1` 就緒通知；本篇的心跳（`WATCHDOG=1`）是同一條 socket 的另一半
- [[02-systemd-timer與cron選型]] —— 本篇兩支 timer 用到的 `OnCalendar=` / `OnUnitActiveSec=` 語法與 `systemctl list-timers` 判讀
- [[03-cron排程實務與陷阱]] —— 告警腳本用到的 `flock` 去重與 `timeout` 保護的完整用法
- [[05-PM2與systemd整合]] —— Node/PM2 情境下，`Restart=` 與 PM2 的 `min_uptime` 該怎麼分層對齊，避免兩層退避互相打架
- [[17-systemd服務管理]] —— `systemctl` 基本操作與 `status` 判讀；本篇假設你已經會這些
- [[19-日誌系統]] —— 重啟迴圈灌爆 journal 之後的輪替與磁碟上限設定
- [[03-系統監控與告警]] / [[04-健康檢查與可用性監控]] —— 本篇是主機自帶的最後一道防線，機器整台當掉時只能靠外部監控
- [[09-事件處理與升級流程]] —— 告警送出去之後的分級、通報鏈與事後檢討
