...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[060-03-02-03-guide-PM2-程序管理入門]]", "[[020-02-02-01-svc-systemd-unit撰寫實戰]]"]
updated: 2026-08-28
---
...
- [[060-03-02-03-guide-PM2-程序管理入門]] — PM2 的安裝、基本指令、`pm2 startup` / `pm2 save`、日誌位置、五個坑。
  **本篇不重講這些**，只引用它的結論。
- [[060-03-02-04-guide-PM2-進階設定與部署]] — `ecosystem.config.cjs` 完整寫法、cluster 原理、零停機 reload 的
  graceful shutdown 程式碼。本篇不重貼設定檔全文。
- [[020-02-02-01-svc-systemd-unit撰寫實戰]] — unit 的相依、`Exec*`、沙箱指令、template unit。
- [[020-02-02-04-svc-systemd-服務自動復原與看門狗]] — `Restart=` 策略、健康檢查 timer、`OnFailure=` 告警單元的完整實作。
  本篇只談「PM2 情境下這些機制該掛在哪一層」。
- [[020-01-17-cmd-Linux-systemd服務管理]] — `systemctl` 基本操作、`start` 與 `enable` 的差別。
- [[060-03-02-01-guide-Node-安裝與版本管理]] — 為什麼正式機**不要用 nvm**（本篇的 PATH 問題直接源自這個結論）。
...
[[060-03-02-03-guide-PM2-程序管理入門]] 是從 **PM2 這一側**寫的：怎麼裝、怎麼用、怎麼設開機自啟。
...
【架構 C】完全不用 PM2
  ...
  systemd (PID 1)
    ...
    └─ nuxt-app@2.service ─ node .output/server/index.mjs  PORT=3002
         stdout ──▶ journald
  → 多核心靠 Nginx upstream 分流（見 [[130-01-03-03-guide-Nuxt-Nginx反向代理與快取]]）
```
...
> 修法有兩層：**根治**是把 node 換成 `/usr/bin/node`（見 [[060-03-02-01-guide-Node-安裝與版本管理]] 的「不要用 nvm」結論）；
...
> ```
> 詳見 [[090-02-07-guide-防護-SELinux與AppArmor]]。Ubuntu 的 AppArmor 預設不管自訂 unit，通常不會擋。
>
...
```javascript
// ecosystem.config.cjs（節錄，完整寫法見 [[060-03-02-04-guide-PM2-進階設定與部署]]）
autorestart: true,
min_uptime: '10s',
...
> 日誌輪替的細節屬於 [[060-03-02-03-guide-PM2-程序管理入門]] 與 [[100-01-02-guide-日誌-日誌集中與輪替]]，本篇只講架構決策。
...
> [!note] 架構 C 的多實例長什麼樣
> 用 template unit（完整寫法見 [[020-02-02-01-svc-systemd-unit撰寫實戰]]）：
> ```bash
...
>      Active: active (running) since Fri 2026-08-28 09:31:02 CST; 5min ago
> ```
> Nginx 那一側用 `upstream` 分流，設定見 [[060-02-02-04-guide-Nginx-反向代理與負載平衡]]。
...
[[020-02-02-04-svc-systemd-服務自動復原與看門狗]] 教的健康檢查 timer 與 `OnFailure=` 告警，
在 PM2 情境下**掛錯層就完全沒效果**：
...
> timer 的完整寫法見 [[020-02-02-04-svc-systemd-服務自動復原與看門狗]]，本篇只點名「打哪裡」。
...
> 依 [[100-02-08-guide-維運-變更管理流程]] 的節奏辦：
> ```
> 【事前】
...
> `systemctl reload nginx` → 停舊的 3000。詳見 [[130-01-06-guide-部署-部署自動化]]。
...
> 並在 service 上掛 `OnFailure=alert@%n.service`，寫法見 [[020-02-02-04-svc-systemd-服務自動復原與看門狗]]。
...
> sudo -u nodeapp env -i PATH=/usr/bin:/bin /usr/bin/pm2-runtime --version
> ```
> 根治方式是照 [[060-03-02-01-guide-Node-安裝與版本管理]] 的結論，正式機用系統套件的 `/usr/bin/node`。
...
- [[060-03-02-03-guide-PM2-程序管理入門]] — PM2 那一側的完整說明：安裝、`pm2 monit`／`pm2 describe` 診斷、
  日誌輪替、五個坑。本篇的所有 PM2 指令細節都以那篇為準。
- [[060-03-02-04-guide-PM2-進階設定與部署]] — `ecosystem.config.cjs` 全文、cluster 原理與限制、
  零停機 reload 的 graceful shutdown 程式碼。本篇「三個逾時對齊」裡的 `T_app` 就是那篇的 handler。
- [[020-02-02-01-svc-systemd-unit撰寫實戰]] — unit 的相依宣告、`Exec*` 家族、沙箱指令、template unit。
  本篇實戰範例的 unit 用到的每一個指令，那篇都有完整說明。
- [[020-02-02-04-svc-systemd-服務自動復原與看門狗]] — `Restart=` 策略、健康檢查 timer、`OnFailure=` 告警單元的實作。
  搭配本篇的「這些機制該掛在哪一層」一起看。
- [[060-03-02-01-guide-Node-安裝與版本管理]] — 「正式機不要用 nvm」的完整論證。本篇一半的 PATH 問題源自違反這條。
- [[130-01-03-02-guide-Nuxt-SSR與PM2部署]] — Nuxt 的建置產物、`.output/server/index.mjs` 與 Nginx 反向代理設定。
- [[130-01-06-guide-部署-部署自動化]] — 換埠切換、符號連結切版、部署腳本要把 `systemctl reload` 放在哪。
- [[100-01-02-guide-日誌-日誌集中與輪替]] — journald 的保留策略、轉送到集中蒐集平台的做法。
