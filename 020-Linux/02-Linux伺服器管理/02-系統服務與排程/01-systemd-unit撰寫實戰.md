...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[17-systemd服務管理]]", "[[10-程序管理與訊號]]"]
updated: 2026-08-28
---
...
| 篇章 | 你需要從那篇帶過來的東西 |
| --- | --- |
| [[17-systemd服務管理]] | `systemctl` 基本操作、`status` 判讀、unit 三個位置的優先序、`Type=` 對照表、`[Install]` 與 `systemctl edit` 入門、資源限制（`MemoryMax=`）|
| [[10-程序管理與訊號]] | SIGTERM／SIGKILL 的差別、程序群組與 cgroup、殭屍與孤兒程序 |
| [[08-檔案權限與擁有者]] | 640／750 的意義、`chown` 與服務帳號 |
| [[19-日誌系統]] | `journalctl -xeu <unit>` 追服務日誌 |
...
> | 主題 | 在哪一篇 |
> | --- | --- |
> | `Restart=` 策略、重啟風暴、`WatchdogSec=`、`OnFailure=` 告警 | [[04-服務自動復原與看門狗]] |
> | timer unit、`OnCalendar=`、oneshot 服務怎麼被排程觸發 | [[02-systemd-timer與cron選型]] |
> | PM2 與 systemd 的分工與衝突 | [[05-PM2與systemd整合]] |
> | Supervisor 本身的設定寫法 | [[03-Laravel-佇列排程與Supervisor]] |
...
| 版本控管 | 沒有 | ★★★ unit 檔納入 git（[[00-伺服器建置與標準化-索引]]）|
...
> 磁碟與掛載本身見 [[15-磁碟分割與掛載]]、[[29-網路儲存與軟體RAID]]。
...
給第 02 篇的 timer 服務用的形狀（timer 寫法見 [[02-systemd-timer與cron選型]]）：
...
> 更嚴謹的做法見 [[03-機密管理與金鑰保護]]。

unit 檔的版本控管（配合 [[00-伺服器建置與標準化-索引]] 與 [[01-新機建置標準流程]]）：
...
（Supervisor 本身的設定寫法見 [[03-Laravel-佇列排程與Supervisor]]，
Laravel 的佇列設計見 [[06-Laravel-佇列排程與事件]]。）
...
`SyslogIdentifier=` 設對的話，`-t` 過濾比 `-u` 更乾淨（日誌設計見 [[19-日誌系統]]）。
...
> 再進一步用 systemd credentials 或外部秘密管理（[[03-機密管理與金鑰保護]]）。
...
| `time-sync.target` | 時間同步完成（見 [[28-時間同步NTP與chrony]]） | ★★ |
...
> `RemainAfterExit=no` 是給 timer 用的形狀（[[02-systemd-timer與cron選型]]）。
...
> timer 的寫法見 [[02-systemd-timer與cron選型]]，本題見〈oneshot + RemainAfterExit〉。

---

## 延伸閱讀

- [[17-systemd服務管理]] —— 本篇的前置：`systemctl` 操作、`status` 判讀、`Type=` 對照表、資源限制
- [[04-服務自動復原與看門狗]] —— `Restart=` 策略、重啟風暴的抑制、`WatchdogSec=` 與 `OnFailure=` 告警，本篇刻意留給它
- [[02-systemd-timer與cron選型]] —— 本篇寫好的 oneshot unit 要怎麼被排程觸發、`OnCalendar=` 語法、失敗如何被發現
- [[05-PM2與systemd整合]] —— Node 服務改用 systemd 時，PM2 自己的 `pm2 startup` 會與你的 unit 打架，這篇講分工與收斂
- [[03-Laravel-佇列排程與Supervisor]] —— Supervisor 的設定寫法與它和 systemd 的取捨（本篇的實戰範例是從它遷過來的）
- [[10-程序管理與訊號]] —— SIGTERM／SIGKILL、程序群組與 cgroup，看懂 `KillMode=` 的前提
- [[07-SELinux與AppArmor]] —— RHEL 系上 unit 沙箱之外的第二層阻擋，`Permission denied` 查不出原因時看這裡
- [[15-磁碟分割與掛載]] —— `RequiresMountsFor=` 背後的 fstab 與 mount unit
