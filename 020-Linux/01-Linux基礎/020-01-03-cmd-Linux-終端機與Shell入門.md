...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[020-01-02-guide-Linux-實驗環境準備與初次登入]]"]
updated: 2026-08-29
---
...
```bash
echo $SHELL      # ★★ 你的預設登入 shell（從 /etc/passwd 讀，未必是現在跑的）
...
> 這也是為什麼建議日常用一般使用者、需要時才 `sudo`（見 [[020-01-09-cmd-Linux-使用者與群組管理]]）。

提示字元由環境變數 `PS1` 決定，可以自訂（見 [[060-01-05-03-guide-終端機-Bash與Zsh效率設定]]）。
...
> ```
> ★★★★★ 這就是為什麼腳本要寫 `set -u`（未定義變數就報錯），見 [[020-01-22-guide-Linux-Shell腳本進階]]。
...
!!                   # ★★ 重複上一個指令
!$                   # ★★ 上一個指令的最後一個參數
!nginx               # ★★★ 執行最近一個以 nginx 開頭的指令（直接跑，不會先給你看）
```
...
> ★★★★ 但 `ps` 的問題還在。詳見 [[090-03-03-guide-應用安全-機密管理與金鑰保護]]。
...
| ★★★ `Ctrl+Z` | 把目前程式丟到背景暫停（見 [[020-01-10-cmd-Linux-程序管理與訊號]]） |
...
```bash
sudo unminimize              # ★★★ Ubuntu：還原被移除的文件與套件（會裝回不少套件，正式機先評估）
...
HISTTIMEFORMAT='%F %T '       # ★★★★ 沒有這行，事後查不出「什麼時候改的」
...
| ★★★ `sudo` 之後找不到指令 | `sudo` 會重設 `PATH`（`secure_path`） | 用完整路徑，或 `sudo env "PATH=$PATH" cmd`，見 [[020-01-20-guide-Linux-環境變數與設定檔]] |
| ★★★★ 檔名含空白，指令只吃到一半 | 沒加引號 | `"$file"` 一律加雙引號 |
| ★★★★ `rm *.txt` 說找不到檔案 | 沒有比對到任何檔案，`*.txt` 原樣傳下去 | 先用 `echo *.txt` 確認展開結果 |
| ★★ 明明改了 `.bashrc` 卻沒生效 | 沒重新載入，或改錯檔案（`.bash_profile` vs `.bashrc`） | `source ~/.bashrc`；載入順序見 [[020-01-20-guide-Linux-環境變數與設定檔]] |
...
| ★★★ `Ctrl+C` 停不下來 | 程式忽略 SIGINT | `Ctrl+Z` 丟背景後 `kill %1`，見 [[020-01-10-cmd-Linux-程序管理與訊號]] |
...
- [[020-01-04-cmd-Linux-檔案系統與目錄結構]] — 知道怎麼打指令了，接著要知道東西放在哪
- [[020-01-11-cmd-Linux-輸入輸出重導向與管線]] — 把指令串起來的關鍵
- [[020-01-20-guide-Linux-環境變數與設定檔]] — `PATH`、`.bashrc` 與 `.profile` 的載入順序
- [[060-01-05-03-guide-終端機-Bash與Zsh效率設定]] — 別名、提示字元與 dotfiles 的完整設定
- [[060-01-05-04-guide-終端機-現代CLI工具集]] — `fzf`、`ripgrep` 等能再提升一個檔次的工具
