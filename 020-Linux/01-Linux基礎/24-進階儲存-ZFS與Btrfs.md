...
> 結果就是 OOM Killer 砍掉你的資料庫（見 [[10-程序管理與訊號]]）。
>
...
> - **不確定、記憶體有限、只想要穩** → **ext4 或 xfs + LVM**（見 [[15-磁碟分割與掛載]]）
>
> 這兩個都是「功能換複雜度」的選擇。
...
> | `xattr=sa` + `acltype=posixacl` | 保留來源的 ACL（見 [[08-檔案權限與擁有者]]） |
...
- [[10-程序管理與訊號]] — OOM Killer 與 ARC 的記憶體競爭
- [[18-排程工作]] — 用 systemd timer 排定 scrub 與備份
- [[03-機密管理與金鑰保護]] — ZFS 原生加密
- [[03-PVE-儲存設定]] — Proxmox VE 上的 ZFS 實務
- [[04-備份災難復原與入侵應變]] — 防止備份被勒索軟體刪除
