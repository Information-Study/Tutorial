...
```
    PID COMMAND         COMMAND
      1 systemd         /sbin/init          # ★★ PID 1 就是 systemd
```
...
| ★★ 排程 | `cron` | **timer unit**（見 [[020-01-18-guide-Linux-排程工作]]） |
| 日誌 | `syslog` | **journald**（見 [[020-01-19-guide-Linux-日誌系統]]） |
...
```
/etc/systemd/system/          ← 你自己寫的、優先度最高        ★★ 你的修改都寫這裡
/run/systemd/system/          ← 執行期產生的                  ★ 重開機就沒了
/usr/lib/systemd/system/      ← 套件安裝的（不要手動改！）    ★★★★ 改了會被升級蓋掉
```
...
● nginx.service - A high performance web server and a reverse proxy server
     Loaded: loaded (/usr/lib/systemd/system/nginx.service; enabled; preset: enabled)   # ★★★ enabled 才會開機自啟
     Active: active (running) since Wed 2026-08-27 09:12:03 CST; 8h ago   # ★★★★ 先看這一行
       Docs: man:nginx(8)
    Process: 880 ExecStartPre=/usr/sbin/nginx -t -q -g daemon on; (code=exited, status=0/SUCCESS)
   Main PID: 891 (nginx)
      Tasks: 3 (limit: 2273)
     Memory: 12.4M (peak: 18.2M)   # ★★ peak 是設 MemoryMax= 的依據
        CPU: 2.104s
...
sudo chmod 640 /etc/myapp/env      # ★★★★ 讓服務帳號讀得到（裡面有 DB 密碼，絕不能 644）
```
...
```bash
sudo useradd --system --no-create-home --shell /usr/sbin/nologin myapp   # ★★★ 系統帳號、不可登入
...
sudo chmod 750 /opt/myapp /var/lib/myapp   # ★★ 其他使用者讀不到
```
...
LOG_LEVEL=info
ENVFILE
sudo chown root:myapp /etc/myapp/env    # ★★★ 擁有者 root、群組才是服務帳號
sudo chmod 640 /etc/myapp/env           # ★★★★ 裡面有 DB 密碼，其他人不可讀
```
...
```bash
sudo systemctl daemon-reload          # ★★★★ 新增 unit 檔後必做
sudo systemctl enable --now myapp     # ★★★ 一次搞定「現在起」與「開機起」

# ★★ 驗證
systemctl status myapp
sudo journalctl -u myapp -n 30 --no-pager
systemctl show myapp -p MemoryMax,Restart,User,NoNewPrivileges   # ★★★ 確認限制真的生效
systemd-analyze security myapp.service
...
> （見 [[060-03-02-03-guide-PM2-程序管理入門]]），兩者的取捨在
> [[060-03-02-04-guide-PM2-進階設定與部署]] 有完整比較。簡單說：
...
> ```
> ★★★★ 每個不必要的服務都是一個攻擊面。
> 這是 TWGCB 與 CIS 的必檢項，見 [[090-06-03-guide-TWGCB-Linux項目分類詳解]]。
...
- [[020-01-18-guide-Linux-排程工作]] — systemd timer 與 cron 的選型
- [[020-01-19-guide-Linux-日誌系統]] — `journalctl` 的完整用法
- [[020-01-10-cmd-Linux-程序管理與訊號]] — 訊號、cgroup 與 OOM Killer
- [[020-01-09-cmd-Linux-使用者與群組管理]] — 服務專屬帳號的建立
- [[060-03-02-03-guide-PM2-程序管理入門]] — Node 應用的另一種管理方式
- [[090-06-03-guide-TWGCB-Linux項目分類詳解]] — 停用非必要服務的合規要求
