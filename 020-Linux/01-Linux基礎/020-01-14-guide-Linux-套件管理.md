...
```bash
sudo rm /etc/apt/sources.list.d/example.sources
...
sudo apt install --allow-downgrades nginx=<官方版本號>
```
...
sudo rpm --import https://rpm.example.com/RPM-GPG-KEY-example
...
[ "$ans" = "yes" ] || { echo "已中止"; sudo rm -f "$KEYRING"; exit 1; }
