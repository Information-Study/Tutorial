...
> 日常排查就完全不需要 root。見 [[020-01-09-cmd-Linux-使用者與群組管理]]。
...
# ── 時間 ──
sudo journalctl --since "2026-08-27 09:00" --until "2026-08-27 09:30"   # ★★★ 鎖定事故時間窗
...
>
> ★★★ **這是每台新機都該做的設定**，列入 [[090-02-01-guide-防護-伺服器初始安全設定]] 清單。
...
>
> （這和 [[020-01-06-cmd-Linux-檢視檔案內容]] 的 `tail -f` 輪替問題是同一個原理。）
>
> 兩種解法：
>
...
> ★★★★ 而是**有沒有不該成功的成功**。發現異常見 [[090-03-04-guide-應用安全-備份災難復原與入侵應變]]，
> predisposition 防護見 [[090-02-05-guide-防護-Fail2ban入侵防護]]。
...
> 死因通常就浮出來了。這個流程值得寫進 [[100-02-10-guide-維運-故障排除方法論]]。
...
| ★★★★ `rm` 大日誌後空間沒釋放 | 服務開著檔案 | `truncate -s 0`；見 [[020-01-15-cmd-Linux-磁碟分割與掛載]] |
...
>   `systemd-journal-upload`）——本機被清了遠端還有，見 [[100-01-02-guide-日誌-日誌集中與輪替]]
...
> ```
> 這三題就是 [[100-02-02-guide-維運-每日維護作業]] 的核心內容——
> 把它們寫成腳本，每天自動產出。
...
- [[020-01-06-cmd-Linux-檢視檔案內容]] — `tail -F`、`zgrep` 讀輪替後的日誌
- [[020-01-17-cmd-Linux-systemd服務管理]] — 服務日誌與 `journalctl -xeu`
- [[100-01-02-guide-日誌-日誌集中與輪替]] — 多機日誌集中與遠端備份
- [[060-02-02-07-guide-Nginx-日誌與除錯]] — Nginx 日誌格式自訂
- [[100-01-01-01-svc-GoAccess-安裝與基本使用]] — 日誌視覺化
- [[090-02-05-guide-防護-Fail2ban入侵防護]] — 讓日誌自動觸發封鎖
