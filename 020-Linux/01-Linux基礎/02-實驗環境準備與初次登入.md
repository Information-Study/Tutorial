...
>   `systemctl` 會直接報錯，第 [[17-systemd服務管理]] 篇會受影響
...
> 更好的做法是把主機名稱放進提示字元，見 [[03-Bash與Zsh效率設定]]。
...
> 底層的 `useradd` 兩系都有。詳見 [[09-使用者與群組管理]]。
...
> 很容易被 OOM Killer 砍掉服務。小機器建議加 1～2GB swap，見 [[15-磁碟分割與掛載]]。
...
| `df` 顯示磁碟滿但找不到大檔 | 檔案被刪但程序還開著，或 inode 用光 | `lsof \| grep deleted`、`df -i`，見 [[15-磁碟分割與掛載]] |
...
>
> 1. 建立一般使用者並設強密碼
> 2. 部署 SSH 金鑰、**停用密碼登入**（見 [[02-SSH-金鑰認證與ssh-agent]]）
> 3. 停用 root 直接登入
> 4. 開防火牆，只放行必要的埠（見 [[02-防火牆-ufw基礎與實務]]）
> 5. 裝上 Fail2ban（見 [[05-Fail2ban入侵防護]]）
>
> 完整清單見 [[01-伺服器初始安全設定]]。
...
>
> 這個習慣之後會延伸到 [[03-備份策略與還原演練]] —— 備份的價值不在備份本身，
> 而在於還原成功。
...
- [[03-終端機與Shell入門]] — 環境建好了，開始學怎麼打指令
- [[03-實驗環境搭建]] — 四種練習環境的完整建置細節
- [[01-伺服器初始安全設定]] — 新機上線的完整安全檢查清單
- [[01-SSH-原理與第一次連線]] — SSH 指紋驗證與 known_hosts
- [[09-使用者與群組管理]] — 使用者、群組與 sudo 的完整說明
