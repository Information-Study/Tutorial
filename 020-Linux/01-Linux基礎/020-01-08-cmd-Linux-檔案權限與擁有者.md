...
>
> ★★★ 這偶爾被用來「刻意讓擁有者唯讀」，但更常見的是**設定錯誤**——看到擁有者權限比群組窄，先當成打錯了。
...
```bash
chmod 644 file.txt                 # ★★ 一般檔案
chmod 755 script.sh                # ★★ 要能被執行的腳本
chmod 600 ~/.ssh/id_ed25519        # ★★★★ 私鑰多開一個位元，OpenSSH 就拒絕使用
```
...
```bash
find /var/www/example.com -type d -exec chmod 750 {} +     # ★★★ 目錄要有 x 才進得去
find /var/www/example.com -type f -exec chmod 640 {} +     # ★★★ 檔案一律不給 x
```
...
sudo chown -R mike:www-data /var/www/example.com   # ★★★★ 路徑打錯（多一個空白、少一層）會改掉整棵樹，很難還原
...
> ```bash
> sudo find / -xdev -type f -perm -4000 -exec ls -l {} + 2>/dev/null   # ★★★★ 排進月巡檢
> ```
> 正常系統上只該有 `sudo`、`su`、`passwd`、`mount`、`umount`、
> `ping`、`chsh`、`newgrp` 等十幾個。★★★★ 多出來的要調查——這是入侵者留後門最愛的地方。
>
...
```bash
sudo chmod +t /shared/dropbox        # ★★★ 只加 sticky，其他位元不動
sudo chmod 1777 /shared/dropbox      # ★★★ 數字寫法，開頭的 1 就是 sticky
```
...
sudo setfacl -b /var/www/app                # ★★★★ 清除所有 ACL：整份拿掉，別跟只刪一條的 -x 搞混
```
...
sudo -u www-data test -w "$APP/public" && echo "⚠ public 不該可寫！"   # ★★★★★ 有輸出＝可被寫入 webshell
...
> 稽核現有的 777：
> ```bash
> sudo find /var/www /srv /opt -perm -o+w -exec ls -ld {} + 2>/dev/null   # ★★★★ 有輸出就是待辦事項
> ```
...
> ```bash
> chmod 700 ~/.ssh
> chmod 600 ~/.ssh/id_ed25519          # ★★★★ 私鑰
> chmod 644 ~/.ssh/id_ed25519.pub
> chmod 600 ~/.ssh/authorized_keys     # ★★★★ 太寬 sshd 會靜默忽略
