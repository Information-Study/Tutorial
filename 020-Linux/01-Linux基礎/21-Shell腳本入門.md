...
見 [[20-環境變數與設定檔]]——只有 `source` 能把變數帶回目前 shell。
...
詳細的陷阱與進階用法（`trap`、`-e` 在條件式中的例外）見 [[22-Shell腳本進階]]。
...
ALERT_CMD="${ALERT_CMD:-logger -t healthcheck}"
...
# ── 2. 磁碟 ──────────────────────────────────────
while IFS= read -r line; do
    usage_pct=$(awk '{print $5}' <<< "$line" | tr -d '%')
...
done < <(df -P -x tmpfs -x devtmpfs -x squashfs | tail -n +2)
...
shellcheck healthcheck.sh && echo "shellcheck 通過"
./healthcheck.sh
...
> | 退出碼 0/1 | 讓 systemd 的 `OnFailure=` 能觸發（見 [[18-排程工作]]） |
...
| `bad interpreter: No such file or directory` | **CRLF 換行**（Windows 編輯過） | `dos2unix`；見 [[06-檢視檔案內容]] |
...
| 腳本在 cron 失敗 | 環境不同 | 見 [[18-排程工作]]、[[20-環境變數與設定檔]] |
...
> 見 [[04-檔案系統與目錄結構]]。`trap` 的用法在 [[22-Shell腳本進階]]。

> [!warning] 腳本的權限與擁有者
> 以 root 執行的腳本若可被其他人寫入，等於把 root 送出去。
> ```bash
> sudo install -m 755 -o root -g root script.sh /usr/local/bin/script
> ```
> 見 [[18-排程工作]] 的排程腳本稽核。
...
- [[11-輸入輸出重導向與管線]] — 重導向與 `pipefail`
- [[12-文字處理三劍客]] — 在腳本中用 `awk`/`sed` 處理資料
- [[18-排程工作]] — 讓腳本自動執行
- [[20-環境變數與設定檔]] — 變數作用域與環境
