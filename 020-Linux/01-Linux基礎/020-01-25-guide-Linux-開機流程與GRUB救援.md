...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[020-01-17-cmd-Linux-systemd服務管理]]", "[[020-01-15-cmd-Linux-磁碟分割與掛載]]"]
updated: 2026-08-29
---
...
```bash
[ -d /sys/firmware/efi ] && echo "UEFI 開機" || echo "BIOS 開機"   # ★★★★ 救援手法完全不同，先確認這個
sudo efibootmgr -v          # ★★★ UEFI 開機項目（BIOS 機器沒有這個表）
lsblk -f | grep -i efi      # ★★★ ESP 分割區，chroot 修復時要一起掛
```

```
BootCurrent: 0001                                 # ★★★ 這次從哪一項開機
BootOrder: 0001,0000                              # ★★★★ 順序被改掉就會開到別的 OS
...
> ```
> GRUB_TIMEOUT=5                                # ★★★★ 少了這 5 秒就進不了救援選單
> GRUB_TIMEOUT_STYLE=menu                       # ★★★★ hidden 等於沒有選單
...
```bash
sudo update-grub                                  # ★★★★ Ubuntu/Debian，忘了跑等於沒改
...
> ```
> 1. GRUB 選單按 e，linux 行尾加：  init=/bin/bash      # ★★★★★ 不需要任何密碼
>    （若有 ro 改成 rw，或進去後 mount -o remount,rw /）   # ★★★★ 忘了 rw 就改不了密碼
> 2. Ctrl+X 開機，得到 root shell
> 3. passwd root            # 或 passwd mike
> 4. 有 SELinux 的系統（RHEL）：touch /.autorelabel       # ★★★★ 漏掉這步之後登不進去
...
```bash
sudo apt autoremove --purge        # ★★★★ 清舊核心的正解，不要自己 rm
```

RHEL 系由 `installonly_limit` 控制（預設 3）：

```bash
grep installonly_limit /etc/dnf/dnf.conf                                # ★★★ 預設保留幾個
sudo dnf remove --oldinstallonly --setopt installonly_limit=2 kernel    # ★★★★ 至少留 2 個
```
...
>
> ★★★★★ 也不要移除**執行中**的核心 —— 模組目錄一起被刪，下次插拔硬體或載入模組就出事：
> ```bash
> uname -r         # ★★★★★ 這個不能刪
> ```
...
```
gzip: stdout: No space left on device                                   # ★★★★ 這時 initramfs 只寫了一半
E: mkinitramfs failure cpio 141 gzip 1
update-initramfs: failed for /boot/initrd.img-6.8.0-45-generic          # ★★★★★ 別重開機，先修好再說
```
...
sudo update-initramfs -u -k all        # ★★★★ 清完一定要把 initramfs 重建回完整的
sudo update-grub                       # ★★★★ 再讓選單與現實一致
```
...
lsinitramfs /boot/initrd.img-$(uname -r) | grep -E 'zfs|lvm|raid'   # ★★★★ 根在 LVM/RAID/ZFS 上就一定要看到
sudo update-initramfs -u                                  # ★★★ 重建目前核心的
sudo update-initramfs -u -k all                           # ★★★★ 全部，改完儲存設定用這個
...
> ```bash
> # RHEL 常用
> sudo grubby --info=ALL                                            # ★★★ 現在每個核心帶什麼參數
> sudo grubby --update-kernel=ALL --args="console=ttyS0,115200"     # ★★★★ ALL 才不會只改到一個核心
> sudo dracut -f --regenerate-all                                   # ★★★★ 等同 update-initramfs -u -k all
> ```
...
```bash
sudo systemctl isolate rescue.target     # ★★★★ 執行中切換（會踢掉所有登入，遠端下這行等於自斷）
...
systemd-analyze blame | head -15          # ★★★ 各服務耗時排序（並列啟動的服務會重複計時）
systemd-analyze critical-chain            # ★★★ 關鍵路徑，才是真正拖慢的那條
systemd-analyze plot > boot.svg           # ★★ 視覺化
```

```
Startup finished in 8.2s (firmware) + 3.1s (loader) + 4.5s (kernel) + 42.3s (userspace) = 58.1s   # ★★★ userspace 佔大宗 = 問題在服務不在硬體
```
...
```
35.412s systemd-networkd-wait-online.service      # ★★★ 典型元凶
...
> ```bash
> sudo systemctl disable systemd-networkd-wait-online.service    # ★★★ 不需要等網路時（有服務相依網路就別停）
> # 或只等特定介面
...
```bash
sudo journalctl -b -1 -e                  # ★★★★ 需要 journal 持久化（見 19 篇），沒開等於沒證據
sudo journalctl -b -1 -p err              # ★★★★ 只看錯誤，找開不了機的原因就從這裡
sudo journalctl --list-boots              # ★★★ 有幾次開機紀錄
last -x reboot shutdown | head            # ★★★ 重開機與關機紀錄（分辨是重開還是斷電）
```
...
掛載系統並 `chroot` 進去，就能像正常開機一樣執行修復指令。★★★★ 這是「機器完全開不起來」時的最後手段。

```bash
# ── 1. 找到根分割區與 ESP ──   ★★★★★ 認錯裝置會修到別顆磁碟
lsblk -f
# 假設根在 /dev/sda2、ESP 在 /dev/sda1（UEFI）

# ── 2. 掛載 ──
sudo mount /dev/sda2 /mnt
sudo mount /dev/sda1 /mnt/boot/efi          # ★★★★ UEFI 才需要，漏掉 grub-install 會裝到錯地方
# 若有獨立 /boot：sudo mount /dev/sdaX /mnt/boot
# ★★★★ 若根是 LVM：sudo vgchange -ay 後掛 /dev/vg/root（Live 環境不會自動啟動 VG）

# ── 3. 綁定虛擬檔案系統（chroot 內的工具需要）──   ★★★★ 少綁一個 grub-install 就會失敗
for d in dev dev/pts proc sys run; do sudo mount --bind "/$d" "/mnt/$d"; done
sudo mount --bind /sys/firmware/efi/efivars /mnt/sys/firmware/efi/efivars 2>/dev/null || true   # ★★★★ UEFI 沒這個就寫不進開機項目

# ── 4. 進入 ──
sudo chroot /mnt /bin/bash

# ── 5. 現在你「在」那個系統裡，執行修復 ──
grub-install /dev/sda                       # ★★★★★ BIOS：重裝 GRUB 到 MBR（寫的是「整顆磁碟」不是分割區）
grub-install --target=x86_64-efi --efi-directory=/boot/efi --bootloader-id=ubuntu   # ★★★★★ UEFI
update-grub                                 # ★★★★ 產生選單
update-initramfs -u -k all                  # ★★★★ 順手把所有核心的 initramfs 重建
# 順便：passwd、修 fstab、apt --fix-broken install ...

# ── 6. 離開並卸載 ──
exit
for d in run sys proc dev/pts dev; do sudo umount "/mnt/$d"; done   # ★★★ 反序卸載，否則卸不掉
...
grub rescue> set prefix=(hd0,gpt2)/boot/grub # ★★★★ 這兩行是關鍵，指錯就還是進不去
...
set -euo pipefail        # ★★★ 任何一步失敗就停，不要帶著半套狀態往下跑

CURRENT=$(uname -r)
echo "目前核心：$CURRENT"

# 1. 空間檢查   ★★★★ /boot 不足會產出半套 initramfs，重開機就開不起來
avail=$(df --output=avail -m /boot | tail -1)
(( avail > 300 )) || { echo "/boot 只剩 ${avail}MB，先清理"; sudo apt autoremove --purge; }

# 2. 記下目前核心的 GRUB 項目名稱，設為「下一次開機」的保險   ★★★★★ 遠端機器唯一的退路
...
| 重開後 `journalctl -b -1` 沒東西 ★★★ | ⑤ | journal 未持久化 | `mkdir /var/log/journal`（見 [[020-01-19-guide-Linux-日誌系統]]） |
...
> 1. ★★★★ **實體安全**：機房門禁、機櫃上鎖（見 [[040-02-13-guide-機房-機房實體安全]]）
...
> # ★★★★ 讓一般開機不需密碼、只有編輯（e）與指令列（c）需要；漏了這步，每次重開機都要有人到現場輸入密碼
...
> ★★★★★ `cryptsetup open /dev/sda2 root` 再掛 `/dev/mapper/root`；金鑰不見就等於資料不見。
> 金鑰保管流程見 [[090-03-03-guide-應用安全-機密管理與金鑰保護]]。
...
- [[020-01-17-cmd-Linux-systemd服務管理]] — target 與服務相依
- [[020-01-15-cmd-Linux-磁碟分割與掛載]] — fstab、UUID 與 emergency mode
- [[020-01-19-guide-Linux-日誌系統]] — journal 持久化才看得到上次開機
- [[020-01-23-guide-Linux-Linux常見疑難排解]] — 開不了機的整體排查流程
- [[020-01-26-guide-Linux-核心模組與sysctl調校]] — 核心參數與模組管理
- [[040-02-09-guide-機房-伺服器上架與初始設定]] — 序列主控台與 IPMI
- [[040-02-13-guide-機房-機房實體安全]] — 實體存取即 root 的防護
