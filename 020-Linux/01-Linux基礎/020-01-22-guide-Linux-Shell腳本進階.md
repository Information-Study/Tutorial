...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[020-01-21-cmd-Linux-Shell腳本入門]]"]
updated: 2026-08-29
---
...
| ★★★★ 重跑 | 很少 | 經常（重試、排程、災後） |
| ★★★ 環境 | 你的 shell | 極簡且不可預測（見 [[020-01-20-guide-Linux-環境變數與設定檔]]） |
...
> 不要想「建立目錄」，想「確保目錄存在」；
> 不要想「加一行」，想「確保那一行在」。
> 這也是 Ansible 等組態管理工具的核心思想——見 [[090-06-06-guide-TWGCB-Linux大量派送]]。
...
> 在 [[020-01-18-guide-Linux-排程工作]] 也提到用 `flock -n lockfile cmd` 從 cron 端包住。
...
> 加一個旗標就能預覽整個流程會做什麼。
> 這在 [[100-02-08-guide-維運-變更管理流程]] 是「變更前驗證」的基本手段。
...
ensure PermitRootLogin no       # ★★★ 冪等：已經是 no 也不會重複附加
ensure PasswordAuthentication no
ensure MaxAuthTries 3
sshd -t                         # ★★★★★ 語法沒過就中止改由 trap 還原；跳過這步等於把自己鎖在門外
...
export -f apply_one; export outdir DRY_RUN REMOTE_SCRIPT   # ★★★ 子 shell 才看得到

grep -vE '^\s*(#|$)' "$list" | xargs -P "$JOBS" -I{} bash -c 'apply_one "$@"' _ {}   # ★★★★ {} 當參數傳
...
> 這仍然是「有風險的批次操作」，先在一台試（見 [[100-02-08-guide-維運-變更管理流程]]）。
...
- [[020-01-21-cmd-Linux-Shell腳本入門]] — 基礎語法
- [[020-01-18-guide-Linux-排程工作]] — 讓腳本自動執行、`flock` 從 cron 端包住
- [[020-01-10-cmd-Linux-程序管理與訊號]] — 訊號、背景工作與孤兒程序
- [[100-02-08-guide-維運-變更管理流程]] — 批次變更的流程與回退計畫
- [[090-06-06-guide-TWGCB-Linux大量派送]] — 從腳本升級到 Ansible
