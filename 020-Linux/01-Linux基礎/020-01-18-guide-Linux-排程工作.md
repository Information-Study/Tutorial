...
`/etc/cron.daily/` 底下放的是**可執行腳本**（不是 crontab 格式）：
...
curl -sS -X POST "$WEBHOOK_URL" \
     ...
     -d "$(jq -n --arg t "❌ [$HOST] $UNIT 執行失敗" --arg l "$LOG" \
              '{text: ($t + "\n```" + $l + "```")}')"
SCRIPT
sudo chmod 750 /usr/local/bin/send-alert.sh
```
...
echo "/sbin/reboot" | at 03:00 tomorrow
