...
> - 用 `rm -I`、`--dry-run`、先 `ls` 後 `rm` 三個習慣避開不可逆的災難
...
> 但記住**別名不會在腳本與 sudo 中生效**（見 [[020-01-03-cmd-Linux-終端機與Shell入門]]）。
...
rm file.txt                # 刪檔案
rm -r dir/                 # 遞迴刪目錄
rm -f file.txt             # 強制，不存在也不報錯
rm -I *.log                # 超過 3 個檔案時確認一次
...
│   ├── storage/
│   └── .env
└── current -> releases/2026-08-27-101542    ← Nginx 指向這裡
```
...
> ```bash
> sudo cp -a /home/mike/.ssh /backup/     # 私鑰跟著複製，且權限保留
> ```
...
> 而 `current` 本身還指著 `v1`。這就是少了 `-n` 的後果。
>
