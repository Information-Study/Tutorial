...
> - 知道 Linux「發行版」是什麼，以及該怎麼選

## 前置知識

- [[02-計概-主機裡面有什麼]] — 有哪些硬體要被管理
- [[06-計概-記憶體與儲存階層]] — 記憶體與儲存的差別
...
> Linux 上用 `cron` 或 `systemd timer` 實作 —— 見 [[18-排程工作]]。
...
| RHEL 系機器上 `apt` 指令找不到 | 發行版家族不同 | RHEL 系用 `dnf`；見 [[14-套件管理]] |
| SELinux 擋住服務，日誌沒有明顯錯誤 | RHEL 系預設啟用 SELinux | `getenforce` 確認；看 `/var/log/audit/audit.log`；見 [[07-SELinux與AppArmor]] |
...
>    正確做法是調整政策 —— 見 [[07-SELinux與AppArmor]]。
...
>
> 見 [[03-弱點與修補管理流程]] 與 [[16-網概-VLAN與網路分段]]。
...
> 這也是 OT／工控環境不能套用一般 IT 修補策略的原因之一。
>
...
- [[09-計概-檔案與檔案系統]] — 作業系統怎麼組織資料
- [[10-計概-軟體授權與程式怎麼跑起來]] — 開源、自由軟體與授權
- [[11-計概-程式程序與多工]] — 程序排程的細節
- [[17-計概-雲端虛擬化與容器]] — 一台機器跑多個作業系統
- [[03-終端機與Shell入門]] — 開始實際使用 CLI（進階）
- [[17-systemd服務管理]] — Linux 的服務管理（進階）
- [[01-Ubuntu與RHEL差異總表]] — 兩大家族的完整對照（進階）
