...
- [[090-01-10-guide-PKI-憑證部署到各服務]] — 憑證已部署
- [[090-01-08-guide-PKI-用自建CA簽發伺服器憑證]] — 簽發與撤銷
...
> [!warning] 效期上限【正在分階段縮短】，不要把天數記死
> 本節刻意不寫死任何天數。**實際上限以 CA/Browser Forum 現行 Baseline Requirements
> 與各 CA 的公告為準**（本文撰寫日：2026-09）。
> 唯一穩定的結論是：**方向只會往更短走，而且回不去了**。

### ★★ 趨勢：效期只會越來越短

```
2015 前  → 3～5 年
2020     → 壓到約【一年】（CA/B Forum）
之後     → ★★ 決議【分階段再縮短】，一階一階往下砍
Let's Encrypt → 90 天（甚至提供更短效期的憑證）

★★★ 對維運來說，重點不是「上限現在是幾天」，而是：
   【「一年換一次憑證」的人工排程，遲早會來不及】
     → 到期日一階一階往前移
       → 你的 SOP、行事曆、交接文件卻停在舊的天數
         → ★★★ 某一次就直接開天窗（服務中斷、公文追究）
```

> [!danger] 最危險的不是「不知道上限縮短了」
> 而是**知道了，但排程與腳本沒跟著改**。
> 所有寫死天數的地方都要改成「讀憑證上的實際到期日」——
> 見上方 `cert-monitor`：它算的是**憑證裡的 `notAfter`**，不是你以為的效期。

### ★★★ 人工續期的失誤率

| 環節 | 人工做法 | ★ 典型失誤 | 後果 |
| --- | --- | --- | --- |
| **記到期日** | 行事曆／便利貼／記在腦袋 | 換人、離職、行事曆沒轉移 | **★★ 到期當天才發現** |
| **產 CSR** | 手打 `openssl req` | 漏 SAN、CN 打錯、金鑰太短 | 重簽一次，與 CA 往返再花幾天 |
| **取得憑證** | 到 CA 網站下載 | **只下載主憑證，忘了中繼** | **★★ 部分客戶端鏈驗證失敗** |
| **部署** | `scp` 覆蓋檔案 | 覆蓋錯路徑、權限沒設對 | 服務起不來，或**私鑰權限外洩** |
| **重載** | 想到才 `reload` | **★★ 忘記 reload** | **檔案是新的、線上還是舊的** |
| **多台** | 一台一台手動換 | **★★ 漏掉 LB 後面的某一台** | 間歇性錯誤，**最難查** |
| **驗證** | 看檔案的到期日 | 沒有從線上驗證 | 以為換好了，其實沒有 |

```
★★ 算一下：就算每個環節只有 5% 出錯率
   7 個環節串起來 → 一次成功的機率只剩約【70%】

★★★ 而且效期越短，同一組流程一年要重跑越多次
   → 一年跑 1 次可能矇混過去
     → 一年跑 4 次、8 次時
       → 【失誤不是「可能」，是「必然」】
...
★ 對照上方〈憑證過期事故的五個原因〉：
   ① 沒有清冊 ② 沒有監控 ③ 負責人離職
   ④ 續期了但忘記 reload ⑤ 只換了 LB 後面的一台
   → ★★ 這五個【全部只會發生在人工流程】
```

### ★★★ 自動化的三條路

**① 公信憑證：ACME（certbot / acme.sh）★★★**

```
適用：對外服務、能做 HTTP-01 或 DNS-01 驗證的網域
做法：certbot 的 systemd timer + ★★ deploy hook 自動 reload
重點：【deploy hook 沒設 = 憑證換了但服務沒換】
```

→ 完整設定見下一節〈自動續期〉與 [[060-02-02-06-guide-Nginx-HTTPS與Certbot]]。

**② Nginx／Angie 內建 ACME：MyGuard 的 `autocert` ★★**

```
適用：已經用 MyGuard 套件庫的 NGINX／Angie
做法：設定檔加 autocert 指令，★ 由 Web 伺服器自己申請與續期
好處：【不需要 certbot、不需要 cron、不需要 deploy hook】
      → 少了三個會壞掉的環節
```

→ 見 [[060-02-05-03-guide-MyGuard-autocert自動憑證模組]]。

**③ 內部 CA：自動簽發 + 自動派送 ★★**

```
適用：內網服務、無法對外驗證的主機、設備管理介面
做法：★ 本篇的 cert-renew 腳本 + 清冊 + 排程
      （或導入 step-ca / Vault PKI 這類支援 ACME 的內部 CA）
重點：內部憑證雖然【不受公信 CA 的效期上限】
      → ★★ 但也應該主動縮短（先把自動續期做穩，再從 365 天縮到 90 天）
```

→ 簽發流程見 [[090-01-08-guide-PKI-用自建CA簽發伺服器憑證]]，
根憑證派送見 [[090-01-09-guide-PKI-根憑證派送與信任]]。

### ★★★ 還在手動的人，現在該做什麼

```
【這週】★★★
  ① 盤點：跑 pki-inventory 與 scan-tls，把所有憑證找出來
  ② 每張都填 owner 與 backup_owner（找不到負責人的先掛在自己身上）
  ③ 部署 cert-monitor + 每日排程，★ 告警送到「會有人看的地方」
     → 先確保【不會再有人是到期才知道】

【這個月】★★
  ④ 挑一個【不重要的對外服務】做 ACME 試點（先用 staging 環境）
  ⑤ ★★ deploy hook 一定要設，並實測「續期後線上憑證真的換了」
  ⑥ 把 LB 後面每一台都納入線上驗證

【這一季】★★★
  ⑦ 把對外服務【全部】轉成 ACME 自動續期
  ⑧ 內部 CA 導入 cert-renew 排程，並縮短內部憑證效期
  ⑨ ★★ 清掉所有 SOP／腳本／交接文件裡【寫死的天數】
     → 改成「讀憑證上的 notAfter」
  ⑩ 把「手動換憑證」從交接文件裡刪掉
     → ★★★ 交接的應該是【自動化系統】，不是【一份行事曆提醒】

★ 驗收標準（很簡單）：
  【你休假兩個月，回來時所有憑證都還是有效的】
```
...
>   2015 前  → 3~5 年
>   2020     → 壓到約【一年】（CA/B Forum）
>   之後     → ★★ CA/B Forum 決議【分階段再縮短】，方向只會往更短走
>              （天數與生效日以現行 Baseline Requirements 與各 CA 公告為準，
>                本文撰寫日：2026-09）
>   Let's Encrypt → 90 天（一開始就是）
>
> ★ 短效期的好處：
>   · 私鑰外洩的暴露期短
>   · 【強迫】自動化（效期縮到幾十天時，手動續期沒人受得了）
>   · 撤銷失效時的風險窗口小
>
> ★★ 內部 CA 也應該朝這個方向：
...
> 短效期等於「內建的撤銷」；
> ③**強迫自動化** —— 效期縮到幾十天時，手動續期沒人受得了，
> 一定會做成腳本，而腳本比人可靠。
> **業界趨勢**：2015 前 3～5 年 → 2020 年壓到約一年 →
> 之後 **CA/B Forum 決議分階段再縮短**，方向只會往更短走
> （具體天數與生效日以現行 Baseline Requirements 與各 CA 公告為準，
> 本文撰寫日：2026-09）。
...
- [[090-01-10-guide-PKI-憑證部署到各服務]] — 部署與重載策略
- [[090-01-08-guide-PKI-用自建CA簽發伺服器憑證]] — 簽發與撤銷
- [[090-01-07-guide-PKI-自建中繼CA與憑證鏈]] — CRL 的自動化產生
- [[090-01-03-guide-PKI-向CA申請憑證]] — 公信 CA 的申請流程
- [[060-02-02-06-guide-Nginx-HTTPS與Certbot]] — certbot 的完整設定
