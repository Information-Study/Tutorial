...
| **10** | certificate has expired | **憑證已過期** | 續期 → [[12-憑證生命週期管理]] |
| **9** | certificate is not yet valid | 生效日還沒到 | **檢查系統時間** |
| **18** | self signed certificate | **自簽憑證** | 裝根憑證或用正式憑證 |
| **19** | self signed certificate in certificate chain | **鏈上有自簽（根）憑證但不信任** | **裝根憑證** → [[09-根憑證派送與信任]] |
...
- [[12-憑證生命週期管理]] — 監控與自動續期（預防勝於排查）
- [[11-憑證格式轉換與檢視工具]] — 檢視與轉換指令
- [[10-憑證部署到各服務]] — 各服務的正確設定
- [[09-根憑證派送與信任]] — 信任存放區的差異
- [[08-用自建CA簽發伺服器憑證]] — SAN 與簽發
