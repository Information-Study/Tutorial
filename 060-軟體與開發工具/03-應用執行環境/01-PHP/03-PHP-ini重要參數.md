...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[02-PHP-FPM設定與Pool調校]]"]
updated: 2026-08-28
---
...
- [[01-PHP-安裝與多版本管理]] — 設定檔位置（CLI 與 FPM 分開）
...
| **改了 `php.ini` 沒生效** ★ | 改到 CLI 的 / 沒重啟 FPM / 被 pool 覆蓋 | 見 [[01-PHP-安裝與多版本管理]] |
...
> 應該用**實際的 RSS** 來算 `max_children`（見 [[02-PHP-FPM設定與Pool調校]]）。
>
...
- [[05-PHP-OPcache與效能]] — OPcache 的完整設定
- [[06-PHP-安全設定]] — open_basedir 與 disable_functions 的深入說明
- [[02-PHP-FPM設定與Pool調校]] — pool 的 php_admin_value
- [[01-PHP-安裝與多版本管理]] — 設定檔位置
- [[04-Composer-套件管理]] — 相依套件
