...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[060-04-02-03-cmd-psql-操作與常用指令]]", "[[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]]", "[[060-01-06-03-guide-傳輸-備份策略與還原演練]]"]
updated: 2026-08-28
---
...
- [[060-04-02-03-cmd-psql-操作與常用指令]] —— 本篇大量使用 `psql -c` / `\l` / `\dt+`，不重複教
- [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]] —— `archive_mode` 要寫進 `postgresql.conf`、備份帳號要在 `pg_hba.conf` 開 `replication` 這條，設定檔位置與 reload/restart 的差別看那篇
- [[060-04-02-02-cmd-PostgreSQL-角色與權限]] —— 本篇沿用該篇的角色觀念，`pg_read_all_data` 這類預設角色在那裡講
- [[060-04-01-03-cmd-MySQL-SQL基礎操作]] —— 比對步驟用到的 `COUNT(*)`、`LEFT JOIN`、`UPDATE ... FROM` 語法在那篇，**本篇不重講 SQL**
- [[060-01-06-03-guide-傳輸-備份策略與還原演練]] —— 3-2-1 原則、RPO / RTO 的一般定義、restic / borg、不可變備份的通論**全部在那篇**。本篇只講「PostgreSQL 這套資料庫怎麼做」
- [[060-04-01-05-svc-MySQL-備份與還原]] —— 同一件事在 MySQL 怎麼做。**兩篇建議對照著看**，本篇會不斷點出差異
...
```text
   ...
   ✗ standby 不是備份 —— 誤 UPDATE 會在毫秒內同步過去。
     串流複寫解決「主機掛掉」，不解決「人做錯事」。建置見 [[060-04-02-07-svc-PostgreSQL-複寫與高可用]]。
...
角色觀念看 [[060-04-02-02-cmd-PostgreSQL-角色與權限]]，這裡只列「備份這件事真正需要什麼」。
...
> （做法見 [[060-04-02-07-svc-PostgreSQL-複寫與高可用]]）。
...
**【c】把資料搬回正式庫**：用 SQL，語法見 [[060-04-01-03-cmd-MySQL-SQL基礎操作]]，本篇不重講。
...
> **必須做兩件事**：
> ① 用 systemd 管理 `pg_receivewal` 並設 `Restart=always`（寫法見 [[020-02-02-01-svc-systemd-unit撰寫實戰]]）；
...
一般性的演練制度看 [[060-01-06-03-guide-傳輸-備份策略與還原演練]] 與 [[120-01-06-guide-雲端-災難復原與異地備援]]。
...
排程（用 systemd timer，理由見 [[020-02-02-02-cmd-systemd-timer與cron選型]]）：
...
| ★★★★ `recovery_target_time` 設對了卻差 8 小時 | 時間字串沒帶時區，套用了伺服器的 `TimeZone`（常是 UTC） | 一律寫成 `'2026-08-26 15:46:30+08'`。時間本身的正確性見 [[020-01-28-cmd-Linux-時間同步NTP與chrony]] |
...
處理順序（**不可跳號**）：
① 先空出磁碟（刪 `/var/log` 的舊檔、擴 LV，見 [[020-01-15-cmd-Linux-磁碟分割與掛載]]）讓資料庫能起來；
...
| **法遵** | 保留期限與當事人權利（如刪除請求）要一併考慮 —— 備份裡的個資也是個資，見 [[090-07-07-guide-資安實踐-台灣資安法規與個資法]] 與 [[090-07-09-guide-資安實踐-資安稽核與符合性檢核]] | ★★★★ |
...
> 那你連「誤操作發生在幾點」都不知道，見 [[020-01-28-cmd-Linux-時間同步NTP與chrony]]。
>
...
> 見 [[020-01-15-cmd-Linux-磁碟分割與掛載]]）讓資料庫能啟動；
...
- [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]] —— `archive_mode` 要 restart、`archive_command` 只要 reload，這個差別與 `pg_settings.context` 的判讀方式在那篇講得最完整；備份角色的 `replication` 認證條目也在那裡
- [[060-04-01-05-svc-MySQL-備份與還原]] —— 同一件事在 MySQL 怎麼做。**特別建議對照 binlog PITR 與本篇 WAL PITR 的差異**，那是兩套資料庫在維運上最大的分歧點
- [[060-04-02-07-svc-PostgreSQL-複寫與高可用]] —— 本篇建立的基礎備份與 WAL 歸檔，正好就是搭建 standby 的材料；也解釋為什麼「有 standby 不等於有備份」
- [[060-01-06-03-guide-傳輸-備份策略與還原演練]] —— 3-2-1 原則、異地與不可變備份、restic / borg 的操作、演練制度的一般性做法，本篇不重複的部分都在那裡
- [[060-04-02-08-svc-PostgreSQL-安全強化]] —— 備份檔的加密、金鑰保管、稽核軌跡要跟整體強化措施一起規劃
- [[100-01-03-guide-日誌-系統監控與告警]] —— 把 `pg_stat_archiver.failed_count`、`pg_wal` 大小、備份腳本 exit code 接上告警的實作
- [[050-02-03-03-guide-範例-Nuxt與PostgreSQL]] —— 實際專案裡這套備份怎麼放進部署流程
