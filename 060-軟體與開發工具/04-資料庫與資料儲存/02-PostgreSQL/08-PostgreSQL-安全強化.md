...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[04-PostgreSQL-設定檔與pg_hba]]", "[[02-PostgreSQL-角色與權限]]", "[[08-用自建CA簽發伺服器憑證]]"]
updated: 2026-08-28
---
...
- [[04-PostgreSQL-設定檔與pg_hba]] — ★★★★ **這篇是本篇的地基**。`pg_hba.conf` 的欄位語法、比對順序、reload 與 restart 的分界都在那裡，本篇只做「安全面該怎麼填」
- [[02-PostgreSQL-角色與權限]] — `CREATE ROLE` / `GRANT` / `ALTER DEFAULT PRIVILEGES` 的完整模型在那篇，本篇只做**盤點與收斂**
- [[08-用自建CA簽發伺服器憑證]] — 本篇直接用機關內部 CA 簽出資料庫憑證，CA 的建置流程不在這裡重寫
- [[02-防火牆-ufw基礎與實務]] — 防火牆語法引用該篇，本篇只講「5432 該放行給誰」
- [[05-SSH-隧道與埠轉發]] — 跨機管理資料庫的正確姿勢

> [!tip] 這篇刻意不重複的內容
> - **SQL 語法**（`GRANT`、`REVOKE`、`CREATE VIEW`）在 [[03-SQL基礎操作]]，本篇只寫安全相關的用法與判讀。
> - 備份的排程、加密與**還原演練**在 [[05-PostgreSQL-備份與還原]]；本篇只講「dump 檔本身是一份個資資產」這一面。
> - 複寫帳號與 `replication` 連線的建置在 [[07-PostgreSQL-複寫與高可用]]；本篇補憑證面。
> - TWGCB 基準文件本身的解讀在 [[01-TWGCB概念與法規要求]]。
> - Wazuh 規則怎麼寫在 [[05-Wazuh-日誌蒐集與解析]]；本篇只講「要送什麼過去、要告警什麼」。

> [!info]- 跟 [[07-MySQL-安全強化]] 對照著看
...
比對順序的完整說明見 [[04-PostgreSQL-設定檔與pg_hba]]，本篇只強調安全後果。
...
版本與套件庫來源見 [[01-PostgreSQL-安裝與初始化]]。
...
> - ★★★★ 多一層 **SELinux**：憑證放到 `/etc/pki/` 以外的路徑會被擋，見 [[07-SELinux與AppArmor]]
> - 防火牆用 `firewalld`（[[04-防火牆-firewalld]]），不是 `ufw`
...
| ★★★★ 離職者／前廠商帳號 | 對照人事名冊與委外契約 | `REASSIGN OWNED` → `DROP OWNED` → `DROP ROLE`（★★★ 順序不能反，見 [[02-PostgreSQL-角色與權限]]） |
...
各欄位語法與 `include_dir` 用法見 [[04-PostgreSQL-設定檔與pg_hba]]。
...
CA 的建置見 [[08-用自建CA簽發伺服器憑證]]，這裡只做資料庫這一段：
...
★★★★ `error` 欄空白才代表對映有載入。複寫本身的設定見 [[07-PostgreSQL-複寫與高可用]]。
...
[[02-PostgreSQL-角色與權限]] 與 [[03-SQL基礎操作]]。
...
> PostgreSQL 主版本）。套件庫設定見 [[03-第三方APT套件庫實務]]。
...
> ★★★★ 送集中日誌前先確認 SIEM 那端的存取控制也到位，見 [[09-日誌集中與SIEM]]。
...
該告警的四件事（規則寫法見 [[05-Wazuh-日誌蒐集與解析]]）：
...
備份檔的加密與**還原演練**在 [[05-PostgreSQL-備份與還原]] 與 [[03-備份策略與還原演練]]，
...
AppArmor（Ubuntu）／SELinux（RHEL）的規則見 [[07-SELinux與AppArmor]]。
...
| ★★★★ 啟用 pgaudit 後磁碟被寫滿，資料庫停止寫入 | `pgaudit.log='all'` + `log_catalog=on`，psql 的 `\d` 就能灌出數百行 | 改用 object audit logging（`pgaudit.role`）只稽核敏感表；`log_catalog=off`；補上磁碟告警與 logrotate（見 [[02-日誌集中與輪替]]） |
...
> 2. 日誌**即時送 SIEM**（本機日誌在入侵時會被刪），見 [[09-日誌集中與SIEM]]
...
>
> ★★★ 動筆前先到 <https://www.nccst.nat.gov.tw/GCB> 確認當下有沒有新增資料庫類基準。
> 見「TWGCB 對應：誠實的說法」與 [[01-TWGCB概念與法規要求]]。

## 延伸閱讀

- [[04-PostgreSQL-設定檔與pg_hba]] — ★★★★ 本篇的地基。pg_hba 的欄位語法、比對順序、`include_dir`、reload 與 restart 的完整分界
- [[02-PostgreSQL-角色與權限]] — `GRANT` / `REVOKE` / `ALTER DEFAULT PRIVILEGES` 的完整模型；本篇只做盤點與收斂
- [[05-PostgreSQL-備份與還原]] — 備份加密、保存期限與 PITR **還原演練**；本篇第 6 層（靜態資料）靠它落實
- [[07-PostgreSQL-複寫與高可用]] — 複寫帳號與 `replication` 連線的建置，本篇補了憑證認證那一段
- [[07-MySQL-安全強化]] — 同一件事在 MySQL 怎麼做，對照著看最快建立差異感（尤其是稽核與 TDE 兩塊完全相反）
- [[08-用自建CA簽發伺服器憑證]] — 本篇用到的資料庫憑證從哪來；[[10-憑證部署到各服務]] 講同一張 CA 怎麼同時服務 Nginx 與資料庫
- [[12-憑證生命週期管理]] — 憑證到期前的輪替流程（★★★ 資料庫憑證輪替只要 reload，不必停機）
- [[05-Wazuh-日誌蒐集與解析]] — 把 `connection authorized` 與 `AUDIT:` 變成會告警的規則
