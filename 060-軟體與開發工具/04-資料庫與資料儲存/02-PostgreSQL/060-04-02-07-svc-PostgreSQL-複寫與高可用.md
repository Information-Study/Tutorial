...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[060-04-02-05-svc-PostgreSQL-備份與還原]]", "[[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]]", "[[060-04-02-02-cmd-PostgreSQL-角色與權限]]"]
updated: 2026-08-28
---
...
| 篇章 | 你會用到裡面的什麼 |
| --- | --- |
| [[060-04-02-05-svc-PostgreSQL-備份與還原]] | ★★★★ WAL 歸檔、PITR 與還原演練。**本篇不取代它** —— 複寫與備份是兩件事 |
| [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]] | ★★★★ `pg_hba.conf` 的比對順序、`scram-sha-256`、改完該 reload 還是 restart |
| [[060-04-02-02-cmd-PostgreSQL-角色與權限]] | 建 `replicator` 角色、`REPLICATION` 屬性、`pg_monitor` 預設角色 |
| [[060-04-02-06-svc-PostgreSQL-效能調校與索引]] | `shared_buffers`、checkpoint 行為；standby 規格為什麼不能比主庫差 |
| [[060-04-01-06-svc-MySQL-主從複寫]] | ★★★ 對照組。兩套資料庫的複寫觀念相通，**故障模式完全不同**，本篇會逐項對照 |
| [[090-02-02-guide-防火牆-ufw基礎與實務]] | 只放行 standby 那一台 IP 的 5432，不對外開放 |
| [[020-01-28-cmd-Linux-時間同步NTP與chrony]] | ★★★ 兩台沒對時，你算出來的延遲秒數全是假的 |

★★ 本篇的 SQL 語法本身不再解釋，需要複習 `SELECT` / `CREATE ROLE` 這類基本語法請看 [[060-04-01-03-cmd-MySQL-SQL基礎操作]]，
`psql` 的操作技巧（`\watch`、`\x`、`-At`）看 [[060-04-02-03-cmd-psql-操作與常用指令]]。
...
> 完整的備份策略、WAL 歸檔與**還原演練**在 [[060-04-02-05-svc-PostgreSQL-備份與還原]]；
> 機關層級的災難復原制度看 [[060-01-06-03-guide-傳輸-備份策略與還原演練]] 與 [[120-01-06-guide-雲端-災難復原與異地備援]]。
> 本篇只負責「硬體故障與單點」這一格。
...
如果真實需求是「查詢很慢」，先看 [[060-01-03-04-guide-監控-效能瓶頸排查方法論]] 與 [[060-04-02-06-svc-PostgreSQL-效能調校與索引]] ——
...
這張表是本篇與 [[060-04-01-06-svc-MySQL-主從複寫]] 的橋。**觀念相通，故障模式完全不同**。
...
| 應用 | `app1` | `10.0.1.21` | Laravel / Nuxt | 見 [[050-02-03-03-guide-範例-Nuxt與PostgreSQL]] |
| WAL 歸檔 | `bak1` | `10.0.1.31` | `/srv/wal`（NFS 或 rsync 目的地） | 見 [[060-04-02-05-svc-PostgreSQL-備份與還原]] |
...
★★★ 第三方套件庫的加入方式（金鑰、pinning、離線機關的鏡像做法）見 [[060-04-02-01-svc-PostgreSQL-安裝與初始化]]。
...
> 規則寫法見 [[090-02-02-guide-防火牆-ufw基礎與實務]]，`listen_addresses` 與 `pg_hba` 的搭配見 [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]]。
...
但**不能讀任何一張表**。這正是我們要的最小權限。角色設計見 [[060-04-02-02-cmd-PostgreSQL-角色與權限]]。
...
> [!danger] ★★★★ `pg_hba.conf` 的三個必踩坑（詳見 [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]]）
...
★★★ 自建 CA 與憑證發放見 [[090-01-08-guide-PKI-用自建CA簽發伺服器憑證]] 與 [[090-01-10-guide-PKI-憑證部署到各服務]]；
...
你不需要像 [[060-04-01-06-svc-MySQL-主從複寫]] 那樣去操心 `super_read_only`。
...
> 這跟 [[060-04-01-06-svc-MySQL-主從複寫]] 裡 `Seconds_Behind_Source: 0` 的假綠燈是同一個陷阱，
> 換了資料庫，人性沒變。
...
所以延遲 standby 是**第三台**，不是把主備那台改設定。真正的誤刪救援還是 PITR，見 [[060-04-02-05-svc-PostgreSQL-備份與還原]]。
...
> | 17 | `pg_basebackup --incremental` + `pg_combinebackup` | 增量 base backup，見 [[060-04-02-05-svc-PostgreSQL-備份與還原]] |
>
...
> 選型的通盤討論見 [[120-01-04-svc-雲端-高可用與負載平衡架構]]。
...
排程（systemd timer 寫法見 [[020-02-02-02-cmd-systemd-timer與cron選型]]）：
...
**【1】公告與維護模式**（依 [[100-02-08-guide-維運-變更管理流程]] 走時窗與通知）
...
| ★★★★ **`FATAL: no pg_hba.conf entry for replication connection from host …`** | `pg_hba.conf` 的 DATABASE 欄寫了 `all`（不匹配複寫），或規則排在一條更寬鬆的規則後面 | 明寫 `replication`；用 `SELECT * FROM pg_hba_file_rules;` 檢查順序，見 [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]] |
...
| ★★★★ **主庫的表急速膨脹、`VACUUM` 清不掉** | standby 開了 `hot_standby_feedback` 又有人在上面掛著一個永不結束的查詢 | 找出 standby 上的長查詢殺掉；設 `idle_in_transaction_session_timeout`；見 [[060-04-02-06-svc-PostgreSQL-效能調校與索引]] |
...
> 這在個資法下是「未採取適當安全維護措施」的典型案例，見 [[090-07-07-guide-資安實踐-台灣資安法規與個資法]]。
...
> 檢核時要一起納入：作業系統強化（[[090-02-08-guide-防護-系統強化與稽核]]）、
> 存取控制、稽核軌跡（[[090-07-09-guide-資安實踐-資安稽核與符合性檢核]])、實體安全。
...
> 見 [[090-01-08-guide-PKI-用自建CA簽發伺服器憑證]] 與 [[090-01-10-guide-PKI-憑證部署到各服務]]。
...
> 然後你會看到莫名其妙的密碼錯誤。
> **④** ★★★★ 複寫帳號的密碼**與應用帳號分開**，並納入 [[090-01-12-guide-PKI-憑證生命週期管理]] 那套輪換節奏。
...
> 見 [[090-08-04-guide-Wazuh-FIM檔案完整性監控]]。

★★★ **切換要留稽核軌跡**：誰在什麼時候 promote、為什麼、影響哪些系統、多久恢復。
這份紀錄是 [[100-02-09-svc-維運-事件處理與升級流程]] 的產出，也是年度資安稽核會被問到的東西。
...
> 參見「先講死一次：複寫不是備份」與 [[060-04-02-05-svc-PostgreSQL-備份與還原]]。
>
> **Q2. 不一定。** ★★★★
...
> ```
> 參見【2】那一段與 [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]]。
>
...
> **②** 把那支報表最佳化（多半是缺索引或全表掃描），見 [[060-04-02-06-svc-PostgreSQL-效能調校與索引]]。
...
- [[060-04-02-05-svc-PostgreSQL-備份與還原]] — ★★★★★ 本篇一直在說「那個要看這篇」的那篇。WAL 歸檔、PITR 與還原演練
- [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]] — ★★★★ `pg_hba.conf` 的比對順序與 `replication` 那一列，本篇最常出錯的地方
- [[060-04-02-02-cmd-PostgreSQL-角色與權限]] — `replicator` 與 `replcheck` 的最小權限設計、`pg_monitor` 預設角色
- [[060-04-02-06-svc-PostgreSQL-效能調校與索引]] — standby 追不上時，減少主庫 WAL 產生量的手段
- [[060-04-02-08-svc-PostgreSQL-安全強化]] — TLS 強制、稽核軌跡、個資保護；本篇 `hostssl` 的完整前置作業
- [[060-04-01-06-svc-MySQL-主從複寫]] — ★★★★ 對照組。同一件事在 MySQL 怎麼做、故障模式差在哪
- [[120-01-04-svc-雲端-高可用與負載平衡架構]] — Patroni / HAProxy / VIP 的選型，以及自動切換值不值得
- [[100-01-03-guide-日誌-系統監控與告警]] / [[100-01-04-guide-日誌-健康檢查與可用性監控]] — 把本篇四個指標接進既有監控
- [[100-02-08-guide-維運-變更管理流程]] / [[100-02-09-svc-維運-事件處理與升級流程]] — 切換演練的公告、紀錄與事後檢討
