...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[060-04-02-01-svc-PostgreSQL-安裝與初始化]]", "[[060-04-02-02-cmd-PostgreSQL-角色與權限]]", "[[060-04-02-03-cmd-psql-操作與常用指令]]"]
updated: 2026-08-28
---
...
- [[060-04-02-01-svc-PostgreSQL-安裝與初始化]] —— 叢集（cluster）概念、`postgres` OS 帳號、服務名稱與資料目錄
- [[060-04-02-02-cmd-PostgreSQL-角色與權限]] —— ★★★★ **`pg_hba.conf` 管「能不能連進來」，`GRANT` 管「連進來能做什麼」**，兩者是不同的關卡，本篇只講前者
- [[060-04-02-03-cmd-psql-操作與常用指令]] —— `psql` 的連線參數、反斜線指令；本篇的驗證步驟大量用到 `psql -c`
- [[060-04-01-04-svc-MySQL-設定檔與調校]] —— 對照組。★★★ **同一件事在 MySQL 是寫在帳號的 host 欄位裡，在 PostgreSQL 是一個獨立檔案**，差異見下一節
- [[090-02-02-guide-防火牆-ufw基礎與實務]] —— 第一道關卡在防火牆，不在資料庫
- [[020-01-17-cmd-Linux-systemd服務管理]] —— `reload` 與 `restart` 在 systemd 層怎麼下、失敗了去哪看

> [!tip] 這篇不講 SQL
> 建帳號、`GRANT`、`REVOKE` 的語法在 [[060-04-02-02-cmd-PostgreSQL-角色與權限]] 與 [[060-04-01-03-cmd-MySQL-SQL基礎操作]]，
...
> | `permission denied for ...` | 【4】授權 | `GRANT`（見 [[060-04-02-02-cmd-PostgreSQL-角色與權限]]） |
>
...
| `replication` | 實體複寫連線（見 [[060-04-02-07-svc-PostgreSQL-複寫與高可用]]） | ★★★★ 必須單獨一行 |
...
> 見 [[090-02-03-guide-防火牆-nftables與iptables]]。
...
> 跨主機的做法見〈完整實戰範例〉與 [[090-01-08-guide-PKI-用自建CA簽發伺服器憑證]]。
...
換成自建 CA 簽的憑證：見 [[090-01-08-guide-PKI-用自建CA簽發伺服器憑證]] 與 [[090-01-05-guide-PKI-自簽憑證快速產生]]。
...
> **不要靠調大 `max_connections` 解決問題** —— 這和 [[060-04-01-04-svc-MySQL-設定檔與調校]]
> 講 `max_connections` 的道理完全一樣，記憶體預算要加得起來。
> 詳細算法見 [[060-04-02-06-svc-PostgreSQL-效能調校與索引]]。
...
日誌輪替與集中見 [[100-01-02-guide-日誌-日誌集中與輪替]]。
...
角色與權限的完整做法見 [[060-04-02-02-cmd-PostgreSQL-角色與權限]]，這裡只做本篇需要的部分。
...
| ★★★ 連線 timeout（等很久才失敗），日誌**完全沒有紀錄** | 封包沒到伺服器 —— 防火牆或路由 | `nc -vz 10.10.20.11 5432`；查 `ufw status`、雲端 SG、實體防火牆（見 [[090-02-02-guide-防火牆-ufw基礎與實務]]） |
...
★★★★ 沒有這些規則就是防火牆擋的。加規則的做法見 [[090-02-02-guide-防火牆-ufw基礎與實務]]。
...
>
> 更完整的稽核設定（`pgaudit`、資料表層級的存取記錄）在 [[060-04-02-08-svc-PostgreSQL-安全強化]]。
...
> `logrotate`（見 [[100-01-02-guide-日誌-日誌集中與輪替]]），或改用 `pgaudit` 做選擇性記錄。
...
> ② 該角色只給唯讀權限（`GRANT SELECT`，見 [[060-04-02-02-cmd-PostgreSQL-角色與權限]]）；
...
> 吃的是真正的資料庫名，不是 `replication` 關鍵字。見〈進階設定與調校〉第一節與 [[060-04-02-07-svc-PostgreSQL-複寫與高可用]]。
>
...
> 見〈排查步驟【2】【7】〉與 [[090-02-02-guide-防火牆-ufw基礎與實務]]。
>
...
> ★★★ 這是本篇〈觀念說明〉那張四關卡圖最想避免的鬼打牆。詳見 [[060-04-02-02-cmd-PostgreSQL-角色與權限]]。
>
> **Q6.** 兩個原因與查證：
...
> 憑證怎麼簽發見 [[090-01-08-guide-PKI-用自建CA簽發伺服器憑證]]。見〈進階設定與調校〉第六節。
>
...
- [[060-04-02-02-cmd-PostgreSQL-角色與權限]] —— ★★★★ 本篇管「連不連得進來」，那篇管「進來能做什麼」。看到 `permission denied` 就該去那一篇
- [[060-04-02-01-svc-PostgreSQL-安裝與初始化]] —— cluster、`pg_createcluster`、資料目錄的來龍去脈，本篇的路徑都從那裡來
- [[060-04-02-03-cmd-psql-操作與常用指令]] —— `\password`、`\drds`、`\dp` 這些排錯必用的反斜線指令
- [[060-04-02-08-svc-PostgreSQL-安全強化]] —— 本篇是「連線層」的安全，那篇涵蓋 `pgaudit`、資料列層級安全（RLS）與稽核落地
- [[060-04-02-07-svc-PostgreSQL-複寫與高可用]] —— `replication` 這一行規則的完整脈絡，以及切換時 `pg_hba.conf` 要跟著改什麼
- [[060-04-01-04-svc-MySQL-設定檔與調校]] —— 對照組：MySQL 把來源限制放在帳號裡，PostgreSQL 放在獨立檔案，兩套模型的差異
- [[090-01-08-guide-PKI-用自建CA簽發伺服器憑證]] —— `hostssl` 與 `cert` 認證需要的憑證怎麼簽、怎麼派送
- [[090-02-02-guide-防火牆-ufw基礎與實務]] —— 四道關卡的第一關，`Connection timed out` 的解法在那裡
