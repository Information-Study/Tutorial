...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[060-04-02-01-svc-PostgreSQL-安裝與初始化]]", "[[060-04-02-03-cmd-psql-操作與常用指令]]", "[[060-04-01-02-cmd-MySQL-使用者與權限]]"]
updated: 2026-08-28
---
...
- [[060-04-02-01-svc-PostgreSQL-安裝與初始化]] —— 已經有一台跑起來的 PostgreSQL 16/17，知道 `postgres` 這個作業系統帳號與 `postgres` 這個資料庫角色是兩回事
- [[060-04-02-03-cmd-psql-操作與常用指令]] —— 會用 `psql`、`\c`、`\dt`，本篇大量使用反斜線指令
- [[060-04-01-03-cmd-MySQL-SQL基礎操作]] —— `SELECT` / `WHERE` / `JOIN` 語法本篇**不重講**，盤點查詢直接寫
- [[060-04-01-02-cmd-MySQL-使用者與權限]] —— 本篇會不斷跟 MySQL 對照；如果你是從 MySQL 過來的，先讀它會省很多力氣
- [[020-01-09-cmd-Linux-使用者與群組管理]] —— Linux 的 user / group；PostgreSQL 角色跟 OS 帳號**只有 `peer` 認證那一個接點**
- [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]] —— 第一道關卡在那一篇，本篇只負責第二道之後
...
> RLS 的完整處理在 [[060-04-02-08-svc-PostgreSQL-安全強化]]，本篇只教你「怎麼確認它是不是元兇」。
...
> [!note] 本篇的界線
> - **`pg_hba.conf` 的比對順序、`peer` / `trust` / `scram-sha-256` 差異**：在 [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]]，那是第一道關卡
> - **RLS（列級安全）、`ssl` 強制、稽核日誌方案**：在 [[060-04-02-08-svc-PostgreSQL-安全強化]]
> - **`bak_hr` 角色實際怎麼跑備份與 PITR**：在 [[060-04-02-05-svc-PostgreSQL-備份與還原]]
> - **複寫角色 `REPLICATION` 屬性怎麼接上 standby**：在 [[060-04-02-07-svc-PostgreSQL-複寫與高可用]]
> - **`SELECT` / `JOIN` 語法本身**：在 [[060-04-01-03-cmd-MySQL-SQL基礎操作]]，本篇的盤點查詢不再解釋語法
...
| `REPLICATION` | 可開複寫連線 | ★★★★ **等於可以整份複製資料庫出去**，見 [[060-04-02-07-svc-PostgreSQL-複寫與高可用]] | ★★★★★ |
...
看到 `SCRAM-SHA-256$4096:` 開頭就對了。★★★★ 若看到 `md5` 開頭，代表 `password_encryption` 被設成 `md5`（PG14 起預設已是 `scram-sha-256`），請到 [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]] 修正後**重設每一個角色的密碼**（改參數不會自動轉換既有雜湊）。
...
> 備份帳號的正確處理見 [[060-04-02-05-svc-PostgreSQL-備份與還原]] 的還原演練那一節。
>
...
機關人事系統 `hr_app`：Laravel 後端 + Nuxt 前端（見 [[050-02-03-03-guide-範例-Nuxt與PostgreSQL]]），
...
| **★★★★★ `pg_dump` 成功但還原後某張表少了列** | 該表有 RLS，備份角色沒有 `BYPASSRLS`（`pg_read_all_data` **不繞過 RLS**） | 備份改用有 `BYPASSRLS` 的角色或表擁有者；見 [[060-04-02-05-svc-PostgreSQL-備份與還原]] |
...
| **★★★ `VALID UNTIL` 到期了帳號還連得進來** | 它只讓**密碼**失效；`pg_hba.conf` 那條若是 `peer` / `trust` / `cert` 就不受影響 | 停用請用 `ALTER ROLE x NOLOGIN;`，並檢查 `pg_hba.conf`（見 [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]]） |
| **★★ `SELECT` 有結果但少了幾列，沒有任何錯誤** | 表上啟用了 RLS 而該角色沒有對應 policy | `SELECT relrowsecurity FROM pg_class WHERE relname='staff';` 為 `t` 就是它；處理見 [[060-04-02-08-svc-PostgreSQL-安全強化]] |
...
有列出來就是 RLS 在過濾。★★★★ 這時**不要**急著給 `BYPASSRLS`（那等於把個資遮蔽整個關掉），正確做法是補一條對應的 policy，見 [[060-04-02-08-svc-PostgreSQL-安全強化]]。
...
★★★ 法規面的完整說明見 [[090-07-07-guide-資安實踐-台灣資安法規與個資法]]，稽核佐證的整理方式見 [[090-07-09-guide-資安實踐-資安稽核與符合性檢核]]。
...
> ```
> 改完 `sudo systemctl reload postgresql@16-main`（reload 不是 restart，見 [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]]）。
>
> ```text
> 【驗證 1】授權欄位讀得到
...
> 別浪費時間查 `\dp`。
> 見「觀念說明 → 五道關卡」與 [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]]。
>
> **Q2.** ★★★★ **錯，而且錯得很典型。**
...
- [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]] —— 本篇的「第 ① 關」在那裡：`pg_hba.conf` 的**比對順序**、`trust` / `peer` / `scram-sha-256` 的差別、改完該 reload 還是 restart
- [[060-04-02-01-svc-PostgreSQL-安裝與初始化]] —— `postgres` 系統帳號與 `postgres` 資料庫角色的關係，以及 `initdb` 時就決定的 `password_encryption`
- [[060-04-02-03-cmd-psql-操作與常用指令]] —— `\dp` `\ddp` `\drg` `\dn+` 的完整用法與輸出格式
- [[060-04-02-08-svc-PostgreSQL-安全強化]] —— RLS（本篇的「第 ⑥ 關」）、強制 SSL、稽核日誌，以及個資遮蔽的完整做法
- [[060-04-02-05-svc-PostgreSQL-備份與還原]] —— `bak_hr` 角色的實際用法、**PITR**，以及「備份帳號少權限導致備份不完整」的還原演練
- [[060-04-02-07-svc-PostgreSQL-複寫與高可用]] —— `REPLICATION` 屬性為什麼等同於「可以整份複製資料庫出去」
- [[060-04-01-02-cmd-MySQL-使用者與權限]] —— 同一件事在 MySQL 怎麼做；兩篇對照著讀，兩套系統的模型差異會非常清楚
- [[060-04-01-03-cmd-MySQL-SQL基礎操作]] —— 本篇盤點查詢用到的 `JOIN` / `string_agg` / `array_to_string` 語法
- [[050-02-03-03-guide-範例-Nuxt與PostgreSQL]] —— 本篇建立的 `app_hr` / `mig_hr` 在實際部署裡怎麼寫進 `.env` 與 CI
- [[070-03-04-guide-Laravel-Eloquent與資料庫]] —— Laravel 的連線設定、migration 與本篇 `SET ROLE` 策略的接點
- [[090-07-02-guide-資安實踐-密碼與帳號管理實務]] —— 機關的密碼原則、離職與異動的帳號清理流程
- [[090-07-07-guide-資安實踐-台灣資安法規與個資法]] —— 欄位層授權與 RLS 對應到的法規義務
- [[090-07-09-guide-資安實踐-資安稽核與符合性檢核]] —— 本篇的盤點查詢輸出要怎麼整理成稽核佐證
