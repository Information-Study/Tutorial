...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]]", "[[060-04-02-03-cmd-psql-操作與常用指令]]", "[[060-04-01-03-cmd-MySQL-SQL基礎操作]]"]
updated: 2026-08-28
---
...
- [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]] —— ★★★★ **本篇只講「該設成多少」，「設定檔在哪、改完 reload 還是 restart」全部在那一篇**，不重述
- [[060-04-02-03-cmd-psql-操作與常用指令]] —— `\d`、`\di+`、`\timing`、`\x` 這些本篇會一直用到
- [[060-04-01-03-cmd-MySQL-SQL基礎操作]] —— ★★★ **SQL 語法與索引的基本概念在那一篇**，本篇假設你已經知道什麼叫 JOIN、什麼叫 WHERE
- [[060-04-01-04-svc-MySQL-設定檔與調校]] —— 對照組。同一件事（編記憶體預算、抓慢查詢）在 MySQL 怎麼做
- [[060-01-03-04-guide-監控-效能瓶頸排查方法論]] —— ★★★ **先確定瓶頸真的在資料庫**再打開這一篇
...
│  PHP-FPM  40 × 48 MB  1920 MB   ← 見 [[060-03-01-02-guide-PHP-FPM設定與Pool調校]] │
│  ────────────────────────────                                     │
...
用 `conf.d` 的獨立檔案。做法與理由見 [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]]，這裡只放值。
...
`public $withinTransaction = false;`（見 [[070-03-04-guide-Laravel-Eloquent與資料庫]]）。
...
>   見 [[070-03-04-guide-Laravel-Eloquent與資料庫]] 與 [[060-04-04-svc-DB-Redis快取入門]]。
...
```text
判斷準則（★★★★★ 直接抄進監控告警，見 [[100-01-03-guide-日誌-系統監控與告警]]）：
...
主機的 WAL 與 XID 就永遠卡在那一天（相關設定見 [[060-04-02-07-svc-PostgreSQL-複寫與高可用]]）：
...
| 12 | 備份仍正常 | 見 [[060-04-02-05-svc-PostgreSQL-備份與還原]] 的還原演練 | 演練通過 | ★★★★★ |
...
| ★★★ 磁碟被 `/var/log/postgresql/` 寫爆 | `log_min_duration_statement = 0`（記錄每一條）或 `log_statement = all` | 改成 `1000`（1 秒）；設定 logrotate，見 [[100-01-02-guide-日誌-日誌集中與輪替]] |
...
mean 很小、calls 爆炸   → ★★★★ 應用層 N+1，資料庫這邊調不動，回 [[070-03-04-guide-Laravel-Eloquent與資料庫]]
...
>    檔案系統、繞過所有 RLS 與 `pg_hba` 的邏輯限制。權限設計見 [[060-04-02-02-cmd-PostgreSQL-角色與權限]]。
...
> 只能從備份還原（見 [[060-04-02-05-svc-PostgreSQL-備份與還原]]）。
...
- [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]] —— 本篇所有「改設定」的動作，reload/restart 的判斷依據都在那一篇
- [[060-04-02-05-svc-PostgreSQL-備份與還原]] —— ★★★★★ 調校前先確認備份可還原；本篇提到的 WAL 與 slot 也是 PITR 的基礎
- [[060-04-02-07-svc-PostgreSQL-複寫與高可用]] —— replication slot 的正確用法，以及副本退役時該怎麼收尾
- [[060-04-02-08-svc-PostgreSQL-安全強化]] —— 慢查詢日誌的個資風險、稽核軌跡與最小權限的完整做法
- [[060-04-01-04-svc-MySQL-設定檔與調校]] —— 對照組：同樣是編記憶體預算，MySQL 的算法為什麼不一樣
- [[060-04-01-03-cmd-MySQL-SQL基礎操作]] —— 索引與 SQL 語法的基礎；本篇假設你已經讀過
- [[070-03-04-guide-Laravel-Eloquent與資料庫]] —— N+1 查詢、eager loading，以及 migration 裡怎麼用 `CONCURRENTLY`
- [[060-01-03-04-guide-監控-效能瓶頸排查方法論]] —— 打開本篇之前，先用它確認瓶頸真的在資料庫
- [[100-01-03-guide-日誌-系統監控與告警]] —— 把 `age(datfrozenxid)`、`n_dead_tup`、命中率接進告警的做法
