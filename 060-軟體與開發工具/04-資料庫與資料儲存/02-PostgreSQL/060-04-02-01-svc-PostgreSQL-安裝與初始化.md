...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[020-01-14-guide-Linux-套件管理]]", "[[020-01-17-cmd-Linux-systemd服務管理]]", "[[060-04-01-01-svc-MySQL-安裝與初始化]]"]
updated: 2026-08-28
---
...
- [[020-01-14-guide-Linux-套件管理]] — apt / dnf 的第三方套件庫、`purge` 與 `remove` 的差別
- [[020-01-17-cmd-Linux-systemd服務管理]] — `systemctl`、template unit（`postgresql@17-main.service` 這種寫法）、`journalctl -u`
- [[020-01-15-cmd-Linux-磁碟分割與掛載]] — datadir 要放獨立磁碟或 LVM 時需要
- [[020-01-28-cmd-Linux-時間同步NTP與chrony]] — 資料庫時間對不上，稽核紀錄就沒有價值
- [[060-04-01-01-svc-MySQL-安裝與初始化]] — ★★★ 本篇大量與它對照。先讀過那篇，這篇會快一倍
- [[130-01-01-guide-部署-部署共通觀念]] — 這台機器在整個部署流程裡的位置
...
**② AI 與向量檢索**（pgvector 擴充）、**③ 新開發專案**（見 [[050-02-03-03-guide-範例-Nuxt與PostgreSQL]]）。
...
> [!note] 這篇不談什麼
> `pg_hba.conf` 的比對順序與各認證方式細節 → [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]]（★★★★ 那篇才是重頭戲）；
> 角色與權限設計 → [[060-04-02-02-cmd-PostgreSQL-角色與權限]]；psql 操作 → [[060-04-02-03-cmd-psql-操作與常用指令]]；
> SQL 語法 → [[060-04-01-03-cmd-MySQL-SQL基礎操作]]（**PostgreSQL 的 SQL 不重講，那篇通用**）；
> 備份與 PITR → [[060-04-02-05-svc-PostgreSQL-備份與還原]]。**本篇只負責把地基打對。**
...
> 離線環境請自行核對。第三方套件庫的通用管理原則見 [[020-01-14-guide-Linux-套件管理]]。
...
>
> ★★★ 兩系差異的通盤整理見 [[980-01-ref-附錄-Ubuntu與RHEL差異總表]]。

---

### ★★★★ peer 認證：`psql -U postgres` 為什麼一定失敗

這是 PostgreSQL 新手卡最久的地方，對應 MySQL 的 `auth_socket`（見 [[060-04-01-01-svc-MySQL-安裝與初始化]]）。
...
與 `pg_hba.conf` 的**比對順序**，是 [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]] 的主題，本篇只讓你能連進去。
...
應用程式與資料庫同機時走 socket 即可；真要跨機，做法見 [[060-04-02-08-svc-PostgreSQL-安全強化]] 與
[[090-02-02-guide-防火牆-ufw基礎與實務]]，**先開防火牆白名單再改 `listen_addresses`**，順序不能反。
...
| `log_min_duration_statement` | `-1` | ★★ reload | 慢查詢紀錄，見 [[060-04-02-06-svc-PostgreSQL-效能調校與索引]] |
...
★★★ 這裡只做「能跑起來」的最小設定，權限收斂交給 [[060-04-02-02-cmd-PostgreSQL-角色與權限]]。
...
> 完整的權限收斂流程見 [[060-04-02-02-cmd-PostgreSQL-角色與權限]]。
...
> 這與 [[060-04-01-01-svc-MySQL-安裝與初始化]] 裡 AppArmor 的坑是同一類問題，只是換成 SELinux。
...
> 各認證方式的差異見 [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]]。
...
正式環境的備份策略請走 [[060-04-02-05-svc-PostgreSQL-備份與還原]]，本腳本的 dump 只是保命網，不是備份。
...
| 17 | **還原演練** | 見 [[060-04-02-05-svc-PostgreSQL-備份與還原]] | ★★★★★ **沒演練過的備份不算數** | ★★★★★ |
...
| **`FATAL: no pg_hba.conf entry for host "10.x.x.x", user "appuser", database "appdb", no encryption`** ★★★★ | `pg_hba.conf` 沒有涵蓋這個來源網段 | 加一列 `host appdb appuser 10.x.x.0/24 scram-sha-256` 後 **reload**；見 [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]] |
...
| **`PANIC: could not write to file "pg_wal/xlogtemp.xxx": No space left on device`** ★★★★★ | WAL 所在磁碟寫滿，資料庫直接停機 | 緊急：清 `log/` 或擴充磁碟後啟動；根治：datadir 移獨立碟＋監控，見 [[100-01-03-guide-日誌-系統監控與告警]] |
| **`FATAL: sorry, too many clients already`** ★★★ | 連線數超過 `max_connections` | 短期調大並 **restart**；長期上連線池，見 [[060-04-02-06-svc-PostgreSQL-效能調校與索引]] |
...
| **稽核軌跡** | `log_line_prefix` 帶 `%m [%p] %u@%d`，日誌集中收；見 [[100-01-02-guide-日誌-日誌集中與輪替]] | ★★★★ |
| **時間正確** | `timedatectl` 同步 + `log_timezone='Asia/Taipei'`；時間錯的稽核紀錄在調查時沒有證據力 | ★★★★ |
| **個資保護** | 含個資的欄位不要進 `log_min_duration_statement` 的參數紀錄；備份檔加密。見 [[090-07-07-guide-資安實踐-台灣資安法規與個資法]] | ★★★★★ |
...
連線加密強制（`hostssl`）這些是 [[060-04-02-08-svc-PostgreSQL-安全強化]] 的範圍。
...
> 見 [[090-02-02-guide-防火牆-ufw基礎與實務]] 與 [[060-04-02-08-svc-PostgreSQL-安全強化]]。
>
> **Q7.** 三種做法，★★★ 由臨時到正式：
> ```bash
> # ① 明確帶埠（臨時查驗最常用）
...
- [[060-04-02-04-svc-PostgreSQL-設定檔與pg_hba]] — ★★★★★ 本篇只讓你連得進去；**`pg_hba.conf` 的比對順序、
  trust／peer／scram-sha-256 的差異、改完 reload 還是 restart**，全部在那篇
- [[060-04-02-02-cmd-PostgreSQL-角色與權限]] — 本篇只建了一個 `appuser`；role 當群組用、`GRANT` 到 schema 層、
  唯讀報表帳號的完整設計在那篇
- [[060-04-02-03-cmd-psql-操作與常用指令]] — `\` 指令、`\copy`、`ON_ERROR_STOP`、批次腳本寫法
- [[060-04-02-05-svc-PostgreSQL-備份與還原]] — ★★★★★ 驗收檢查表第 17 項；**PITR（WAL 歸檔 + 還原到時間點）
  與還原演練**，沒演練過的備份不算數
- [[060-04-02-06-svc-PostgreSQL-效能調校與索引]] — `shared_buffers`／`work_mem` 怎麼算、`EXPLAIN` 怎麼看
- [[060-04-02-08-svc-PostgreSQL-安全強化]] — TLS 連線、`pgaudit`、RLS、個資與稽核情境
- [[060-04-01-01-svc-MySQL-安裝與初始化]] — ★★★ 同一件事在 MySQL 怎麼做；兩篇對照著看，差異會很清楚
- [[060-04-01-03-cmd-MySQL-SQL基礎操作]] — 建完庫之後的第一批 SQL（**PostgreSQL 通用，本章不重講語法**）
- [[050-02-03-03-guide-範例-Nuxt與PostgreSQL]] — 這個資料庫要交給誰用
