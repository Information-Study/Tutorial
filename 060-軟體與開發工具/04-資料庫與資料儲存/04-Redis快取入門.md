...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[14-套件管理]]", "[[17-systemd服務管理]]", "[[26-核心模組與sysctl調校]]"]
updated: 2026-08-28
---
...
- [[14-套件管理]] —— `apt` / `dnf` 安裝與套件版本查詢，本篇不重複
- [[17-systemd服務管理]] —— 本篇會用到 **template unit**（`redis-server@.service`）與 drop-in
- [[26-核心模組與sysctl調校]] —— `vm.overcommit_memory`、`net.core.somaxconn` 的機制在那篇，
  本篇只講「Redis 該設什麼值、不設會看到什麼錯誤」
- [[02-防火牆-ufw基礎與實務]] —— 規則語法在那篇，本篇只講「該放行給誰」
- [[03-ss-netstat-與lsof]] —— 看監聽面與連線數的工具

> [!tip] 這篇不重複的內容（先講清楚邊界，省你時間）
> - **Laravel 應用層的快取 API**（`Cache::remember()`、`Cache::tags()`、`cache:clear` 與
>   `config:cache` 的差別）在 [[04-Laravel-快取最佳化與部署流程]]。
>   本篇只從 Redis 這一側講「那些指令打進來之後，Redis 上發生了什麼」。
> - **佇列與 Horizon**（`queue:work`、Supervisor、失敗重試）在 [[03-Laravel-佇列排程與Supervisor]]。
>   本篇只講兩個伺服器面的後果：**payload 佔記憶體**、**沒開持久化時任務會消失**。
> - **session 與認證流程**（Sanctum／JWT、cookie 設定）在 [[04-認證串接-Sanctum與JWT]]。
>   本篇只講 session 存進 Redis 之後的儲存面問題：被淘汰、被 flush、重啟消失。
> - [[01-Laravel-環境需求與安裝]] 用大約 10 行給了「最小可動版」的 Redis 安裝，
...
| 日誌 | `/var/log/redis/redis-server.log` | 送集中日誌的做法見 [[19-日誌系統]] |
...
處理方式在 [[03-Laravel-佇列排程與Supervisor]]，本篇只負責告訴你「它會吃掉你的 maxmemory」。
...
> ```
>
> 設定方式（機制與其他 sysctl 項目見 [[26-核心模組與sysctl調校]]）：
>
> ```bash
...
```bash
# ④ 備份資料目錄（還原演練與保存週期見 [[03-備份策略與還原演練]]）
...
> prefix 底下的 key，細節見 [[04-Laravel-快取最佳化與部署流程]]）。
>
...
>
> 對策：
> 1. ★★★★★ **刪掉所有 `phpinfo()` 頁面**，並用 [[04-nmap-埠掃描與盤點]] 的方式定期盤點
...
| 命中率 < 80% | TTL 太短，或快取 key 設計把不該快取的東西丟進來 | 回到應用層看 [[04-Laravel-快取最佳化與部署流程]] | ★★★ |
...
```bash
# 關掉 THP（Redis 啟動時會警告；機制見 [[26-核心模組與sysctl調校]]）
...
應用層設定的細節在 [[04-Laravel-快取最佳化與部署流程]]。
...
| ★★★★★ `top` 出現不明高 CPU 行程、`crontab` 多出項目 | Redis 曾經未授權對外，被寫入 crontab 挖礦 | 立刻隔離主機，走 [[04-資安事件應變流程]]。★★★★ **不要只殺行程重裝 Redis** |
...
>
> 對策（Compose 語法見 [[02-Compose-多服務編排實戰]]，本篇只給結論）：
...
> 4. ★★★★ 每次部署後用 [[04-nmap-埠掃描與盤點]] 從**外部**掃一次，
>    不要相信 `ufw status`
...
```bash
# ⑤ 監聽面與防火牆（語法見 [[02-防火牆-ufw基礎與實務]]）
#    bind 內網IP + 127.0.0.1、ufw 只放行 web 主機
# ⑥ 跨網段一律走 SSH 隧道（[[05-SSH-隧道與埠轉發]]）或 tls-port
...
>    資料庫層的稽核論述見 [[07-MySQL-安全強化]]，本篇不重複。
...
- [[04-Laravel-快取最佳化與部署流程]] —— 應用層怎麼用快取（`Cache::remember()`、標籤、
  `cache:clear` vs `config:cache`）。本篇講「Redis 這一側會發生什麼」，那篇講「程式怎麼寫」
- [[03-Laravel-佇列排程與Supervisor]] —— `queues:default` 一直變長時，
  worker 與 Supervisor 該怎麼調；本篇只負責告訴你它會吃掉 `maxmemory`
- [[04-認證串接-Sanctum與JWT]] —— session 與 token 的差別、多台 web 為什麼需要共用 session store
- [[26-核心模組與sysctl調校]] —— `vm.overcommit_memory`、`somaxconn`、THP 的完整機制與其他調校項目
- [[02-防火牆-ufw基礎與實務]] —— 本篇的 ufw 白名單規則語法與規則順序
- [[04-資安事件應變流程]] —— 發現挖礦跡證之後的保全、通報與重建程序
- [[03-備份策略與還原演練]] —— 把 `/var/lib/redis*` 納入備份排程，以及還原演練怎麼做
- [[07-MySQL-安全強化]] —— 同一台機器上另一個資料服務的加固思路，可對照閱讀
