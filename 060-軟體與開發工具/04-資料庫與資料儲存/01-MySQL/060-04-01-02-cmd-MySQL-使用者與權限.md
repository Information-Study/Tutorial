...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[060-04-01-01-svc-MySQL-安裝與初始化]]", "[[020-01-09-cmd-Linux-使用者與群組管理]]"]
updated: 2026-08-28
---
...
- [[060-04-01-01-svc-MySQL-安裝與初始化]] —— 已經有一台跑起來的 MySQL，知道 `mysql_secure_installation` 做了什麼
- [[060-04-01-03-cmd-MySQL-SQL基礎操作]] —— 會用 `SELECT` 與 `WHERE`（本篇的稽核查詢全是 SQL）
- [[020-01-09-cmd-Linux-使用者與群組管理]] —— Linux 的 user / group 觀念；MySQL 帳號跟 OS 帳號**完全無關**（除了 socket 認證那一個例外）
- [[090-07-02-guide-資安實踐-密碼與帳號管理實務]] —— 機關的密碼原則與離職帳號清理流程
...
> 相關設定見 [[130-01-04-02-guide-Laravel-Nginx與PHP-FPM設定]] 與 [[060-03-01-02-guide-PHP-FPM設定與Pool調校]]。
...
> [!note] 本篇的界線
> - **憑證與 `REQUIRE SSL` / `REQUIRE X509`**：只在角色設計時提一句，實作在 [[060-04-01-07-svc-MySQL-安全強化]]
> - **`bind-address` 收斂、稽核日誌方案**：全部在 [[060-04-01-07-svc-MySQL-安全強化]]
> - **backup 帳號怎麼跑備份**：在 [[060-04-01-05-svc-MySQL-備份與還原]]
> - **repl 帳號怎麼建複寫**：在 [[060-04-01-06-svc-MySQL-主從複寫]]
> - **`SELECT` / `JOIN` 語法本身**：在 [[060-04-01-03-cmd-MySQL-SQL基礎操作]]
...
> ```
> 相關設定檔說明見 [[060-04-01-04-svc-MySQL-設定檔與調校]]。

> [!tip] 優先修用戶端，不要降伺服器
> - PHP：7.4 以後的 mysqlnd 對 `caching_sha2_password` 支援穩定；7.1 以前不支援。
>   若卡在舊 PHP，正解是升 PHP（見 [[060-03-01-02-guide-PHP-FPM設定與Pool調校]]），不是降 MySQL 安全性。
...
| Laravel queue / schedule | 同「只跑查詢與寫入」 | 見 [[130-01-04-03-guide-Laravel-佇列排程與Supervisor]] | ★★ |
...
> 2. 容器化（見 [[130-01-02-03-guide-Vue-Docker部署]] 的同類情境）遷移時語意一致
...
-- 部署後（★★★ 這行常被忘記，要寫進部署腳本，見 [[130-01-06-guide-部署-部署自動化]]）
...
> ```
> 個資揭露範圍的判斷原則見 [[090-07-09-guide-資安實踐-資安稽核與符合性檢核]]。

#### 3) `backup` —— 備份帳號（[[060-04-01-05-svc-MySQL-備份與還原]] 會直接用）
...
#### 4) `repl` —— 複寫帳號（[[060-04-01-06-svc-MySQL-主從複寫]] 會直接用）
...
> 帳號生命週期的機關規範見 [[090-07-02-guide-資安實踐-密碼與帳號管理實務]]。
...
> - 備份這台機器時要把它視為機密資料（見 [[090-03-03-guide-應用安全-機密管理與金鑰保護]]）
...
實際的備份策略、保留週期與**還原演練**在 [[060-04-01-05-svc-MySQL-備份與還原]] 與 [[060-01-06-03-guide-傳輸-備份策略與還原演練]]。
...
`bind-address` 收斂與防火牆規則屬於 [[060-04-01-07-svc-MySQL-安全強化]]，這裡不重複。
...
> 稽核佐證的整理方式見 [[090-07-09-guide-資安實踐-資安稽核與符合性檢核]]。
...
走 [[090-03-04-guide-應用安全-備份災難復原與入侵應變]] 的應變流程。
...
> 機密的正確存放方式見 [[090-03-03-guide-應用安全-機密管理與金鑰保護]]。
...
> 見「進階應用 → backup 帳號」與 [[060-04-01-05-svc-MySQL-備份與還原]]。
>
> **Q6.** ★★★ 答案是 **(C)**。
...
> 若來源不明或頻率極高，視為暴力破解事件，走 [[090-03-04-guide-應用安全-備份災難復原與入侵應變]]。
...
- [[060-04-01-01-svc-MySQL-安裝與初始化]] —— `mysql_secure_installation` 實際做了哪四件事，以及 root 的 `auth_socket` 從哪來
- [[060-04-01-07-svc-MySQL-安全強化]] —— 本篇建立的帳號要在這裡加上 `REQUIRE SSL`、收斂 `bind-address`、開稽核日誌
- [[060-04-01-05-svc-MySQL-備份與還原]] —— `backup` 帳號的實際用法、備份檔加密與**還原演練**
- [[060-04-01-06-svc-MySQL-主從複寫]] —— `repl` 帳號怎麼接上複寫，以及為什麼它只需要一個權限
- [[060-04-01-04-svc-MySQL-設定檔與調校]] —— `validate_password`、`activate_all_roles_on_login` 這些參數放哪個設定檔、怎麼安全地改
- [[130-01-04-02-guide-Laravel-Nginx與PHP-FPM設定]] —— `.env` 的 `DB_HOST` / `DB_USERNAME` 與本篇帳號的對應
- [[090-07-02-guide-資安實踐-密碼與帳號管理實務]] —— 機關的密碼原則、離職與異動帳號清理流程
- [[090-03-04-guide-應用安全-備份災難復原與入侵應變]] —— 憑證外洩或帳號被暴力破解後的應變步驟
- [[090-07-09-guide-資安實踐-資安稽核與符合性檢核]] —— 盤點 CSV 要怎麼整理成稽核佐證
