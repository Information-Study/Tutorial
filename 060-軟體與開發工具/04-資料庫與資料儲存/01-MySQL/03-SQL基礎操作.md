...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[01-MySQL-安裝與初始化]]", "[[02-MySQL-使用者與權限]]"]
updated: 2026-08-28
---
...
- [[01-MySQL-安裝與初始化]] — 服務起得來、`mysql` client 連得上，本篇才有得下手
- [[02-MySQL-使用者與權限]] — 本篇一律用**唯讀帳號**先查，要改資料才換帳號
- [[14-計概-資料庫是什麼]] — 表、列、欄、主鍵這些名詞的白話說明
- [[10-故障排除方法論]] — 「先確認問題範圍再動手」的通用流程，本篇是它在資料庫上的實作
...
正規化、ACID 的完整定義不在本篇範圍（見 [[14-計概-資料庫是什麼]]），
維運上只要記住這張對照：
...
| ★★★★ | 日常用 `root` 連 | 查資料用唯讀帳號就夠，見 [[02-MySQL-使用者與權限]] |
...
> 而且沒有任何確認提示**。唯一的救法是 [[05-MySQL-備份與還原]] 的
...
**相關參數（調整屬於 [[04-MySQL-設定檔與調校]]，這裡只要看得懂）**
...
（見 [[04-Laravel-Eloquent與資料庫]]），維運端能做的是：把上面這段抓下來交給開發。
...
> ```
> 慢查詢日誌與錯誤日誌的開啟方式見 [[04-MySQL-設定檔與調校]]。
>
...
`stage/innodb/alter%`），細節見 [[04-MySQL-設定檔與調校]]。
...
> 有主從架構時還要加 `--max-lag`，避免把複寫延遲拉爆（見 [[06-MySQL-主從複寫]]）。
...
> 環境：Ubuntu 24.04 + MySQL 8.0.42 + PHP-FPM + Nginx（見 [[01-範例-Nginx-PHP-MySQL]]）。
...
慢查詢日誌的開啟與 `pt-query-digest` 分析屬於 [[04-MySQL-設定檔與調校]]，
這裡假設它已經開著，我們只是**使用結果**。
...
| 現象 | 原因 | 解法 |
| --- | --- | --- |
| ★★★★★ 執行 `UPDATE` 後 `Rows matched: 3118422`，整張表被改 | WHERE 漏寫、被終端機截斷，且 `autocommit=1` 立刻落地 | **立刻停止寫入**，依 [[05-MySQL-備份與還原]] 做全備 + binlog 時間點還原。事前防：`--safe-updates` + `BEGIN` 包起來 |
...
- **看到 `Command` 幾乎全是 `Sleep` 且數量逼近 `max_connections`
  → 是連線池問題，不是 SQL 問題，見 [[04-MySQL-設定檔與調校]] 與 [[02-PHP-FPM設定與Pool調校]]。**
...
- **`type` 都很漂亮但還是慢 → 不是 SQL 問題，看磁碟與 buffer pool（[[04-MySQL-設定檔與調校]]）。**
...
| 要求面向 | 本篇對應做法 | ★ |
| --- | --- | --- |
| **最小權限** | 查詢一律用 `ops_ro`（只有 `SELECT`），要改資料才切 `ops_rw`，DDL 另有 `ops_ddl`。見 [[02-MySQL-使用者與權限]] | ★★★★ |
...
| **傳輸加密** | 遠端連線一律走 TLS：`mysql --ssl-mode=REQUIRED`；驗證 `SHOW STATUS LIKE 'Ssl_cipher';` 不為空。設定見 [[07-MySQL-安全強化]] | ★★★★ |
| **備份可還原** | 動任何 `UPDATE`/`DELETE`/`ALTER` 之前確認**昨天的備份還原演練是通過的**。見 [[05-MySQL-備份與還原]] 與 [[04-備份災難復原與入侵應變]] | ★★★★★ |
...
> 資安面的完整要求見 [[07-MySQL-安全強化]] 與 [[01-資安全景圖與縱深防禦]]。

**SQL injection 不在本篇範圍** —— 那是應用層要用參數化查詢解決的問題，
見 [[02-應用層安全]] 與 [[04-Laravel-Eloquent與資料庫]]。
但維運端要知道：**慢查詢日誌裡出現 `' OR 1=1` 這類字串，就是被打了**，
應依 [[04-備份災難復原與入侵應變]] 啟動應變。
...
| `/var/log/mysql/mysql-slow.log` | 慢查詢日誌（見 [[04-MySQL-設定檔與調校]]） | ★★★★ |
...
- [[01-MySQL-安裝與初始化]] — 本篇所有指令的前提：服務起得來、client 連得上
- [[02-MySQL-使用者與權限]] — 把 `ops_ro` / `ops_rw` / `ops_ddl` 三種帳號建起來，這是最小權限的實作
- [[04-MySQL-設定檔與調校]] — ★★★★ 慢查詢日誌的開啟、`pt-query-digest` 分析、buffer pool 與連線數；本篇「使用」慢查詢的結果，「怎麼產生」在這一篇
- [[05-MySQL-備份與還原]] — ★★★★★ 本篇每一次「敢按 Enter」的底氣都來自這裡的**還原演練**，沒演練過的備份等於沒有備份
- [[06-MySQL-主從複寫]] — 本篇一再強調「在複本上分析」，複本怎麼建、延遲怎麼看在這裡
- [[07-MySQL-安全強化]] — 連線加密、稽核日誌、備份加密與機關個資情境的完整要求
- [[04-Laravel-Eloquent與資料庫]] — 開發端怎麼避免 N+1、怎麼寫參數化查詢；本篇找出的問題最終要在這裡修
- [[04-效能瓶頸排查方法論]] — 當 `EXPLAIN` 都很漂亮卻還是慢時，往上一層看 CPU / 記憶體 / 磁碟
- [[04-備份災難復原與入侵應變]] — 資料被誤刪或疑似遭入侵時的應變流程
