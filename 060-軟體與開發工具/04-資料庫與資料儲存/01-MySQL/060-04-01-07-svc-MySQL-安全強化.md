...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[060-04-01-02-cmd-MySQL-使用者與權限]]", "[[060-04-01-04-svc-MySQL-設定檔與調校]]", "[[090-01-08-guide-PKI-用自建CA簽發伺服器憑證]]"]
updated: 2026-08-28
---
...
- [[060-04-01-02-cmd-MySQL-使用者與權限]] — 本篇只做**盤點與收斂**，`CREATE USER` / `GRANT` 的語法與角色設計在那一篇
- [[060-04-01-04-svc-MySQL-設定檔與調校]] — `my.cnf` 的載入順序、drop-in 目錄、`SET PERSIST` 的行為
- [[090-01-08-guide-PKI-用自建CA簽發伺服器憑證]] — 本篇會直接用機關內部 CA 簽出資料庫憑證，CA 的建置流程不在這裡重寫
- [[090-02-02-guide-防火牆-ufw基礎與實務]] — 防火牆規則語法引用該篇，本篇只講「資料庫該放行給誰」
- [[020-02-01-05-cmd-SSH-隧道與埠轉發]] — 跨機管理資料庫的正確姿勢

> [!tip] 這篇不重複的內容
> - 備份的排程、加密與**還原演練**在 [[060-04-01-05-svc-MySQL-備份與還原]]；本篇只講「備份檔本身是一份個資資產」這一面。
> - 複寫連線的建置在 [[060-04-01-06-svc-MySQL-主從複寫]]；本篇給憑證面的完整做法，但不重寫複寫設定。
> - TWGCB 基準文件本身的解讀在 [[090-06-01-guide-TWGCB-TWGCB概念與法規要求]]；本篇只講「資料庫主機怎麼對應」。
...
> **不是** `setenforce 0`。詳見 [[090-02-07-guide-防護-SELinux與AppArmor]]。
>
...
監聽面收好之後，防火牆是第二道。規則語法見 [[090-02-02-guide-防火牆-ufw基礎與實務]]，
這裡只講「資料庫該放行給誰」：
...
>
> 完整說明見 [[020-02-01-05-cmd-SSH-隧道與埠轉發]]。
...
`CREATE USER` / `GRANT` 的語法在 [[060-04-01-02-cmd-MySQL-使用者與權限]]，這裡做的是**已上線系統的收尾**：
把兩年來累積的帳號攤開來看。
...
CA 的建置與 `openssl ca` 的操作在 [[090-01-08-guide-PKI-用自建CA簽發伺服器憑證]]，這裡只列資料庫這一張憑證的
**特殊要求**：
...
> 憑證輪換與到期監控見 [[090-01-12-guide-PKI-憑證生命週期管理]]。
...
> PHP 端的其他安全參數見 [[060-03-01-06-guide-PHP-安全設定]]。
...
>    見 [[060-04-01-05-svc-MySQL-備份與還原]] 與 [[090-03-03-guide-應用安全-機密管理與金鑰保護]]。
>
...
要送 Wazuh 監看的四類事件（規則寫法見 [[090-08-05-guide-Wazuh-日誌蒐集與解析]]，
日誌管線見 [[090-05-09-guide-資安設備-日誌集中與SIEM]]）：
...
WAF 的部分見 [[090-05-04-guide-資安設備-Web應用防火牆WAF]]，應用端的注入防護見 [[090-03-02-guide-應用安全-應用層安全]]
與 [[070-03-04-guide-Laravel-Eloquent與資料庫]]。
...
民眾服務系統（LXMP，見 [[050-02-03-02-guide-範例-Laravel完整堆疊]]），**已上線兩年**，
...
| ★★★ 表空間加密開了，但 `mysqldump` 出來的檔還是明文 | 表空間加密只保護磁碟上的檔案 | 備份另外加密，見 [[060-04-01-05-svc-MySQL-備份與還原]] |
...
> 完整說明見 [[090-07-07-guide-資安實踐-台灣資安法規與個資法]]。
...
導入與檢測流程見 [[090-06-01-guide-TWGCB-TWGCB概念與法規要求]]、[[090-06-04-guide-TWGCB-Linux本機導入]]
與 [[090-06-07-guide-TWGCB-Linux檢測與符合性報告]]；符合性報告的組織方式見 [[090-07-09-guide-資安實踐-資安稽核與符合性檢核]]
與 [[090-02-08-guide-防護-系統強化與稽核]]。
...
> 見「靜態資料保護：三層各擋什麼」那張表與 [[060-04-01-05-svc-MySQL-備份與還原]]。
>
> **Q6. 風險是應用會在半夜集體掛掉。★★★★**
...
> 見 `--ssl-mode` 那張對照表與 [[090-01-09-guide-PKI-根憑證派送與信任]]。
>
...
- [[060-04-01-02-cmd-MySQL-使用者與權限]] — 本篇只做盤點與收斂，帳號與角色的完整設計在那裡
- [[060-04-01-05-svc-MySQL-備份與還原]] — 備份加密、保存期限與**還原演練**，本篇的第 5 層靠它落實
- [[090-03-04-guide-應用安全-備份災難復原與入侵應變]] — 資料庫被拖庫或勒索後的應變順序
- [[090-01-08-guide-PKI-用自建CA簽發伺服器憑證]] — 本篇用到的資料庫憑證從哪來
- [[090-01-10-guide-PKI-憑證部署到各服務]] — 同一張 CA 怎麼同時服務 Nginx、MySQL 與內部工具
- [[090-08-05-guide-Wazuh-日誌蒐集與解析]] — 把 error log 與連線紀錄變成會告警的規則
