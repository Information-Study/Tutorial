...
★★ 這篇在 LXMP（Linux + Nginx/Apache + MySQL + PHP/Laravel）的 **M** 從單機變兩台之後，
上層要跟著改：Laravel 的 `config/database.php` 要分 read/write（本篇最後一段），
Nginx 那層的維護模式見 [[04-Nginx-反向代理與負載平衡]]，整套堆疊見 [[02-範例-Laravel完整堆疊]]。
...
★★★ 分成兩條執行緒是為了**把「拿到資料」與「套用資料」解耦**：
主庫掛掉時 IO thread 早已把事件抓進 relay log，applier 可以繼續跑完 ——
...
| **群組複寫 MGR / InnoDB Cluster** | 多數節點達成共識才回 | ★ 最低 | 最高，對網路品質敏感 | 需要**自動故障切換**且有人力長期維運時 |

> [!tip] ★★★ MGR / InnoDB Cluster 什麼時候該考慮（本篇不展開建置）
> 三個條件同時成立才考慮：**①** 需要秒級自動切換（人來切太慢）、
> **②** 三個節點在同一機房、網路延遲穩定在 1ms 以內、**③** 有人能長期維運
> （MGR 出事時的排查難度遠高於主從）。
> 只滿足一兩項時，「主從 + 寫得清楚的切換 runbook + 定期演練」更可靠。
...
| 機關情境的要求 | 做法 | 對應 |
| --- | --- | --- |
| **連線加密** ★★★★ | `repl` 帳號 `REQUIRE SSL`、`SOURCE_SSL=1` + `SOURCE_SSL_CA`；監控帳號 `ssl-mode=VERIFY_CA` | [[07-MySQL-安全強化]] |
| **最小權限** ★★★★ | 複寫帳號只給 `REPLICATION SLAVE`、監控帳號只給 `REPLICATION CLIENT, PROCESS`，來源限內網 | [[02-MySQL-使用者與權限]] |
| **稽核軌跡** ★★★★ | 從庫的 `super_read_only` 每一次被關閉都要有紀錄；切換每一步寫進事件單 | [[09-事件處理與升級流程]] |
| **備份加密與異地** ★★★★ | 初始同步的產出是**完整資料庫明文複本**，用完要加密或安全刪除 | [[05-MySQL-備份與還原]] |
...
導入前先讀 [[01-TWGCB概念與法規要求]] 與 [[02-TWGCB-Linux基準文件解讀]]
確認你的機關適用哪一份基準與版本，★★★ **不要憑印象引用條號**。
