...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[020-01-14-guide-Linux-套件管理]]", "[[020-01-17-cmd-Linux-systemd服務管理]]"]
updated: 2026-08-28
---
...
- [[020-01-14-guide-Linux-套件管理]] — apt / dnf 的套件庫、`purge` 與 `remove` 的差別
- [[020-01-17-cmd-Linux-systemd服務管理]] — `systemctl`、drop-in 覆寫、`journalctl -u`
- [[020-01-15-cmd-Linux-磁碟分割與掛載]] — 資料要搬到獨立磁碟或 LVM 時需要
- [[020-01-28-cmd-Linux-時間同步NTP與chrony]] — 資料庫時間對不上的源頭多半在這裡
...
> [!note] 這篇不談什麼
> 權限與 `GRANT` 設計 → [[060-04-01-02-cmd-MySQL-使用者與權限]]；參數調校 → [[060-04-01-04-svc-MySQL-設定檔與調校]]；
> TLS 與稽核 → [[060-04-01-07-svc-MySQL-安全強化]]；備份 → [[060-04-01-05-svc-MySQL-備份與還原]]。本篇只負責**把地基打對**。
...
>   選版本時**先確認你打算用的備份工具支援它**，見 [[060-04-01-05-svc-MySQL-備份與還原]]。
> - **複寫**：8.4 之後的複寫語法改用 `SOURCE`／`REPLICA` 系列關鍵字，
>   舊的 `MASTER`／`SLAVE` 語法已移除，見 [[060-04-01-06-svc-MySQL-主從複寫]]。
...
參數本身的意義留給 [[060-04-01-04-svc-MySQL-設定檔與調校]]，路徑速查見 [[000-03-ref-索引-設定檔路徑速查]]。
...
要開放遠端連線的正確做法（限定來源、走 TLS、搭配防火牆）見 [[060-04-01-07-svc-MySQL-安全強化]] 與 [[090-02-02-guide-防火牆-ufw基礎與實務]]。
...
> 檔案權限與備份加密仍然要做，見 [[090-03-04-guide-應用安全-備份災難復原與入侵應變]]。
...
既有專案要拿掉前先確認資料表的 row format，見 [[070-03-04-guide-Laravel-Eloquent與資料庫]]。
...
```text
yes          # ★★★★ 是 no 的話上面所有設定都沒有意義，去看 [[020-01-28-cmd-Linux-時間同步NTP與chrony]]
```
...
權限的完整設計、`%` 與 `localhost` 的差別、唯讀報表帳號怎麼開，全部在 [[060-04-01-02-cmd-MySQL-使用者與權限]]。
...
前置：`/data` 已經是獨立磁碟或 LV 並且掛好了（見 [[020-01-15-cmd-Linux-磁碟分割與掛載]]）。
...
> 於是「上個月明明好好的，昨天更新完就起不來了」。AppArmor 本身見 [[090-02-07-guide-防護-SELinux與AppArmor]]。
...
> ```
> 備份與**還原演練**（備份存在不等於還原得回來）見 [[060-04-01-05-svc-MySQL-備份與還原]] 與 [[060-01-06-03-guide-傳輸-備份策略與還原演練]]。
...
> 並且**把救援時間壓到最短**（目標五分鐘內）。
> 機關環境還要留一筆紀錄：誰、什麼時候、為什麼做了這次救援，見 [[090-03-04-guide-應用安全-備份災難復原與入侵應變]]。
...
| 17 | **備份已排程且還原演練過** | 見 [[060-04-01-05-svc-MySQL-備份與還原]] | 有還原演練紀錄 | ★★★★★ |
...
| **`ERROR 2003 (HY000): Can't connect to MySQL server on '10.x.x.x' (111)`** ★★★ | `bind-address=127.0.0.1`（正常）或防火牆擋住 | 本機服務走 socket 即可；真要遠端請照 [[060-04-01-07-svc-MySQL-安全強化]] 開，不要直接改 `0.0.0.0` |
...
| **啟動後幾秒又自己停掉，日誌有 `Cannot allocate memory`** ★★★ | `innodb_buffer_pool_size` 超過實體記憶體 | 先調小啟動，再依 [[060-04-01-04-svc-MySQL-設定檔與調校]] 重新計算 |
...
>   接著就是密碼暴力破解。要遠端存取請走 SSH tunnel 或限定來源 IP ＋ TLS，見 [[060-04-01-07-svc-MySQL-安全強化]]。
...
| 項目 | 本篇要做到的 | 延伸 |
| --- | --- | --- |
| **最小權限** | 應用帳號只綁 `localhost`、只授單一資料庫；root 維持 `auth_socket` | [[060-04-01-02-cmd-MySQL-使用者與權限]] |
| **傳輸加密** | 本機連線走 socket（不經網路）；跨機一律 TLS | [[060-04-01-07-svc-MySQL-安全強化]] |
| **稽核軌跡** | 時區與 NTP 必須正確，否則日誌時間無法作為證據 | [[020-01-19-guide-Linux-日誌系統]] |
| **備份加密** | 備份檔含全部個資，落地前就要加密，且要有還原演練 | [[060-04-01-05-svc-MySQL-備份與還原]]、[[090-03-04-guide-應用安全-備份災難復原與入侵應變]] |
...
> 唯一的補救是從損壞前的備份還原（所以請看 [[060-04-01-05-svc-MySQL-備份與還原]]）。
...
> ```
> 密碼放 `~/.my.cnf`（600）或 `--login-path`，不要寫在指令列。見 [[060-04-01-02-cmd-MySQL-使用者與權限]]。
>
...
> 移除前先確認既有資料表的 row format，見 [[070-03-04-guide-Laravel-Eloquent與資料庫]]。
>
...
- [[060-04-01-02-cmd-MySQL-使用者與權限]] — 本篇只建了一個 `appuser`，權限設計、`%` 與 `localhost`、唯讀報表帳號在那篇
- [[060-04-01-04-svc-MySQL-設定檔與調校]] — `my.cnf` 的參數（buffer pool、`max_connections`、慢查詢）與載入順序細節
- [[060-04-01-05-svc-MySQL-備份與還原]] — ★★★★★ 本篇的驗收表第 17 項；**沒做過還原演練的備份不算數**
- [[060-04-01-07-svc-MySQL-安全強化]] — `bind-address` 收斂、TLS 連線、稽核日誌、TWGCB 與個資法情境
- [[060-04-01-03-cmd-MySQL-SQL基礎操作]] — 建完庫之後的第一批 SQL
- [[130-01-04-01-guide-Laravel-環境需求與安裝]] — 這個資料庫要交給誰用：`.env` 的 `DB_*` 設定與 `migrate`
- [[050-02-03-01-guide-範例-Nginx-PHP-MySQL]] — LXMP 四件套組起來的完整範例
- [[020-02-03-01-svc-標準化-新機建置標準流程]] — 本篇的佈建與驗收腳本要掛進這份流程
