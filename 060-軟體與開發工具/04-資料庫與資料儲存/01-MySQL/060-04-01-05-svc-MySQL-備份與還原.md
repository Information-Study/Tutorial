...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[060-04-01-02-cmd-MySQL-使用者與權限]]", "[[060-04-01-04-svc-MySQL-設定檔與調校]]", "[[060-01-06-03-guide-傳輸-備份策略與還原演練]]"]
updated: 2026-08-28
---
...
- [[060-04-01-02-cmd-MySQL-使用者與權限]] —— 本篇直接沿用該篇建立的 `backup` 帳號，不重複 GRANT 語句
- [[060-04-01-04-svc-MySQL-設定檔與調校]] —— binlog 參數要寫進 `my.cnf`，設定檔位置與載入順序看那篇
- [[060-01-06-03-guide-傳輸-備份策略與還原演練]] —— 3-2-1 原則、RPO / RTO 的一般性定義、restic / borg 的操作、勒索軟體不可變備份的通論，**全部在那篇**。本篇只講「MySQL 這個資料庫要怎麼做」
- [[060-04-01-03-cmd-MySQL-SQL基礎操作]] —— 比對步驟會用到 `COUNT(*)` 與 `CHECKSUM TABLE`
...
```text
   ...
   ✗ replica 不是備份 —— 誤 DELETE 會在 0.1 秒內同步過去。
     複寫解決「主機掛掉」，不解決「人做錯事」。建置見 [[060-04-01-06-svc-MySQL-主從複寫]]。
```
...
帳號在 [[060-04-01-02-cmd-MySQL-使用者與權限]] 已建好，這裡只列「備份這件事真正需要什麼」。
...
> 金鑰保管制度見 [[090-03-03-guide-應用安全-機密管理與金鑰保護]]。
...
> **所以備份時段要跟 Laravel 的 `php artisan migrate` 排開**（見 [[130-01-05-06-guide-Vue-Laravel完整部署實戰]]）。
...
> 制度面見 [[090-03-03-guide-應用安全-機密管理與金鑰保護]]，法規面見 [[090-07-07-guide-資安實踐-台灣資安法規與個資法]]。
...
★★★ 用 systemd 服務跑它、`Restart=always`，再用 [[060-01-06-02-guide-rsync-同步與備份]] 每 5 分鐘同步異地。
...
> 檔案系統快照本身見 [[020-01-24-guide-進階儲存-ZFS與Btrfs]] 與 [[020-01-15-cmd-Linux-磁碟分割與掛載]]。
...
**你驗證的是一個你根本沒有的環境**。容器操作見 [[050-02-01-01-svc-Docker-容器概念與Docker安裝]]。
...
`IOSchedulingClass` 可以限流。完整比較見 [[020-02-02-02-cmd-systemd-timer與cron選型]]。
...
> buffer pool 調校見 [[060-04-01-04-svc-MySQL-設定檔與調校]]。
...
之後若涉及行政責任或資安事件通報，原始檔案要留著。通報流程見 [[090-03-04-guide-應用安全-備份災難復原與入侵應變]]。
...
| 2 | 承辦有正式庫 DELETE 權限 | ★★★★ 收回直連權限改走應用，見 [[060-04-01-07-svc-MySQL-安全強化]] | 09-30 |
...
| **★★★★ 備份腳本從三週前就在失敗，沒人發現** | 沒有失敗告警，日誌沒人讀 | 加告警（webhook + mail）＋**成功心跳**；監控接法見 [[100-01-03-guide-日誌-系統監控與告警]] |
...
- `0` → 時間窗口猜錯（機器時區？`SHOW VARIABLES LIKE 'time_zone'`；見
  [[020-01-28-cmd-Linux-時間同步NTP與chrony]]），或事件在別的 binlog 檔裡，往前一個檔找。
...
> **離線副本 + 不可變（immutable / WORM）副本**，做法見 [[090-05-14-guide-資安設備-備份與抗勒索防護]]。
>
...
> 手法見 [[090-05-08-guide-資安設備-資料防護DLP與加密]]。
>
> **⑥ 拿備份直接覆蓋正式庫來「救」誤刪的資料** ——
...
| 面向 | 具體要求 | 星級 |
| --- | --- | --- |
| **個資保護** | 備份檔**靜態加密**、傳輸走加密通道（SSH / TLS）；備份也是個資檔案的一部分，適用個資法的安全維護義務。見 [[090-07-07-guide-資安實踐-台灣資安法規與個資法]] | ★★★★ |
| **保留與銷毀** | 保留期限要**寫成規定**（日 7 / 週 4 / 月 12），到期**確實銷毀**並留銷毀紀錄；不能「硬碟還有空間就一直留」—— 留越久外洩面越大 | ★★★★ |
| **稽核軌跡** | 誰在何時**取用了備份檔**、誰執行了還原、演練紀錄與覆核簽章都要留存；備份主機的存取日誌接進 SIEM，見 [[090-05-09-guide-資安設備-日誌集中與SIEM]] | ★★★ |
...
> 本篇不引用任何條號以免誤導。實作見 [[060-04-01-07-svc-MySQL-安全強化]] 與 [[090-02-08-guide-防護-系統強化與稽核]]。
...
>   與 [[100-02-07-ref-維運-維護檢查表範本]] 的季度維護表併存。
...
- [[060-01-06-03-guide-傳輸-備份策略與還原演練]] —— 3-2-1 原則、RPO / RTO 的一般性定義、restic / borg 的操作、勒索軟體不可變備份的通論都在那篇，本篇刻意不重講
- [[090-03-04-guide-應用安全-備份災難復原與入侵應變]] —— 事故的通報流程、證據保全、對外說明；本篇的「保全 binlog」只是其中一步
- [[060-04-01-02-cmd-MySQL-使用者與權限]] —— 備份帳號 `backup` 與還原帳號 `restore` 的建立與 `ACCOUNT LOCK`
- [[060-04-01-06-svc-MySQL-主從複寫]] —— 「複寫不是備份」的完整論證與 replica 建置；本篇的 binlog 設定是它的前置
- [[060-04-01-04-svc-MySQL-設定檔與調校]] —— `my.cnf` 載入順序、buffer pool 與 `innodb_flush_log_at_trx_commit` 的取捨
- [[060-04-01-07-svc-MySQL-安全強化]] —— 連線加密、最小權限、稽核日誌，與本篇的備份加密互補
- [[090-05-14-guide-資安設備-備份與抗勒索防護]] —— 不可變（immutable / WORM）副本與離線副本的實作
- [[020-02-02-02-cmd-systemd-timer與cron選型]] —— 為什麼備份排程建議用 timer 而不是 cron
- [[090-07-07-guide-資安實踐-台灣資安法規與個資法]] —— 備份檔的保存、加密、銷毀在法規上的義務
