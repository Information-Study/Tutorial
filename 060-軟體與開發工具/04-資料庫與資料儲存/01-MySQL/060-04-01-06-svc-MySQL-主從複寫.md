...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[060-04-01-05-svc-MySQL-備份與還原]]", "[[060-04-01-02-cmd-MySQL-使用者與權限]]", "[[060-04-01-04-svc-MySQL-設定檔與調校]]"]
updated: 2026-08-28
---
...
| 篇章 | 你會用到裡面的什麼 |
| --- | --- |
| [[060-04-01-05-svc-MySQL-備份與還原]] | ★★★★ 備份與 PITR。**本篇不取代它** —— 複寫與備份是兩件事，兩件都要做 |
| [[060-04-01-02-cmd-MySQL-使用者與權限]] | 建 `repl` 複寫帳號、`REPLICATION SLAVE` 權限、來源網段限制 |
| [[060-04-01-04-svc-MySQL-設定檔與調校]] | `my.cnf` 的載入順序、buffer pool 與 `innodb_flush_log_at_trx_commit` |
| [[060-04-01-07-svc-MySQL-安全強化]] | `bind-address`、TLS 憑證的產製與 `REQUIRE SSL`。本篇只說「複寫連線要走加密」 |
| [[090-02-02-guide-防火牆-ufw基礎與實務]] | 只放行從庫 IP 對 3306，不對外開放 |

★★ 這篇在 LXMP（Linux + Nginx/Apache + MySQL + PHP/Laravel）的 **M** 從單機變兩台之後，
上層要跟著改：Laravel 的 `config/database.php` 要分 read/write（本篇最後一段），
Nginx 那層的維護模式見 [[060-02-02-04-guide-Nginx-反向代理與負載平衡]]，整套堆疊見 [[050-02-03-02-guide-範例-Laravel完整堆疊]]。
...
> 備份策略、備份驗證與**還原演練**全部在 [[060-04-01-05-svc-MySQL-備份與還原]]，
> 機關層級的災難復原制度見 [[090-03-04-guide-應用安全-備份災難復原與入侵應變]] 與 [[120-01-06-guide-雲端-災難復原與異地備援]]。
> 本篇只負責「硬體故障與單點」這一格。
...
如果是「查詢很慢」，先看 [[060-01-03-04-guide-監控-效能瓶頸排查方法論]] —— 加從庫不會讓缺索引的查詢變快。
...
安裝與初始化（`mysql_secure_installation`、`bind-address`）見 [[060-04-01-01-svc-MySQL-安裝與初始化]]。
...
> 規則寫法與順序陷阱見 [[090-02-02-guide-防火牆-ufw基礎與實務]]；
> `bind-address` 與 TLS 憑證的產製見 [[060-04-01-07-svc-MySQL-安全強化]]。
...
> 制度面見 [[100-02-08-guide-維運-變更管理流程]]。
...
> （順便讓 PITR 的可回溯範圍變長，見 [[060-04-01-05-svc-MySQL-備份與還原]]）。
...
> 嚴重時直接把正式庫磁碟撐爆。
> **超過 50 GB 一律用 XtraBackup，並排在離峰時段**，公告流程走 [[100-02-08-guide-維運-變更管理流程]]。
...
帳號改用 `SHOW CREATE USER` / `pt-show-grants` 另外匯出，見 [[060-04-01-02-cmd-MySQL-使用者與權限]]。
...
> 記得把「重啟後檢查複寫是否啟動」寫進 [[100-01-04-guide-日誌-健康檢查與可用性監控]] 的檢查項。
...
告警門檻怎麼設，寫進 [[100-01-03-guide-日誌-系統監控與告警]]：
...
修法（★★★ 大表要走線上 DDL，並依 [[080-01-03-guide-專管-風險與變更管理]] 的流程）：
...
大表加主鍵本身就是一次高風險變更。把上面那段 SQL 放進 [[070-06-07-guide-測試-自動化健康檢查實戰]] 的每日檢查。
...
| 比較 | 延遲從庫 | PITR（[[060-04-01-05-svc-MySQL-備份與還原]]） |
| --- | --- | --- |
...
> 三件事一定要做：
> **①** 把跳過的 GTID 與原因寫進 [[100-02-09-svc-維運-事件處理與升級流程]] 的事件單。
...
> 這個數字就是你的 RTO，也是寫進 [[120-01-06-guide-雲端-災難復原與異地備援]] 的依據。
...
> 規模大一點會用 **Keepalived VIP** 或 **ProxySQL**，建置見 [[120-01-04-svc-雲端-高可用與負載平衡架構]]。
...
> Eloquent 細節見 [[070-03-04-guide-Laravel-Eloquent與資料庫]]，佇列見 [[130-01-04-03-guide-Laravel-佇列排程與Supervisor]]。
...
排程（systemd timer，每分鐘一次；unit 寫法見 [[020-02-02-02-cmd-systemd-timer與cron選型]]）：
...
也是 [[100-02-08-guide-維運-變更管理流程]] 要求的演練證明。
...
>   在機關內網被側錄就是一次個資外洩事件（[[090-07-07-guide-資安實踐-台灣資安法規與個資法]]）。

| 機關情境的要求 | 做法 | 對應 |
| --- | --- | --- |
| **連線加密** ★★★★ | `repl` 帳號 `REQUIRE SSL`、`SOURCE_SSL=1` + `SOURCE_SSL_CA`；監控帳號 `ssl-mode=VERIFY_CA` | [[060-04-01-07-svc-MySQL-安全強化]] |
| **最小權限** ★★★★ | 複寫帳號只給 `REPLICATION SLAVE`、監控帳號只給 `REPLICATION CLIENT, PROCESS`，來源限內網 | [[060-04-01-02-cmd-MySQL-使用者與權限]] |
| **稽核軌跡** ★★★★ | 從庫的 `super_read_only` 每一次被關閉都要有紀錄；切換每一步寫進事件單 | [[100-02-09-svc-維運-事件處理與升級流程]] |
| **備份加密與異地** ★★★★ | 初始同步的產出是**完整資料庫明文複本**，用完要加密或安全刪除 | [[060-04-01-05-svc-MySQL-備份與還原]] |
...
導入前先讀 [[090-06-01-guide-TWGCB-TWGCB概念與法規要求]] 與 [[090-06-02-guide-TWGCB-Linux基準文件解讀]]
...
> **①** 從最近一次全備 + binlog 做 PITR，回到 `DROP` 前一秒（[[060-04-01-05-svc-MySQL-備份與還原]]）。
...
> [[080-01-03-guide-專管-風險與變更管理]] 的流程，並評估 `ALGORITHM=INPLACE, LOCK=NONE` 是否適用。
...
> 參見本篇「先把這件事講死：複寫不是備份」與 [[060-04-01-05-svc-MySQL-備份與還原]]。
>
> **Q2. 不代表。** ★★★★★
...
> ★★★★ **做完立刻設回 `ON`**，並把這一開一關記進事件單（[[100-02-09-svc-維運-事件處理與升級流程]]）。
...
> **③** 排程任務與第三方回呼。
> 給開發團隊的原則一句話：**「凡是需要看到自己剛寫的資料，就走主庫。」**
> 參見「應用端讀寫分離」與 [[070-03-04-guide-Laravel-Eloquent與資料庫]]。

---

## 延伸閱讀

- [[060-04-01-05-svc-MySQL-備份與還原]] — ★★★★ 本篇一直在說「那個要看這篇」的那篇。備份策略、PITR 與**還原演練**
- [[060-04-01-02-cmd-MySQL-使用者與權限]] — `repl` 與 `replcheck` 帳號的建立、來源限制與權限盤點
- [[060-04-01-04-svc-MySQL-設定檔與調校]] — `my.cnf` 載入順序、buffer pool 與記憶體預算（從庫規格不要低於主庫）
- [[060-04-01-07-svc-MySQL-安全強化]] — `bind-address`、TLS 憑證產製、稽核日誌；本篇的 `REQUIRE SSL` 前置作業
- [[060-04-02-07-svc-PostgreSQL-複寫與高可用]] — 對照組：PostgreSQL 的 WAL 流複寫與本篇的差異
- [[100-01-03-guide-日誌-系統監控與告警]] / [[100-01-04-guide-日誌-健康檢查與可用性監控]] — 把本篇的四個指標接進既有監控
- [[100-02-08-guide-維運-變更管理流程]] / [[100-02-09-svc-維運-事件處理與升級流程]] — 切換演練的公告、紀錄與事後檢討
- [[090-03-04-guide-應用安全-備份災難復原與入侵應變]] — 勒索與入侵情境下，為什麼從庫幫不上忙
