...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[010-02-13-guide-網概-HTTP與HTTPS]]", "[[010-02-14-guide-網概-一個網頁請求的完整旅程]]"]
updated: 2026-08-28
---
...
> ```
> 完整實戰見 [[130-01-00-idx-部署實戰]]。

## 前置知識

- [[010-02-13-guide-網概-HTTP與HTTPS]] — HTTP 協定的基礎
- [[010-02-14-guide-網概-一個網頁請求的完整旅程]] — 從瀏覽器到伺服器的全貌
...
| 關卡 | 做什麼 | 相關章節 |
| --- | --- | --- |
| **① 防火牆** | 只放行 80/443 | [[090-02-02-guide-防火牆-ufw基礎與實務]] |
| **② TLS 終止** | 解密 HTTPS，驗證憑證 | [[090-01-00-idx-PKI-憑證與PKI]] |
| **③ 虛擬主機** | 依 `Host` 標頭決定用哪組設定 | [[060-02-02-02-guide-Nginx-設定語法與虛擬主機]] |
| **④ WAF** | 檢查是否為攻擊請求 | [[090-04-00-idx-ModSecurity]] |
| **⑤ 路由** | 靜態直接回傳、動態轉給後端 | [[060-02-02-03-guide-Nginx-location與rewrite]] |
| **⑥ 後端** | PHP-FPM / Node.js 產生內容 | [[060-03-00-idx-應用執行環境]] |
...
>
> 詳見 [[060-02-04-guide-Web-Nginx與Apache選型與共存]]。
...
| 模組 | 作用 | 對應章節 |
| --- | --- | --- |
| **`autocert`** | **Nginx 內建 ACME 客戶端**，`autocert on;` 就自動申請與續期，**不需 certbot 與 cron** | [[060-02-02-06-guide-Nginx-HTTPS與Certbot]] |
| **`http-shield`** | 攔截 SQLi、Log4Shell、Shellshock、RCE 鏈等已知攻擊 | [[090-04-00-idx-ModSecurity]] |
| **`error-abuse`** | 對 404 濫用來源限流 | [[060-02-02-09-guide-Nginx-安全設定]] |
| **`sentinel`** | 用戶端信譽評分與 AI 爬蟲 tarpit（實驗中） | [[060-02-02-09-guide-Nginx-安全設定]] |
| **`cache-turbo`** | 共享記憶體邊緣快取、stale-while-revalidate | [[060-02-02-05-guide-Nginx-靜態資源與快取]] |
| **`strip-filter`** | HTML／CSS／JS／JSON 回應體精簡 | [[060-02-02-08-guide-Nginx-效能調校]] |
| **`zstd`** | Zstandard 壓縮 | [[060-02-02-08-guide-Nginx-效能調校]] |

> [!tip] `autocert` 是最有價值的功能
> **傳統做法**（見 [[060-02-02-06-guide-Nginx-HTTPS與Certbot]]）：
> ```
...
> ```
> □ 【評估供應鏈風險】（見 [[090-07-11-guide-資安實踐-委外與供應鏈資安]]）
> □ 確認套件庫的簽章金鑰與來源
...
| 層 | 元件 | 章節 |
| --- | --- | --- |
| **邊界** | 防火牆 | [[090-02-02-guide-防火牆-ufw基礎與實務]] |
| **Web** | Nginx / Apache | **本章** |
| **WAF** | ModSecurity + CRS | [[090-04-00-idx-ModSecurity]] |
| **憑證** | 申請 / 自簽 CA | [[090-01-00-idx-PKI-憑證與PKI]] |
| **前端** | Vue / Nuxt | [[070-02-00-idx-Vue與Nuxt]]、[[130-01-02-00-idx-Vue部署]]、[[130-01-03-00-idx-Nuxt部署]] |
| **執行環境** | PHP-FPM / Node.js + PM2 | [[060-03-00-idx-應用執行環境]] |
| **後端** | Laravel + Nova/Filament | [[070-03-00-idx-Laravel-PHP與Laravel]]、[[130-01-04-00-idx-Laravel部署]] |
| **資料** | MySQL / Redis | [[060-04-00-idx-資料庫]] |
| **部署** | Git → 正式環境 | [[060-01-01-08-guide-Git-伺服器端與自動部署]]、[[130-01-00-idx-部署實戰]] |
...
> ```
>
> **PM2 vs systemd 的完整比較見 [[060-03-02-03-guide-PM2-程序管理入門]]。**
...
> → 上傳目錄要明確禁止執行 PHP（見 [[060-02-02-09-guide-Nginx-安全設定]]）
> ```
...
- [[060-02-02-01-guide-Nginx-安裝與目錄結構]] — Nginx 的實作起點
- [[060-02-03-01-guide-Apache-安裝與目錄結構]] — Apache 的實作起點
- [[060-02-04-guide-Web-Nginx與Apache選型與共存]] — 兩者並存的完整設定
- [[060-03-00-idx-應用執行環境]] — PHP-FPM 與 Node.js
- [[090-01-00-idx-PKI-憑證與PKI]] — HTTPS 憑證
- [[090-04-00-idx-ModSecurity]] — WAF
- [[130-01-00-idx-部署實戰]] — **LXMP 全套整合實戰**
