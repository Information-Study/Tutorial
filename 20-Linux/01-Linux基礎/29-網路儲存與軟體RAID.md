...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[15-磁碟分割與掛載]]", "[[16-網路基礎指令]]"]
updated: 2026-08-27
---
...
| **集中帳號**：LDAP / AD（SSSD） | 正規做法，見 [[05-TWGCB-Linux網域導入]] |
| NFSv4 `idmapd` 名稱對應 | 兩端 domain 設定一致時，v4 可用名稱對應 |
| `all_squash` 全部匿名 | 公共唯讀分享 |

```bash
# NFSv4 名稱對應：兩端都設同一個 Domain
...
> AD 環境下用 `winbind`/SSSD 讓 Samba 直接用網域帳號，見 [[04-電腦加入網域與管理]]。
...
> 火災水災、同批磁碟同時壞。備份是另一件事，見 [[03-備份策略與還原演練]]。
...
> （與 [[24-進階儲存-ZFS與Btrfs]] 的 RAIDZ1 警告是同一件事。）
...
sudo mdadm --monitor --scan --test --oneshot        # 測試通知
```
...
> （[[03-系統監控與告警]]）。加上每月 SMART 檢查（[[15-磁碟分割與掛載]]）。
...
watch cat /proc/mdstat                      # 看 recovery 進度
```
...
sudo resize2fs /dev/md0                      # 或 xfs_growfs
...
> 而且不用 remount。新建家目錄伺服器考慮直接用 ZFS，見 [[24-進階儲存-ZFS與Btrfs]]。
...
sudo ufw allow from 192.168.0.0/16 to any port 2049 proto tcp
...
> | setgid + default ACL | 團隊共用目錄權限一致，見 [[08-檔案權限與擁有者]] |
...
> 對外絕不開 445——它是勒索軟體橫向移動的主要通道，見 [[03-常用電腦與使用者原則]] 的防火牆原則。
...
> 正規解法是集中帳號（LDAP/AD + SSSD），讓所有機器的 UID 天生一致——見 [[05-TWGCB-Linux網域導入]]。
...
- [[15-磁碟分割與掛載]] — fstab、UUID、SMART
- [[24-進階儲存-ZFS與Btrfs]] — 有校驗的替代方案
- [[08-檔案權限與擁有者]] — 共享目錄的 setgid 與 ACL
- [[09-使用者與群組管理]] — UID 規劃
- [[05-TWGCB-Linux網域導入]] — 集中帳號解決 UID 問題
- [[03-備份策略與還原演練]] — RAID 之外的保護
- [[03-系統監控與告警]] — 把 mdstat 接進監控
- [[03-PVE-儲存設定]] — PVE 上的 NFS/iSCSI 儲存
