...
tags: [群組/基礎概論, 網概/入門, 主題/計算機網路, 主題/總複習]
category: 計算機網路
difficulty: 入門
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[010-02-13-guide-網概-HTTP與HTTPS]]", "[[010-02-11-guide-網概-DNS網域名稱系統]]", "[[010-02-09-guide-網概-TCP與UDP]]"]
updated: 2026-08-29
---
...
- [[010-02-13-guide-網概-HTTP與HTTPS]]、[[010-02-11-guide-網概-DNS網域名稱系統]]、
  [[010-02-09-guide-網概-TCP與UDP]]、[[010-02-07-guide-網概-路由與封包旅程]]
...
>            ^^^^^^^^^^^^^ IP 是最終目的地   ★★ MAC 每跳都換，IP 從頭到尾不變
> ```
>
> 詳見 [[010-02-05-guide-網概-MAC位址與交換器]] 與 [[010-02-07-guide-網概-路由與封包旅程]]。
...
```
① SYN     →  「我要連線」
② SYN-ACK ←  「好，我也要」      ★★ 收到 RST 而不是 SYN-ACK，就是 refused
③ ACK     →  「確認」
```
...
>
> ```bash
> $ nc -zv example.com 443                              # ★★★ 先確認第 4 層通不通
...
② Server Hello + 憑證
③ 瀏覽器驗證憑證（信任鏈、SAN、有效期、撤銷狀態）   ★★★★ 整條 HTTPS 的信任就靠這步
④ 交換金鑰材料
⑤ 雙方算出對稱工作金鑰
⑥ Finished
```
...
> resolver 1.1.1.1 8.8.8.8 valid=300s;   # ★★★ 少了 resolver，stapling 不會生效
> ```
...
echo -e "\n[L7-DNS] 名稱解析"
dig +short "$DOMAIN"
dig @8.8.8.8 +short "$DOMAIN"   # ★★★ 和上一行答案不同 → 本機／內部 DNS 的問題

echo -e "\n[L4] 傳輸層"
nc -zv -w3 "$DOMAIN" 443   # ★★★ 第 4 層通不通
...
> 3. 防火牆擋掉 443 埠 → 應該卡在 ⑤　★★
> 4. 把預設閘道刪掉（`sudo ip route del default`）→ 應該卡在 ④　★★★★★ 只能在本機測試環境做，遠端主機下這行會當場斷線
>
> 每次做完記得復原。
> **這個練習會讓「分層排錯」變成你的直覺。**
...
- [[010-02-11-guide-網概-DNS網域名稱系統]] — 階段 ③
- [[010-02-05-guide-網概-MAC位址與交換器]] — 階段 ④
- [[010-02-09-guide-網概-TCP與UDP]] — 階段 ⑤
- [[010-02-13-guide-網概-HTTP與HTTPS]] — 階段 ⑥⑦⑩
- [[010-02-07-guide-網概-路由與封包旅程]] — 階段 ⑧
- [[010-02-17-guide-網概-網路排錯入門]] — 完整的排錯方法論
- [[090-05-01-guide-資安設備-資安全景圖與縱深防禦]] — 每一階段的防護（進階）
- [[060-02-00-idx-Web伺服器]] — 階段 ⑨ 的完整教學（進階）
