...
> - ★★★★ 知道機關與企業該選什麼線路，以及為什麼要有第二條 —— 備援沒規劃好，一次挖斷就是整個機關斷網
> - ★★★★ 理解 **CGNAT** 為什麼讓你架不了伺服器 —— 採購時漏看這點，服務根本上不了線
...
> ```bash
> $ traceroute 那個很慢的網站   # ★★★ 看它有沒有繞出國再繞回來
> ```
> 如果你看到封包從台灣繞到日本、再到美國、再繞回台灣 ——
> ★★★ 那就是路由不佳，而不是你的網路壞了 —— 打給 ISP 要提供這段 traceroute 才有用。
...
```bash
# Linux / macOS
$ traceroute -n www.google.com   # ★★★ -n 不做反解，卡住時輸出快很多

# Windows
> tracert www.google.com         # ★★ Windows 用 tracert，走 ICMP 不是 UDP
```
...
```
 ...
 2  100.64.12.1        8.512 ms      ← ★★★ ISP 的 CGNAT 節點（看到 100.64 就別想埠轉發）
 3  168.95.98.254     12.334 ms      ← 中華電信接取層
 4  220.128.x.x       13.001 ms      ← 中華電信骨幹
 5  203.75.x.x        14.223 ms      ← 國內交換中心
 6  72.14.x.x         45.882 ms      ← ★★ 進入 Google 網路（延遲跳升 = 跨海，正常）
...
$ speedtest-cli                   # ★★★ 機關要看的是 Upload 那一行，不是 Download
...
```bash
# ★★★ 1. 確認網卡協商速度（不是 100Mb/s 才對）—— 這是最常見的假故障
...
#!/usr/bin/env bash
# 檢查是否處於 CGNAT 之後
WAN_IP=$(ip route get 8.8.8.8 2>/dev/null | awk '{print $7; exit}')   # ★★ 本機出口位址，不等於對外 IP
REAL_IP=$(curl -s --max-time 5 ifconfig.me)   # ★★★ 網際網路實際看到的來源 IP

echo "本機對外介面 IP：$WAN_IP"
echo "網際網路看到的 IP：$REAL_IP"

case "$REAL_IP" in
  # ★★★ 100.64.0.0/10 是 CGNAT 專用段，落在這裡就等於沒有對外 IP
...
> - VPN 連線會不會斷掉？
>
> ★★★★ 沒演練過的備援，等於沒有備援 —— 真的斷線那天才發現沒切過去，就是整個機關對外中斷。
...
```
100.64.0.0 ～ 100.127.255.255    ← ★★★ CGNAT 專用範圍，看到就別談埠轉發
10.x.x.x / 172.16-31.x.x / 192.168.x.x    ← ★★★ 一般私有位址，同樣連不進來
```
...
> 到對面再翻譯回數位。★★ 光纖的正確名稱是 ONU/ONT，叫它「小烏龜」只是俗稱。
>
