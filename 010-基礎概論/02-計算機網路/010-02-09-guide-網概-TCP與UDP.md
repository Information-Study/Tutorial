...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[010-02-07-guide-網概-路由與封包旅程]]", "[[010-02-03-guide-網概-網路分層模型]]"]
updated: 2026-08-29
---
...
- [[010-02-03-guide-網概-網路分層模型]] — 傳輸層在第 4 層
- [[010-02-07-guide-網概-路由與封包旅程]] — IP 是「盡力而為」的
...
>
> ```bash
> # ★★★★ 用 tcpdump 找 zero window（排錯時最容易誤判成網路問題）
...
```bash
# ★★★ 終端機 1：抓封包（要先開抓包再發連線，順序反了就看不到 SYN）
...
> - ★★★ **限速**（rate limiting）
>
> 見 [[090-02-05-guide-防護-Fail2ban入侵防護]] 與 [[090-05-03-guide-資安設備-入侵偵測與防禦IDS-IPS]]。
...
>
> **Q4.**（★★★★ 看 tcpdump 最容易誤讀的一點）`ACK=1500` 的意思是「**1500 之前的資料我全部都收到了，
> 請從 1500 開始送**」，也就是「**我期待的下一個序號**」，
> 而不是「我收到了 1500 這一段」。
> 這叫**累積確認（cumulative ACK）** —— 一個 ACK 可以確認前面所有資料。
>
> **Q5.**（★★★ 記住「3 個重複 ACK」這個門檻）當傳送方**連續收到 3 個重複的 ACK** 時觸發
> （接收方一直說「我還是要 1500」）。
> 它比逾時重傳好在**不用等逾時**（逾時可能要等好幾百毫秒），
> **立刻重送**，大幅降低延遲。
>
> **Q6.**（★★★★ 兩者混淆會把磁碟瓶頸誤判成網路問題）**流量控制**解決「**接收方處理不過來**」——
...
> 靠掉包或延遲上升來判斷。
>
> **Q7.**（★★★ 解釋很多「網路好像變慢」的錯覺）因為 TCP 的**慢啟動（Slow Start）**機制 ——
> 連線一開始只送一點點，每收到一個 ACK 就加倍（指數成長），
> **在探測網路能承受多少**，要幾百毫秒才會達到全速。
> 這也是為什麼**很多小連線比一個大連線效率差**（每條都要重新慢啟動），
> HTTP/2 與 HTTP/3 的多工設計就是為了解決這個問題。
>
> **Q8.**（★★★★★ 判斷錯了就會亂調 sysctl，或放過真正會拖垮服務的 bug）**`TIME_WAIT` 很多通常不用擔心** ——
...
>
> **Q9.**（★★★★ 對外服務必備防護）**原理**：攻擊者送出大量 SYN 但**故意不回第三次 ACK**，
...
> **攻擊者送 SYN 卻不回 ACK，伺服器完全不受影響。**
>
> **Q10.**（★★★ 選型判斷，也是 HTTP/3 的由來）**視訊用 UDP**，因為**遲到的封包不如不要** ——
...
- [[010-02-10-guide-網概-連接埠與應用層協定]] — 埠號怎麼對應到服務
- [[010-02-03-guide-網概-網路分層模型]] — 傳輸層的位置
- [[010-02-13-guide-網概-HTTP與HTTPS]] — 建立在 TCP 之上的網頁協定
- [[010-02-14-guide-網概-一個網頁請求的完整旅程]] — 把交握放進完整流程
- [[010-02-17-guide-網概-網路排錯入門]] — 第 4 層的排錯
- [[020-01-26-guide-Linux-核心模組與sysctl調校]] — TCP 參數調校（進階）
- [[090-05-11-guide-資安設備-DDoS防護與CDN]] — SYN Flood 與放大攻擊防護（進階）
