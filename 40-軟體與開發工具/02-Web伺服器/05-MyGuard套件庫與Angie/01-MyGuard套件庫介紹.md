...
status: 完成
distro: [ubuntu]
prerequisites: ["[[01-Nginx-安裝與目錄結構]]", "[[14-套件管理]]"]
updated: 2026-08-28
---
...
- [[01-Nginx-安裝與目錄結構]] — NGINX 的基礎
- [[14-套件管理]] — APT 與套件庫
- [[03-第三方APT套件庫實務]] — 第三方套件庫的通用做法
...
| 模組 | 作用 | 對應章節 |
| --- | --- | --- |
| **`autocert`** | **★★★★ NGINX 內建 ACME 客戶端**，`autocert on;` 就自動申請與續期，**不需要 certbot 與 cron** | [[03-autocert自動憑證模組]] |
| **`http-shield`** | 攔截 SQLi、Log4Shell、Shellshock、RCE 鏈等已知攻擊 | [[04-http-shield攻擊攔截]] |
| **`error-abuse`** | 對 4xx/5xx 濫用的來源限流與封鎖 | [[05-error-abuse與sentinel]] |
| **`sentinel`** | **★★ 用戶端信譽評分與 AI 爬蟲 tarpit**（實驗中） | [[05-error-abuse與sentinel]] |
| **`cache-turbo`** | 共享記憶體的邊緣快取、stale-while-revalidate | [[06-cache-turbo與壓縮模組]] |
| **`strip-filter`** | HTML／CSS／JS／JSON 回應體精簡 | [[06-cache-turbo與壓縮模組]] |
| **`zstd`** | Zstandard 壓縮 | [[06-cache-turbo與壓縮模組]] |
...
★★★★ 不支援時的處理：
  ① 升級系統到支援的版本
  ② ★★ 用官方套件庫 + 自行編譯需要的模組
  ③ ★★ 用官方提供的 Docker 映像（見 [[08-MyGuard實戰組合]]）
```
...
| **`nginx -t` 說 unknown directive** ★★★★ | **模組沒載入** | `load_module`；見 [[07-動態模組管理]] |
...
> `autocert_staging on;`（測試用）。詳見 [[03-autocert自動憑證模組]]。
>
...
- [[02-Angie伺服器入門]] — Angie 是什麼、和 NGINX 的差異
- [[03-autocert自動憑證模組]] — **★★★★ 不用 certbot 的自動憑證**
- [[07-動態模組管理]] — 模組的安裝與載入
- [[08-MyGuard實戰組合]] — 完整的實戰配置
- [[03-第三方APT套件庫實務]] — 第三方套件庫的通用做法
- [[01-Nginx-安裝與目錄結構]] — NGINX 基礎
