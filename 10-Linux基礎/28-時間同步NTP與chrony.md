...
status: 完成
distro: [ubuntu, rhel]
prerequisites: ["[[17-systemd服務管理]]", "[[16-網路基礎指令]]"]
updated: 2026-08-27
---
...
| 症狀 | 真正原因 |
| --- | --- |
| `certificate is not yet valid` / `has expired`（其實憑證沒問題） | 本機時間落後或超前 |
| Kerberos `Clock skew too great`、AD 登入失敗 | 與 DC 差超過 5 分鐘 |
| TOTP（Google Authenticator）驗證碼一直錯 | 時間差超過 30 秒 |
| 日誌時間對不上、事故時間線兜不攏 | 各機器時間不一致 |
| cron 在奇怪的時間跑 | 時區或時間錯 |
...
> 十秒鐘排除一整類原因。這是 [[23-Linux常見疑難排解]] 的第 9 類。

### 三個名詞

| 名詞 | 意義 |
| --- | --- |
| **RTC**（硬體時鐘） | 主機板上的電池時鐘，關機時繼續走，精度差（每天可漂移數秒） |
| **系統時鐘** | 核心維護的時間，開機時從 RTC 讀入，之後由 NTP 校正 |
| **NTP**（Network Time Protocol） | 透過網路與時間源比對並持續微調系統時鐘的協定，UDP 123 |
...
Stratum 是「離原子鐘幾層」，數字越小越準；`16` 代表未同步。
...
| **step** | 直接跳到正確時間 | **時間倒退**會讓資料庫、日誌、cron 混亂 |
...
System clock synchronized: yes          ← 這行最重要
              NTP service: active
...
[Time]
NTP=ntp1.example.internal ntp2.example.internal
FallbackNTP=time.stdtime.gov.tw tw.pool.ntp.org
C
...
systemctl status chrony                  # RHEL: chronyd
```
...
makestep 1.0 3                            # 前 3 次校正若差 >1 秒允許直接跳
rtcsync                                   # 定期把系統時間寫回 RTC
...
System time     : 0.000214 seconds fast of NTP time     ← 目前偏差
...
| **`^?`** | **無法連線**（防火牆、DNS、對方沒開） |
| `^x` | 判定為錯誤（falseticker） |
| `Reach 377` | 八進位，最近 8 次全部成功；`0` = 全失敗 |
...
chronyc ntpdata ntp1.example.internal    # 詳細封包資訊
```
...
> | 服務名 | `chrony` | **`chronyd`** |
> | 設定檔 | `/etc/chrony/chrony.conf` | **`/etc/chrony.conf`** |
...
```
# VM 的 chrony 建議
makestep 1.0 -1          # -1 = 任何時候差 >1 秒都允許 step（VM 恢復後快速追上）
```
...
Kerberos 預設容忍 **5 分鐘**時間差，超過就是 `Clock skew too great`。
...
Linux 加入 AD 時（[[05-TWGCB-Linux網域導入]]），chrony 指向 DC 或同一個內部 NTP：

```
server dc1.example.internal iburst
server dc2.example.internal iburst
```

Windows 端 PDC 的設定見 [[02-網域控制站建置]]（`w32tm /config /manualpeerlist:...`）。
...
server time.cloudflare.com iburst nts
pool tw.pool.ntp.org iburst maxsources 3
peer ntp2.example.internal                # ntp2 上寫 ntp1
driftfile /var/lib/chrony/chrony.drift
makestep 1.0 3
rtcsync
leapsectz right/UTC
...
sudo systemctl restart chrony
sudo ufw allow from 192.168.0.0/16 to any port 123 proto udp
sleep 30; chronyc tracking; chronyc sources -v
...
# ═══ 3. VM 額外 ═══
# makestep 1.0 -1；裝 qemu-guest-agent；不要開平台的時間同步
...
> 把 `check-ntp.sh` 接進監控，把「全設備時間差」列入 [[05-每季維護作業]]。
...
- [[16-網路基礎指令]] — UDP 埠測試與防火牆
- [[17-systemd服務管理]] — timesyncd 與 chrony 服務管理
- [[18-排程工作]] — 把時間檢查排進 timer
