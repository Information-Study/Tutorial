...
> 0 3 * * * root /usr/local/bin/backup.sh
> ```
> 但**跨時區環境的最佳做法是全部用 UTC**，見 [[02-實驗環境準備與初次登入]]。
...
> 這是 `netplan try` 的通用版本，見 [[16-網路基礎指令]]。
...
- [[17-systemd服務管理]] — unit 檔的完整語法與安全強化
- [[19-日誌系統]] — `journalctl` 與日誌輪替
- [[22-Shell腳本進階]] — 排程腳本的錯誤處理與鎖檔
- [[03-備份策略與還原演練]] — 備份排程的完整設計
- [[04-每月維護作業]] — 排程稽核
- [[08-系統強化與稽核]] — `cron.allow` 與排程安全
