...
> 混用兩個工具時特別容易搞錯，見 [[02-rsync-同步與備份]]。
>
...
> 但記住**別名不會在腳本與 sudo 中生效**（見 [[03-終端機與Shell入門]]）。
...
>    ```
>    `${VAR:?訊息}` 是 Shell 內建的防呆語法，見 [[22-Shell腳本進階]]。
...
> 否則等於把機密複製到一個大家都讀得到的地方。見 [[10-機密管理與金鑰保護]]。
...
>
> 更完整的部署流程見 [[04-Laravel-快取最佳化與部署流程]] 與 [[06-部署自動化]]。

---

## 延伸閱讀

- [[06-檢視檔案內容]] — 檔案找到了，接著要看裡面有什麼
- [[08-檔案權限與擁有者]] — `ls -l` 第一欄的完整解讀
- [[07-尋找檔案與內容]] — 用 `find` 精準定位檔案
- [[13-壓縮與封存]] — `tar` 打包與備份
- [[02-rsync-同步與備份]] — 比 `cp` 更適合備份的工具
- [[15-磁碟分割與掛載]] — inode 用盡與磁碟空間問題
