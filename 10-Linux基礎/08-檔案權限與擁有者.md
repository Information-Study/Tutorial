...
> 服務產生的檔案權限要在 unit 檔裡設定。見 [[17-systemd服務管理]]。
...
> 有沒有 `avc: denied`，那就是 SELinux。詳見 [[07-SELinux與AppArmor]]。
...
> 或 PHP-FPM pool 的 `env[]` 指令，見 [[10-機密管理與金鑰保護]]。
>
...
- [[07-尋找檔案與內容]] — `find -perm` 稽核權限
- [[07-SELinux與AppArmor]] — 權限對了卻仍被拒絕時
- [[02-Laravel-Nginx與PHP-FPM設定]] — Web 應用權限的完整實作
- [[10-機密管理與金鑰保護]] — `.env` 與私鑰的正確處理
- [[08-系統強化與稽核]] — 把權限稽核制度化
