...
> | cron 裡指令失敗 | cron 只有極簡環境（見 [[18-排程工作]]） | 腳本自帶 `export PATH=...` |
> | systemd 服務讀不到變數 | systemd 不經過 shell | `EnvironmentFile=`（見 [[17-systemd服務管理]]） |
>
...
最後一行在 [[05-路徑導覽與檔案操作]] 提過——**這是保命語法**。

### 讀取 `.env` 檔案

應用程式常用 `.env` 存設定（見 [[08-前後端分離的環境變數與建置流程]]）：
...
> | 位置 | 為什麼危險 |
> | --- | --- |
> | 指令列參數 | `ps aux` 所有人可見（見 [[10-程序管理與訊號]]） |
...
> ```
> 全域啟動檔應為 `644 root:root`。這是 [[08-系統強化與稽核]] 的項目之一。
...
- [[03-終端機與Shell入門]] — shell 的基本概念
- [[18-排程工作]] — cron 的環境問題完整說明
- [[17-systemd服務管理]] — `EnvironmentFile=` 用法
- [[21-Shell腳本入門]] — 在腳本中使用變數
- [[10-機密管理與金鑰保護]] — 機密的正確存放
- [[03-Bash與Zsh效率設定]] — dotfiles 的組織與同步
