...
- [[10-網概-連接埠與應用層協定]] — DNS 用 UDP/TCP 53
- [[06-網概-IP位址與子網路]] — IP 位址
...
| **NS** | Name Server | **誰管理這個網域** | `example.com → ns1.example.com` |
...
>
> ✅ www.example.com.  CNAME  something.cdn.com.
> ```
>
> 想在根網域指向 CDN？
...
>
> ```
> 搬遷前 3 天：把 TTL 從 86400 改成 300（5 分鐘）
...
;; Query time: 24 msec
;; SERVER: 192.168.1.1#53(192.168.1.1)     ← 用哪台 DNS 查的
```
...
Non-authoritative answer:      ← 「非權威」= 從快取來的
Name:   example.com
...
> **正確做法**：
> ```
> 內部 DNS：解析 internal.example.com → 10.10.20.5
> 外部 DNS：只有 www、mail 等對外服務的記錄
> ```
>
> 檢查方式：
> ```bash
> # 從外部（用公用 DNS）查詢內部名稱，應該要查不到
> $ dig @8.8.8.8 +short fileserver.example.com
> # 應該沒有輸出
>
> # 嘗試區域轉送（應該被拒絕）
...
- [[10-網概-連接埠與應用層協定]] — 為什麼 DNS 要開 TCP 與 UDP 53
- [[12-網概-DHCP自動取得設定]] — DNS 伺服器位址怎麼取得
- [[13-網概-HTTP與HTTPS]] — DNS 之後的下一步
- [[14-網概-一個網頁請求的完整旅程]] — DNS 在整趟旅程中的位置
- [[17-網概-網路排錯入門]] — DNS 排錯流程
- [[09-日誌集中與SIEM]] — DNS 日誌的資安價值（進階）
- [[06-郵件與網頁閘道防護]] — SPF/DKIM/DMARC 實作（進階）
